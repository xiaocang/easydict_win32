...
- **Hover Word Lookup** (悬浮取词): Hold a trigger key (Ctrl by default; None/Shift/Alt) and rest the pointer on a word → non-activating popup shows the word, phonetics and meaning. Reads the word via UI Automation first, then OCR of the pixels around the pointer. Toggle in Settings → Labs or the tray menu
...
- **PopButtonService**: Orchestrates the lifecycle — on selection detected, waits 150ms after a drag (after a multi-click, only whatever is left of the pop delay once the hook's settle wait is subtracted, capped at 30ms — usually nothing, since the settle wait already spends the budget), queries `TextSelectionService` for selected text, shows icon at cursor position, auto-dismisses after 5s
...
- **Settings**: `MouseSelectionTranslate` in SettingsService (default: on), toggle in Settings → Behavior; `MouseSelectionPopDelayMs` (default 220ms, range 10–600) is a slider in a collapsed-by-default Selection pop-up delay expander directly below Mouse selection translate in Settings → General → Behavior, shown together with excluded apps when the feature is enabled. Quick-action button visibility has its own collapsed expander. It is an upper bound on the whole pre-query wait, not on each step: `MouseHookService` caps the multi-click settle wait with it and reports the wait it actually spent, so `PopButtonService.GetSelectionDelayMs` only pays out what is left of the budget. The 10ms floor pops the icon as soon as the source app can be read (never 0 — reading a selection with no wait races the source app).
- **Flow**: `MouseHookService.OnDragSelectionEnd` / `OnMultiClickSelectionEnd` (point + settle wait) → `PopButtonService.OnDragSelectionEnd` / `OnMultiClickSelectionEnd` → `TextSelectionService.GetSelectedTextAsync` → `PopButtonWindow.ShowAt` → user clicks → `MiniWindowService.ShowWithText`
...
- **Feature placement**: New unstable features start in Settings → Labs and graduate to the regular tabs when stable. Individual experimental badges are unnecessary inside Labs. Custom dictionary input suggestions are stable and remain in General without an experimental badge.
- **Dwell setting**: `HoverWordLookupDelayMs` (10–600 ms, default 350 ms) is persisted, clamped on load/save assignment, passed through `HoverLookupOptions.DelayMs`, and used by `HoverDwellDetector.Tick`. Settings → Labs → Hover Word Lookup keeps the trigger key visible; delay slider, service choice, and OCR fallback are inside a collapsed-by-default Advanced settings expander. Saving reapplies options without restarting. The 50 ms modifier-hold guard and timer interval still apply; extraction/translation time is additional.
- **HoverWordLookupService** (`Services/HoverWordLookupService.cs`): Orchestrator. Consumes `MouseHookService.OnMouseMove` / `OnKeyboardEvent` (vk-aware key down + key up) on the UI thread, tracks the trigger key through `HoverLookupKeyHold` (at least 50 ms continuously held; release resets the clock, repeated key-down does not), runs a 50 ms `DispatcherQueueTimer` only while the trigger condition holds, and starts a lookup when `HoverDwellDetector` fires. Lookups run on the thread pool under a CTS (6-second extraction timeout, then separate per-service timeouts) and report back through the dispatcher with a generation counter so stale results never show
- **HoverDwellDetector** (`Services/HoverDwellDetector.cs`): Pure pointer-rest detector — configurable 10–600 ms dwell (default 350 ms), 3 px jitter tolerance, fires once per rest, re-arms after 8 px of movement; `Rearm()` lets the same rest fire again after a dismiss / trigger-key release
...
- **HoverLookupWindow** (`Views/HoverLookupWindow.xaml`): Non-activating card (`WS_EX_NOACTIVATE | WS_EX_TOOLWINDOW | WS_EX_TOPMOST`, `WDA_EXCLUDEFROMCAPTURE`, never `Activate()`) with word, phonetics, meaning, service name and an "open in Mini window" button. Measures the inner content at the final wrapping width, refits after layout, and caps height at the monitor work area, clipping overflowing content without scrolling. Uses the anchor monitor's DPI (`DpiHelper.GetDpiForPoint`), placed by `HoverLookupPlacement` (below the word, else above, clamped to the work area)
- **Translation**: `HoverLookupRules.SelectServiceIds` orders the configured service first, then enabled `youdao` → `google_web` → `mdx::*` → remaining usable enabled services, without duplicates. `HoverLookupFallback` calls `TranslationManager.TranslateAsync(request, ct, serviceId)` sequentially (manager cache + phonetic enrichment), skipping failures, timeouts and results without displayable meanings until the first useful result. Word extraction has a 6-second timeout; each service gets its own 5-second timeout, and dismissal cancels the whole chain. Target language from `TargetLanguageSelector.ResolveAutoTargetLanguage` after `HoverLookupRules.GuessSourceLanguage(word, firstLanguage, secondLanguage)` infers a compatible script from the configured language pair (first preferred when ambiguous); text formatted by `HoverLookupContentBuilder`
- **Dismiss rules**: pointer leaves the safe zone (word rect ∪ popup rect, inflated 12 DIP); click / scroll / right click / any non-modifier key (`DismissForInput` also blocks further lookups until the trigger key is released, so Ctrl+C never triggers a lookup); feature disabled; pointer moving > 16 px away from the dwell point cancels an in-flight lookup; own-process windows, an active screen capture and drags are skipped. Hover lookup does not use the mouse-selection excluded-app list.
- **Settings**: `HoverWordLookupEnabled` (default: off), `HoverWordLookupModifier` (`HoverLookupModifier` name, default `Ctrl`), `HoverWordLookupUseOcrFallback` (default: on), `HoverWordLookupServiceId` (`""` = Auto); UI in Settings → Labs, plus a checkable tray menu item (`TrayIconService.OnHoverWordLookupToggled`). `SettingsService.HoverWordLookupEnabledChanged` synchronizes an open Settings page after tray changes without discarding other pending edits; the page unsubscribes on teardown
- **Saved items**: hover queries handed to the Mini window use `QuerySourceKind.Hover` (stored as `"hover"`)
- **UI regression tests**: `HoverLookupLayoutTests` drives the real popup with deterministic fixtures through `WINUI_TEST`-only messages (requires `EasydictUiTestBuild=true`), covering source wrapping, 12-line result growth and shrink, work-area height limits, overflow clipping without scrolling, scroll dismissal, and repeated loading transitions. Checks native window bounds and visible UIA content; no OCR or translation network calls. `HoverLookupSettingsTests` exercises the tray handler with the page open and verifies both toggle directions survive an unrelated save. Both classes run in the Core & Settings CI shard using direct EXE launch so temporary settings reach the process. The tray test uses a settings-only fixture without bootstrapping History or requiring a 1280-DIP desktop. Immediate-scroll coverage checks the outer scroller in narrow Settings and the detail scroller in wide Settings.
...
4. As soon as the region is captured, `MiniWindowService.ShowBusy()` puts the mini window on screen with a "Recognizing text…" spinner, so recognition never runs with nothing on screen; a failure replaces it with the reason via `ClearBusy()` (silent OCR stays silent — no window). Each pipeline invocation owns its status through a token, so a request cancelled by a newer one cannot clear the newer one's spinner as it unwinds
...
- Store listing metadata is maintained as YAML files in `.winstore/listings/`, one per language (en-us, zh-cn, zh-tw, ja-jp, ko-kr)
- **Store listings only support 5 languages** (en-us, zh-cn, zh-tw, ja-jp, ko-kr), which is a smaller set than the app's 15 UI languages. Do NOT add new store listing languages without explicit approval.
