...
- **Dwell setting**: `HoverWordLookupDelayMs` (10–600 ms, default 350 ms) is persisted, clamped on load/save assignment, passed through `HoverLookupOptions.DelayMs`, and used by `HoverDwellDetector.Tick`. Settings → Behavior → Hover Word Lookup keeps the trigger key visible; delay slider, service choice, and OCR fallback are inside a collapsed-by-default Advanced settings expander. Saving reapplies options without restarting. The 50 ms modifier-hold guard and timer interval still apply; extraction/translation time is additional.
