...
- **悬浮取词（实验性）** - 默认关闭，可在托盘菜单或「设置 → 行为」中启用。持续按住 `Ctrl` 至少 50ms（触发键可配置，也可不按键），并将鼠标停在任意单词上，即可在指针旁弹出小窗显示单词、音标和释义。通过 UI 自动化读取文本，无法读取时自动回退到 OCR。优先使用选定的取词服务，请求失败、超时或无释义时，依次回退到后续已启用的服务
