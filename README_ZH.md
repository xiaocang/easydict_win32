...
- **文本动作** - 对选中或输入的文本执行某一个指定服务，或把它发送到 Google、Bing、GitHub、维基百科、百度，以及你自定义的 URL 模板。可在任意窗口的"动作"菜单中使用，也可以选择显示在划词按钮条上（设置 → 高级 → 文本动作）

- **Bob 插件兼容** - 导入 Bob 生态的 `.bobplugin` 翻译插件并作为翻译服务使用。插件运行在内嵌的 JavaScript 引擎中，默认采用保守策略（手动查询、不缓存、不重试），可按插件逐项开启；每个已安装的插件都会在划词按钮条上获得自己的按钮；插件结果始终带有"Bob 插件"徽章，不会与内置服务混淆
