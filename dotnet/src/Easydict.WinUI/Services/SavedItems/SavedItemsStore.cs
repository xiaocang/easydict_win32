...
public sealed class SavedItemsStore
{
    ...
    private static string ToDb(QuerySourceKind kind) => kind switch { QuerySourceKind.Manual => "manual", QuerySourceKind.Clipboard => "clipboard", QuerySourceKind.Selection => "selection", QuerySourceKind.Ocr => "ocr", QuerySourceKind.HistoryRerun => "history_rerun", QuerySourceKind.Hover => "hover", _ => throw new ArgumentOutOfRangeException(nameof(kind)) };
