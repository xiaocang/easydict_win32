...
namespace Easydict.WinUI.Views
{
    ...
    public partial class MainPage : Page
    {
        ...
        private bool _deferLoadedThemeChrome;
        ...
        private Services.LongDocumentTranslationService LongDocumentService =>
            _longDocumentService ??= new Services.LongDocumentTranslationService();

        private LongDocumentDeduplicationService LongDocDedupService =>
            _longDocDedupService ??= new LongDocumentDeduplicationService();
        ...
        public MainPage()
        {
            ...
            // One language bar is retained across responsive layouts.
        ...
        private void EnsureLongDocFeaturesInitialized()
        {
            ...
            _longDocFeaturesInitialized = true;
        }

        public void ApplyThemeChrome(bool refreshServiceResults = true)
        {
            ...
            SourcePlayButton.Visibility = SettingsService.Instance.ShowSourcePlayButton
        ...
        private void ApplyMainLayoutChrome(bool minimal, bool compact)
        {
            if (minimal)
            {
                ...
                QuickContentGrid.Padding = new Thickness(4);
                LongDocContentGrid.Padding = new Thickness(4);
                ActionBarWide.Margin = new Thickness(0, 4, 0, 4);

                QuickInputCard.Margin = new Thickness(0, 0, 0, 2);
                ...
                LongDocInputCardContent.Margin = new Thickness(4);
                LongDocOutputCard.Margin = new Thickness(0, 2, 0, 0);
                ...
                TranslateButton.Margin = new Thickness(0);
        ...
        {
            StatusText.Text = MinimalThemeService.IsActive
        ...
        private void SetStatusSummary(string text, bool important = false)
        {
            _lastStatusSummaryText = text;
            _lastStatusSummaryIsImportant = important;
            ApplyStatusSummaryChrome();
        }

        private void ApplyStatusSummaryChrome()
        {
            StatusSummaryText.Text = _lastStatusSummaryText;
            StatusSummaryText.Visibility = string.IsNullOrEmpty(_lastStatusSummaryText) ? Visibility.Collapsed : Visibility.Visible;
        ...
        private void ApplyQuickFooterChrome(bool compact)
        {
            ...
            var showFooter = !string.IsNullOrEmpty(_lastStatusSummaryText)
        ...
        private void ApplyTranslateButtonsChrome()
        {
            ...
            _longDocTranslateButtonDefaultContent ??= LongDocTranslateButton.Content;

            var minimal = MinimalThemeService.IsActive;
            var text = _isQuerying
                ? LocalizationService.Instance.GetString("Cancel")
                : LocalizationService.Instance.GetString("TranslateTooltip");
            if (string.IsNullOrWhiteSpace(text) || text == "TranslateTooltip")
            {
            ...
            ApplyTranslateButtonChrome(
                ...
                minimal,
                _isLongDocTranslating ? "Cancel" : "Translate",
                normalWidth: 40,
                normalHeight: 40);
        }

        private static void ApplyTranslateButtonChrome(
            Button button,
            object? normalContent,
            bool minimal,
            string text,
            double normalWidth,
            double normalHeight)
        ...
        private async Task CleanupResourcesAsync()
        {
            ...
            _longDocFeaturesInitialized = false;
        ...
        private void ApplyModeState(
            ...
            bool initializeLongDocFeatures = true,
            bool refreshServiceResultChrome = true)
        {
            var loc = LocalizationService.Instance;
            HideSuggestionPopup();
            ...
            if (isLongDoc && initializeLongDocFeatures)
            {
            ...
            ApplyThemeChrome(refreshServiceResults: refreshServiceResultChrome);
        }
        ...
        private async Task<bool> SwitchModeAsync(QueryMode newMode)
        {
            ...
            try
            {
                ...
                if (showLoading)
                {
                    await ApplyModeStateForModeSwitchAsync(newMode);
                }
                else
                {
                    ApplyModeState();
        ...
        private async Task ApplyModeStateForModeSwitchAsync(QueryMode expectedMode)
        {
            var isLongDoc = expectedMode == QueryMode.LongDocument;

            ApplyModeState(
                reinitializeServiceResults: false,
                initializeLongDocFeatures: false,
                refreshServiceResultChrome: false);

            await YieldToDispatcherAsync(Microsoft.UI.Dispatching.DispatcherQueuePriority.Normal);
            if (_isClosing || _currentMode != expectedMode)
            {
                return;
            }
            ...
            if (isLongDoc)
            {
                ...
                await YieldToDispatcherAsync(Microsoft.UI.Dispatching.DispatcherQueuePriority.Low);
                if (_isClosing || _currentMode != expectedMode)
                {
                    return;
                }
            ...
            ApplyThemeChrome(refreshServiceResults: true);
        }
                ...
                Microsoft.UI.Dispatching.DispatcherQueuePriority.Normal)
        {
            ...
            if (!DispatcherQueue.TryEnqueue(priority, () => tcs.TrySetResult()))
            {
                tcs.TrySetResult();
            }
        ...
        private void InitializeServiceResults(bool skipRebuildWhenDebugFlagSet = false, string reason = "Unspecified")
        {
            ...
            if (TryReuseServiceResultControls(descriptors, reason))
            {
#if DEBUG
        ...
        private void OnQuickTranslateContentViewChanged(object? sender, ScrollViewerViewChangedEventArgs e)
        {
            ...
            ServiceResultViewHost.UpdateStickyHeaders(_resultControls, _quickWideLayout ? QuickResultsScrollViewer : QuickTranslateContent);
        }
        ...
        private void RefreshServiceResultView(ServiceQueryResult serviceResult)
        {
            ServiceResultViewHost.Refresh(_resultControls, serviceResult);
        }

        private async Task<TranslationLanguage> DetectSourceLanguageForQueryAsync(
            string inputText,
        ...
        private async Task StartQueryAsync(QuerySourceKind sourceKind = QuerySourceKind.Manual)
        {
            ...
            if (_detectionService is null)
            {
                SetStatusSummary(LocalizationService.Instance.GetString("ServiceNotInitialized"), important: true);
                InitializeTranslationServices();
                return;
            }
            ...
            try
            {
                ...
                DispatcherQueue.TryEnqueue(() =>
                {
                    ...
                    {
                        SetStatusSummary(string.Format(loc.GetString("ServiceResultsComplete"), summary.SuccessCount));
                        UpdateStatus(true, loc.GetString("StatusReady"));
                    }
                    else if (summary.ErrorCount > 0)
                    {
                        SetStatusSummary(loc.GetString("TranslationFailed"), important: true);
                        UpdateStatus(false, loc.GetString("StatusError"));
                    }
                    else
                    {
                        SetStatusSummary(string.Empty);
                        UpdateStatus(true, loc.GetString("StatusReady"));
                    }
                });
            }
            ...
            catch (Exception ex)
            {
                if (!_isClosing)
                {
                    SetStatusSummary($"Error: {ex.Message}", important: true);
                    UpdateStatus(false, LocalizationService.Instance.GetString("StatusError"));
            ...
            CancellationToken ct)
        {
            ...
            DispatcherQueue.TryEnqueue(() =>
            {
                ...
                if (successCount > 0)
                {
                    SetStatusSummary(string.Format(
                ...
                {
                    SetStatusSummary(loc.GetString("TranslationFailed") ?? "Check failed", important: true);
                    UpdateStatus(false, loc.GetString("StatusError"));
                }
                else
                {
        ...
        private async Task ProcessLongDocQueueAsync(List<string> filePaths, string serviceId, string outputFolder, LongDocumentInputMode mode, LayoutDetectionMode layoutDetection, CancellationToken cancellationToken)
        {
            ...
            for (var i = 0; i < filePaths.Count; i++)
            {
                ...
                var dedupKey = await LongDocDedupService.CreateDedupKeyAsync(
                    mode,
                    filePath,
                    serviceId,
                    from,
                ...
                var existingPath = await LongDocDedupService.TryGetExistingOutputPathAsync(dedupKey, cancellationToken);
                ...
                var result = await LongDocumentService.TranslateToPdfAsync(
                    mode,
                    filePath,
                    from,
                    to,
                ...
                {
                    await LongDocDedupService.RegisterOutputAsync(dedupKey, result.OutputPath, cancellationToken);
                    completed++;
        ...
        private async void OnLongDocTranslateClicked(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                _longDocLastDedupKey = await LongDocDedupService.CreateDedupKeyAsync(
                    mode,
                    input,
                    serviceId,
                    _longDocLastFrom,
                ...
                var existingOutputPath = await LongDocDedupService.TryGetExistingOutputPathAsync(_longDocLastDedupKey, cancellationToken);
                ...
                if (result.State == LongDocumentJobState.Completed)
                {
                    await LongDocDedupService.RegisterOutputAsync(_longDocLastDedupKey, result.OutputPath, cancellationToken);
        ...
        private async void OnLongDocRetryClicked(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                var result = await LongDocumentService.RetryFailedChunksAsync(
                    _longDocCheckpoint,
                    _longDocLastFrom,
                    _longDocLastTo,
                    outputPath,
        ...
        {
            await ShowPageNavigationLoadingAsync();
            try
            {
            ...
            {
                Debug.WriteLine($"[MainPage] Failed to navigate to settings: {ex.Message}");
                CrashDiagnostics.LogException("MainPage.NavigateSettings", ex, isTerminating: false, isHandled: true);
                HidePageNavigationLoading();
            }
        }

        private async Task ShowPageNavigationLoadingAsync()
        {
            if (PageNavigationLoadingOverlay is null || PageNavigationLoadingRing is null)
            {
                return;
            }

            PageNavigationLoadingOverlay.Visibility = Visibility.Visible;
            PageNavigationLoadingRing.Visibility = Visibility.Visible;
            PageNavigationLoadingRing.IsActive = true;
            SettingsButton.IsEnabled = false;

            await Task.Delay(PageNavigationRenderDelayMs);
        }

        private void HidePageNavigationLoading()
        {
            if (PageNavigationLoadingOverlay is null || PageNavigationLoadingRing is null)
            {
                return;
            ...
            PageNavigationLoadingRing.IsActive = false;
            PageNavigationLoadingOverlay.Visibility = Visibility.Collapsed;
            ...
            LongDocumentInputMode inputMode, CancellationToken ct)
        {
            ...
            var downloadService = LongDocumentService.GetLayoutModelDownloadService();
