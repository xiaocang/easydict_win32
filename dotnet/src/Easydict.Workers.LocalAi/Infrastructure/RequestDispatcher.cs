...
internal sealed class RequestDispatcher
{
    ...
    /// <summary>
    /// Currently a no-op placeholder used by streaming handlers that want to know
    /// "my own request id" without it being threaded as a parameter. Today the
    /// request id is passed explicitly through HandlerDelegate, so this method
    /// is not needed and returns null. Kept for forward compatibility.
    /// </summary>
    public string? LookupRequestId() => null;
