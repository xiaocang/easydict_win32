...
/// <summary>
/// Integration tests for NiuTransService using real API calls.
...
public class NiuTransServiceIntegrationTests : IDisposable
{
    ...
    public NiuTransServiceIntegrationTests()
    {
        _apiKey = Environment.GetEnvironmentVariable("NIUTRANS_API_KEY");
    ...
    {
        Skip.If(string.IsNullOrEmpty(_apiKey), "NIUTRANS_API_KEY not set");
