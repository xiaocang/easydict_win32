...
public sealed class MemoryGateTests : IDisposable
{
    ...
    [Fact]
    public void InterfaceSwitching_ShouldStayResponsiveAcrossMainSettingsAndFloatingSurfaces()
    {
        var window = _launcher.GetMainWindow(TimeSpan.FromSeconds(60));
        window.Should().NotBeNull("main window must be available for the interface responsiveness regression");
        WritePhaseMarker("freeze-00-main-window-observed");

        window.SetForeground();
        Thread.Sleep(500);

        MeasureResponsiveStep("translation -> long document", TimeSpan.FromSeconds(15), () =>
            SwitchToLongDocMode(window));

        MeasureResponsiveStep("long document controls", TimeSpan.FromSeconds(18), () =>
            ExerciseLongDocControls(window));

        MeasureResponsiveStep("long document -> translation", TimeSpan.FromSeconds(15), () =>
            SwitchToQuickTranslateMode(window));

        MeasureResponsiveStep("settings tabs and return", TimeSpan.FromSeconds(55), () =>
            OpenSettingsExerciseTabsAndReturn(window));

        MeasureResponsiveStep("post-settings translation/long document roundtrip", TimeSpan.FromSeconds(25), () =>
        {
            SwitchToLongDocMode(window);
            SwitchToQuickTranslateMode(window);
        });

        MeasureResponsiveStep("mini/fixed floating windows", TimeSpan.FromSeconds(45), () =>
            ExerciseFloatingWindows(window));

        MeasureResponsiveStep("main page final responsiveness probe", TimeSpan.FromSeconds(10), () =>
        {
            window.SetForeground();
            var inputBox = UITestHelper.FindInputTextBox(window, TimeSpan.FromSeconds(5));
            inputBox.Should().NotBeNull("main page input should remain reachable after switching across UI surfaces");
            inputBox!.Click();
            Thread.Sleep(250);
            inputBox.Text = "UI freeze regression probe";
        });
    }
    ...
    private void MeasureResponsiveStep(string label, TimeSpan budget, Action action)
    {
        ...
        finally
        {
            stopwatch.Stop();
            _output.WriteLine(
                $"[UiFreeze] {label}: {stopwatch.Elapsed.TotalMilliseconds:F0}ms " +
                $"(budget {budget.TotalMilliseconds:F0}ms)");
        }

        stopwatch.Elapsed.Should().BeLessThan(
            budget,
            "UI thread should not freeze while switching {0}",
            label);
    }
