...
- 工具项目
  ...
  - 已有覆盖：Rust `easydict_msix_validate` CLI 测试 `verify_bundle_minversion_cli_defaults_to_rust_only_payload_policy` 构造带 `workers/` 和 `dotnet/` marker 的 bundle，证明漏传 `--runtime-profile` 时按 Rust-only payload policy 失败，显式 `--runtime-profile hybrid` 才允许同一共存布局；`prepare_package_inputs_cli_defaults_to_rust_only_and_rejects_retained_payload_before_manifest_write` 构造带 retained payload 的 publish dir，证明真实 `prepare-package-inputs` CLI 漏传 `--runtime-profile` 时在写 prepared manifest 前失败，显式 `hybrid` 才继续。
...
- OCR 引擎
  ...
  - 已有覆盖：`OcrTextMergerTests`、`OcrServiceFactoryTests`、`CustomApiOcrServiceTests`、`OllamaOcrServiceTests`、Rust `ocr_behavior`（含 `windows_native_ocr_availability_maps_languages_without_recognition`、app-dir runner 即使看到旧 `workers/ocr` / `Easydict.CompatHost.exe` 标记也走 Rust-native Windows OCR 本地路径、Ollama HTTP provider 或 Custom API HTTP provider，不探测 legacy OCR worker、BGRA BMP/JPEG data URL 通过 `base64` crate standard padded engine 编码/解码）、Rust `easydict_windows_ocr` crate tests。
