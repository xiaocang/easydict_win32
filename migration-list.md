...
- 工具项目
  ...
  - Rust-native LongDoc PDF source extraction：PDFium/source extractor 失败或返回空时必须继续走现有 Rust `pdf-extract` / `lopdf` content-stream / OCR fallback 链路，且不得探测 retained LongDoc worker；`Both` / `Bilingual` 输出模式必须在任何写入前预检全部 TXT/MD/PDF/sidecar 输出路径，避免留下 partial output；native Text/Markdown/simple-PDF LongDoc 必须支持 `resultJsonPath`，在翻译前预检 JSON sidecar 目标，完成后写入完整 `TranslateDocumentResult` JSON，并在返回结果里带回 sidecar path；`easydict_long_doc` CLI 的 `--result-json` / `--result-json-path` 和 `scripts/translate-long-doc.ps1` 的 `-ResultJsonPath` 必须透传到同一 Rust-native sidecar 参数。
...
- 导出
  ...
  - 已有覆盖：Rust `long_document_behavior` 覆盖 Text/PDF `Both` 输出目标 preflight 在 provider/translator 调用前失败且不写半个 monolingual 文件；Rust `long_document_export_behavior`、Rust `document_layout_behavior`、Rust `pdf_export_blocks_behavior`、`DocumentExportServiceTests`、`PdfExportServiceLayoutTests`、`MuPdfExportServiceSpacingTests`、`LongDocWorkerPipelineTests`、`PdfTranslationVisualTest`。
...
- 索引和建议
  ...
  - 已有覆盖：Rust `lex_index_behavior`、Rust `easydict-lex-index` CLI tests、Rust `local_dictionary_index_behavior`、Rust `quick_translate_behavior` 覆盖 persistent native index reuse、wildcard native index route、default native index result-list full stop before opening later dictionaries、mixed missing/encrypted dictionary local errors and feature-gated lazy bridge skip when native prefix fills results；`LocalDictionaryIndexServiceTests`。
