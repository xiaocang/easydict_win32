...
- Worker 进程
  ...
  - 已有覆盖：Rust `default_api_boundary_behavior` 覆盖默认 API 不保留 legacy `*_with_packaged_app_dir` alias，crate-root retained worker modules/re-exports 必须贴着 `#[cfg(feature = "retained-dotnet-workers")]`，且默认源码面（含 settings migration/storage）不得出现 `CompatHost`、`Easydict.Workers`、`dotnet.exe`、`DOTNET_ROOT`、`host/fxr` / `hostfxr` 等 retained runtime/worker 入口 marker；Rust `protocol_behavior` 默认测试覆盖 core translation/settings/MDX/LongDoc DTO 只经 `protocol` facade 序列化，`protocol_behavior --features retained-dotnet-workers` 覆盖 retained worker IPC envelope/lifecycle DTO；Rust `compat_client` 覆盖 retained worker ready/capability/event/cancel/shutdown/exit，并覆盖 packaged worker command/facade 在 unset/rust-only profile 下先报 Rust-native-required 而不是探测 worker exe；`.NET` `WorkerProtocolSerializationTests` 仍保留 worker DTO parity。
- Sidecar/IPC
...
- OCR 引擎
  ...
  - 已有覆盖：`OcrTextMergerTests`、`OcrServiceFactoryTests`、`CustomApiOcrServiceTests`、`OllamaOcrServiceTests`、Rust `ocr_behavior`（含 `windows_native_ocr_availability_maps_languages_without_recognition`）、Rust `easydict_windows_ocr` crate tests。
...
- 查词与资源
  ...
  - 可验证标准：MDD resource key 可读取；MDX HTML 中的 `src`/`href`/`srcset`/`poster`/lazy-load 属性/CSS `url(...)` 和 `sound://...` 本地资源可被 Rust 内联为 data URL；加密词典正确/错误密钥行为明确；dictionary webview 有截图可复查。
  - 已有覆盖：Rust `mdd_new_can_read_minimal_v2_resource_file`、`locate_raw_result_can_span_multiple_record_blocks`、`mdd_lookup_respects_case_sensitive_and_strip_key_headers`、`mdd_resource_lookup_prefers_exact_resource_path_over_stripped_collision`、`strip_key_for_mdd_preserves_resource_extensions`、`selected_mdx_dictionary_auto_discovers_companion_mdd_files`、`native_mdd_companion_discovery_matches_mdict_resource_file_convention`、`native_mdd_resource_lookup_normalizes_keys_and_uses_first_hit`、`native_mdd_resource_lookup_treats_empty_key_as_neutral_miss`、`native_mdd_resource_lookup_skips_failed_mdd_files`、`native_mdd_resource_lookup_skips_mdd_lookup_errors`、`native_mdd_resource_lookup_reads_real_rs_mdict_mdd_fixture`、`native_mdd_html_inline_decodes_dictassets_urls_and_strips_cache_busters`、`native_mdd_html_inline_rewrites_srcset_poster_and_lazy_resource_attrs`、`native_mdd_html_inline_rewrites_background_data_srcset_and_sound_variants`、`native_mdx_lookup_inlines_mdd_resources_into_webview_ready_html`（含 `sound://` 音频和 `entry://` 保留）、`native_mdx_lookup_can_skip_mdd_resource_inline_for_key_only_callers`、`native_mdx_lookup_continues_mdd_inline_after_lookup_error`、`mdx_service_result_keeps_rich_html_only_when_mdd_resources_are_attached`；`.NET` `MdxMddResourceTests`、`MdxEncryptedLookupTests`、`MdxEncryptionTests`、UI `DictionaryWebViewRenderingTests`。
- 索引和建议
  - 行为：
...
- 可验证标准
  - release workflow 必须把 Rust portable zip 作为独立 artifact/release asset 上传；默认/tag release 只能走 `create-rs-portable-release` 上传 `easydict-rs-portable-*`，Rust 第一版不得接入 MSIX bundle、WinApp CLI、Inno installer、WinGet 或 Store submission；dotnet 版现有 MSIX/installer/portable artifact 只允许在显式 `release_flavor=hybrid` 共存发布中继续存在。
  ...
  - Rust helper packaging 必须走 Rust `easydict_packager build-rust-helpers`；必须构建 `easydict-native-bridge`、`easydict_browser_registrar`、`easydict_cli`、`easydict_long_doc`，复制四个 `.exe`，并把 `easydict_browser_registrar.exe` 复制为 legacy alias `BrowserHostRegistrar.exe`。
