...
- 工具项目
  ...
  - `easydict_packager --help` / usage 输出必须把 `extract-dotnet-runtime` 标注为 hybrid/coexistence packaging only，并且不得暗示 `rust-only` profile 可以提取 `.NET` runtime；该命令永远不应出现在 rs portable 的正常打包说明或命令块中，只能出现在明确的 hybrid-only retained runtime 小节。根目录中英文 README 的推荐 portable 下载入口必须指向 `easydict-rs-portable-*` 和 `Easydict.Rust.exe`，并明确不包含 `.NET` runtime；旧 `easydict_win32-*` / `Easydict.WinUI.exe` 只能作为 legacy/hybrid 共存包说明保留。dotnet/hybrid publish/MSIX 脚本不得在 PowerShell `ValidateSet` / 参数补全中枚举 `RustOnly` / `rust-only`，只允许脚本内部识别这些值并报“第一版 rs release 请使用 `rs/scripts/Package-Portable.ps1`”。
...
- 可验证标准
  ...
  - Browser extension package 必须走 Rust `easydict_packager package-browser-extension`；Chrome/Edge 输出 `easydict-ocr-chrome-v{version}.zip`，Firefox 输出 `easydict-ocr-firefox-v{version}.xpi`；包内只能包含 Rust 白名单文件，目标 manifest 必须统一写成 `manifest.json`，顶层 `key` 字段必须在包内移除但不得修改源 manifest。
