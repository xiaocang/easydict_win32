...
- 索引和建议
  ...
  - 已有覆盖：Rust `lex_index_behavior`、Rust `easydict-lex-index` CLI tests、Rust `local_dictionary_index_behavior` 覆盖 legacy shared default root、settings-aware index root 和 blank cache_dir fallback；Rust `quick_translate_behavior` 覆盖 persistent native index reuse、settings cache_dir native index route、wildcard native index route、default native index result-list full stop before opening later dictionaries、mixed missing/encrypted dictionary local errors and feature-gated lazy bridge skip when native prefix fills results；`LocalDictionaryIndexServiceTests`。
