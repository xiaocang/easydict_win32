...
- 保存、返回和即时反馈
  ...
  - 敏感设置用 `LocalCredentialProtector` 保护；旧明文/legacy sensitive settings 会迁移到 protected storage。Rust credential payload 的 base64 编解码应复用 `base64` crate 的 standard padded engine；decode 必须继续拒绝缺失 padding/非法字符，同时允许旧手写 decoder 曾接受的 trailing-bit 宽松 legacy payload，避免旧设置不可读。`EASYDICT_SETTINGS_DIR` 可隔离设置目录，UIA 使用它避免污染用户设置。
- 可验证标准
  ...
  - load/save 后 settings JSON 不写回 legacy `TargetLanguage`，也不保留 runtime-only worker isolation keys（`UseLongDocWorker` / `UseLocalAiWorker` / `UseOcrWorker`）；旧 `openvino-local-ai` / `foundry-local` service id 会迁到聚合 `windows-local-ai`，并在无聚合服务且无显式 provider 时分别保留 `LocalAIProvider=OpenVINO` / `FoundryLocal` 偏好。
