...
- 工具项目
  ...
  - Rust `rs/crates/easydict_packager`：portable ZIP 生成、浏览器扩展 ZIP/XPI 生成、Rust helper 构建/复制、retained worker 共享 `.NET` runtime 下载/解压工具；release workflow 的 dotnet portable ZIP 和 `rs/scripts/Package-Portable.ps1` 的 Rust portable ZIP 都必须通过 Rust `zip-directory` subcommand 创建，避免回退到 PowerShell `Compress-Archive`；`browser-extension/scripts/Package-Extension.ps1` 只作为 Rust `package-browser-extension` subcommand shim 保留，避免在 PowerShell 中维护 manifest JSON 修改、文件白名单和 ZIP/XPI 创建逻辑；`dotnet/scripts/Build-RustHelpers.ps1` 只作为 Rust `build-rust-helpers` subcommand shim 保留，避免在 PowerShell 中维护 `cargo build` target/bin 列表和 helper copy/alias 逻辑；`dotnet/scripts/Extract-DotnetRuntime.ps1` 只作为 Rust `extract-dotnet-runtime` subcommand shim 保留，避免回退到 PowerShell `Invoke-WebRequest` / `Expand-Archive`。
...
- 聚合服务
  ...
  - 可验证标准：
    ...
    - Auto provider 在已配置 Foundry endpoint 或 Rust resolver 能发现正在运行的 Foundry endpoint 时可走 Rust native Foundry route；Quick Translate、CLI quick translate 和 Rust-native Text/Markdown/simple-PDF LongDoc route 都应在进入 retained LocalAI/LongDoc worker policy 前尝试该 Foundry endpoint probe；Auto 且 endpoint 为空、resolver 也找不到 endpoint 时，如果 OpenVINO cache 完整且语言对支持，应按 Auto 顺序继续 fallback 到 Rust-native `HuggingFaceNllbTokenizer + OrtNllbInferenceEngine`；只有 OpenVINO 也不可用时，显式 worker-policy 兼容测试入口才可继续保留 LocalAI worker route。rs GUI 和 CLI 不再启动 retained LocalAI worker，`--app-dir` / `--host` 只保留为 legacy no-op，并在需要 worker 的场景本地提示需要 Rust-native route。
    ...
    - Explicit OpenVINO 翻译目标为 Auto 或无法映射到 NLLB/FLORES 支持语言时，应在 Rust 本地报 “No local AI provider supports this language pair”，不应启动 LocalAI worker/CompatHost；支持的非 Auto 语言对如果模型/runtime 缓存未完整，应在 Rust 本地报 OpenVINO runtime/model not downloaded，也不应启动 LocalAI worker/CompatHost；缓存完整时 GUI 和 CLI Quick Translate 应直接走 Rust-native `HuggingFaceNllbTokenizer + OrtNllbInferenceEngine` 路线，即使在显式 hybrid 过渡 profile 下也不应再启动 retained LocalAI worker。
  ...
  - 已有覆盖：`LocalAITranslationServiceIntegrationTests`、`LocalAITranslationServiceLazyInitTests`、`OpenVinoTranslationServiceTests`、`PhiSilicaTranslationServiceTests`、`FoundryLocalServiceTests`；Rust `quick_translate_behavior` 覆盖 Auto/explicit Foundry native route、Auto Foundry miss 后 cache-ready OpenVINO native fallback、显式 worker-policy 兼容 route、默认 packaged GUI LocalAI 不探测 worker、disabled-worker policy、target Auto 本地失败、OpenVINO unknown target/cache-missing 本地失败、OpenVINO cache-ready native ORT route 不启动 worker、以及 BCP-47 extended NLLB language bridge；Rust `cli_translate_behavior` 覆盖 CLI Auto Foundry endpoint probe 优先发生、`--app-dir`/legacy `--host` 不再触发 retained LocalAI worker 查找、需要 worker 的 CLI LocalAI 场景本地报 Rust-native route；Rust `long_document` 内部测试覆盖 Auto Foundry endpoint discovery 在 LongDoc 进入 retained worker 前切到 native route，LongDoc Text/Markdown/simple-PDF route 也复用 Auto OpenVINO native fallback。
- Foundry Local
  - 行为：
...
- 查词与资源
  ...
  - 可验证标准：MDD resource key 可读取；MDX HTML 中的 `src`/`href`/CSS `url(...)` 本地资源可被 Rust 内联为 data URL；加密词典正确/错误密钥行为明确；dictionary webview 有截图可复查。
...
- Makefile
  ...
  - Rust 第一版发布：`rs/scripts/Package-Portable.ps1` 只生成 portable zip，不生成 MSIX/installer/Store 包；输出名必须使用 `easydict-rs-portable-*`，主程序别名为 `Easydict.Rust.exe`，不得覆盖 dotnet 版 `Easydict.WinUI.exe` 或 `easydict_win32-*` 包名；包内不携带 retained `.NET` workers 或 bundled `.NET` runtime，允许与 dotnet 版本并存；未显式设置 runtime profile 时 rs portable 运行期必须按 Rust-only policy 处理，不应自动探测 retained worker；ZIP 创建必须走 Rust `easydict_packager`，不使用 PowerShell `Compress-Archive`。
