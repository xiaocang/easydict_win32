...
- Updated ARM64 MSIX smoke failure guidance so Rust-only/unknown profile users are pointed to the direct `easydict_packager pack-rs-portable` path, keeping `Package-Portable.ps1` as compatibility-only wording.
- Added a desktop integration registry guard: shell verb, protocol, and startup registration helpers now reject retained runtime, worker, or script command targets before writing HKCU command values. This reuses the existing `lib/easydict-runtime-guards` command-target classifier; no new library was needed.
- Added Rust contract coverage for benchmark and memory workflows, expanded the UI automation MSIX/profile contract to cover the parallel WinUI unit-test job, and added desktop integration unit coverage for rejected registry command targets.
...
- `cargo test --manifest-path rs/Cargo.toml -p easydict_packager --test release_contract_behavior arm64_msix_smoke_requires_explicit_hybrid_runtime_profile -- --exact --nocapture`
...
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app desktop_integration --lib -- --nocapture`
...
- `cargo test --manifest-path lib/easydict-runtime-guards/Cargo.toml command_target_classifier_rejects_retained_runtime_and_script_targets -- --nocapture`
- `git diff --check -- .github/workflows/arm64-msix-smoke.yml .github/workflows/benchmark.yml .github/workflows/ui-automation.yml .github/workflows/memory-gate.yml .github/workflows/memory-nightly.yml rs/crates/easydict_packager/tests/release_contract_behavior.rs rs/crates/easydict_packager/tests/msix_runtime_profile_contract_behavior.rs rs/crates/easydict_app/src/desktop_integration.rs`

## 2026-06-14: Rejected legacy CLI worker options by default and rechecked Collins MDD

- Tightened the default `easydict_cli` parser so `--host`, `--host-arg`, and `--app-dir` now return `UnknownOption` in the default build instead of being accepted as retained-worker no-ops. Compatibility parsing remains available only under the explicit `retained-dotnet-workers` feature.
- Added a default API boundary test that requires any legacy CLI option arms to be both feature-gated and paired with a default unknown-option guard.
- Kept the browser extension packaging shim on the Rust packager path with a fake-cargo contract for `browser-extension/scripts/Package-Extension.ps1 -> easydict_packager package-browser-extension`.
- Closed another MSIX validation gap through the parallel QDC/UI automation worker. QDC deploy/install now defaults `RuntimeProfile` to `rust-only`, copies or locates the Rust `easydict_msix_validate` binary, and validates the MSIX before cert import, uninstall, or `Add-AppxPackage`. The UI automation workflow now fixes both runtime profile env vars to `rust-only`, passes `-p:RuntimeProfile=${{ env.RUNTIME_PROFILE }}` to restore/publish, and validates the MSIX before upload and before install.
- Re-ran the Collins COBUILD English Usage corpus specified by the user, including the companion `.mdd`. The real corpus still reports `Encrypted=2`, 1517 MDX keywords, and one MDD resource `\cceu.css` with 4579 raw CSS bytes. Settings discovery, native HTML inlining, and Quick Translate all passed against the same files.
- No new parser dependency was needed. The in-tree `lib/rs-mdict` fork remains the right reuse point because it already owns the MDict encryption, key normalization, MDD resource lookup, and app-level data-URL inlining semantics.

Validation:

- `cargo fmt --manifest-path rs/Cargo.toml -p easydict_app --check`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app cli_translate::tests::default_build_rejects_legacy_host_and_app_dir_options --lib -- --exact --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --test cli_translate_behavior default_cli_rejects_legacy_retained_worker_options -- --exact --nocapture`
...
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --features retained-dotnet-workers cli_translate::tests::retained_feature_ignores_legacy_host_and_app_dir_together --lib -- --exact --nocapture`
- `cargo fmt --manifest-path rs/Cargo.toml -p easydict_packager --check`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_packager --test release_contract_behavior browser_extension_powershell_shim_delegates_to_rust_packager -- --exact --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_packager --test msix_runtime_profile_contract_behavior -- --nocapture`
- PowerShell parser check for `dotnet/scripts/qdc/Deploy-ToQdc.ps1` and `dotnet/scripts/qdc/Install-OnQdc.ps1`
...
## 2026-06-14: Aligned stale .NET packaging tests with hybrid-only ZIP helper gating

- Updated `dotnet/tests/Easydict.WinUI.Tests/Services/WorkerPackagingTests.cs` so the legacy .NET test oracle no longer treats `zip-directory` as evidence for the default rs portable release path. The test now scopes assertions to `publish-rs-portable` and `publish-msix`: rs portable must call direct `pack-rs-portable`, while the legacy/hybrid ZIP step must use `easydict_packager --features hybrid-dotnet-runtime-packaging -- zip-directory`.
- Tightened the legacy `dotnet/scripts/publish.ps1 -CreateZip` assertion so it requires the same explicit hybrid packager feature before `zip-directory`. This keeps the stale .NET-side contract in sync with the Rust release-contract tests added earlier.
- No new library was needed. This is test coverage alignment for the already-migrated Rust packager boundary.

Validation:

- `cargo test --manifest-path rs/Cargo.toml -p easydict_packager --test release_contract_behavior legacy_publish_create_zip_is_hybrid_named_and_excluded_from_rs_release_contract -- --exact --nocapture`
...
- `dotnet test tests/Easydict.WinUI.Tests/Easydict.WinUI.Tests.csproj --filter "FullyQualifiedName~WorkerPackagingTests" --no-restore` was retried after a 30s wait but still failed before running the filtered tests because unrelated long-document/layout/table test sources currently do not compile (`DocLayoutYoloService`, `LayoutDetectionStrategy`, `TableStructureRecognitionService`, `DocumentOutputMode`, and related stale symbols).

## 2026-06-14: Hid build-rust-helpers legacy alias knobs from the default API

- Tightened the helper build API after gating the legacy registrar alias itself. Default `BuildRustHelpersOptions` now only contains the Rust helper build inputs, and default `copy_built_rust_helpers(...)` only copies the four Rust helper executables. The `include_legacy_registrar_alias` field, `runtime_profile` field, alias-specific error variants, and extended copy arguments compile only with `hybrid-dotnet-runtime-packaging`.
- The default `build-rust-helpers` CLI no longer parses `--runtime-profile`; that knob is reserved for the explicit hybrid feature path where it must pair with `--include-legacy-registrar-alias`.
- No new library was needed. This is another Rust packager API-surface cleanup around already migrated helper binaries.

Validation:

- `cargo fmt --manifest-path rs/Cargo.toml -p easydict_packager --check`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_packager default_build_rejects_runtime_profile_flag_before_workspace -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_packager --features hybrid-dotnet-runtime-packaging build_rust_helpers_rejects_legacy_registrar_alias_without_explicit_hybrid_before_workspace -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_packager --test release_contract_behavior dotnet_runtime_extraction_shim_requires_explicit_hybrid_profile -- --exact --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_packager -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_packager --features hybrid-dotnet-runtime-packaging -- --nocapture`

## 2026-06-14: Moved standalone zip-directory behind the hybrid packager feature

- Tightened the packager CLI boundary: default `easydict_packager` usage no longer advertises `zip-directory`, and invoking that standalone command in a default build now follows the unknown-command path. The internal private `zip_directory(...)` implementation remains available because `pack-rs-portable` uses it to create the first rs portable ZIP before `validate-rs-portable`; the public standalone ZIP API is feature-gated for hybrid packaging.
- Updated the retained legacy/hybrid callers that still need the standalone ZIP helper. The release workflow's legacy ZIP step and `dotnet/scripts/publish.ps1 -CreateZip` now run `easydict_packager` with `--features hybrid-dotnet-runtime-packaging` before `zip-directory`.
- No new library was needed. This is a packager CLI/feature-boundary cleanup around the existing Rust ZIP implementation, keeping rs portable callers on `pack-rs-portable`.

Validation:

- `cargo fmt --manifest-path rs/Cargo.toml -p easydict_packager --check`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_packager default_build_rejects_legacy_zip_directory_command_before_args -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_packager default_usage_hides_dotnet_runtime_extraction_command -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_packager --features hybrid-dotnet-runtime-packaging usage_marks_dotnet_runtime_extraction_as_hybrid_only_not_rs_portable -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_packager --test release_contract_behavior dotnet_runtime_extraction_shim_requires_explicit_hybrid_profile -- --exact --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_packager --test release_contract_behavior legacy_publish_create_zip_is_hybrid_named_and_excluded_from_rs_release_contract -- --exact --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_packager --lib -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_packager --features hybrid-dotnet-runtime-packaging --lib -- --nocapture`

## 2026-06-14: Gated the legacy registrar alias behind the hybrid packager feature

- Tightened the helper packaging boundary again: default `easydict_packager` builds now reject `--include-legacy-registrar-alias` before workspace/output side effects and no longer advertise that flag in default usage. The legacy `BrowserHostRegistrar.exe` alias copy code only compiles under `hybrid-dotnet-runtime-packaging`.
- Kept the explicit coexistence path working. `dotnet/scripts/Build-RustHelpers.ps1` now adds `--features hybrid-dotnet-runtime-packaging` only when `-IncludeLegacyRegistrarAlias` is requested, and it still forces child `cargo`/helper builds to `EASYDICT_RUNTIME_PROFILE=rust-only` and `RUNTIME_PROFILE=rust-only`.
- No new library was needed. This is a Rust packager feature boundary cleanup around the already migrated helper build path.

Validation:

- `cargo fmt --manifest-path rs/Cargo.toml -p easydict_packager`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_packager legacy_registrar_alias -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_packager default_usage_hides_dotnet_runtime_extraction_command -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_packager default_build_rejects_legacy_registrar_alias_flag_before_workspace -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_packager --features hybrid-dotnet-runtime-packaging legacy_registrar_alias -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_packager --features hybrid-dotnet-runtime-packaging usage_marks_dotnet_runtime_extraction_as_hybrid_only_not_rs_portable -- --nocapture`
...
- `cargo test --manifest-path rs/Cargo.toml -p easydict_packager --test release_contract_behavior build_rust_helpers_powershell_shim_enables_hybrid_packager_feature_only_for_legacy_alias -- --exact --nocapture`

## 2026-06-14: Removed WinFluent Windows platform dev-dependency from the default app manifest

- Tightened the default app dependency boundary so `rs/crates/easydict_app/Cargo.toml` no longer lists `win_fluent_platform_win` even as a dev-dependency. This keeps the default rs app manifest aligned with the first portable release rule: platform/hybrid implementation crates stay outside the app crate unless explicitly needed by another package's own tests.
- Replaced the Quick Translate tray projection test that called `WindowsPlatformAdapter::plan_tray(...)` with an app-owned `TrayMenu` shape contract. The app test still verifies default item, submenu, separator, browser support, and command enablement structure, while native adapter projection remains the responsibility of the WinFluent platform crate.
- No new library was needed. This was a manifest/test-surface cleanup after the tray, shell, registry, and native task paths had already moved to Rust-owned helpers.

Validation:

- `cargo fmt --manifest-path rs/Cargo.toml -p easydict_app`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --test default_api_boundary_behavior default_app_manifests_do_not_link_hybrid_packaging_or_dotnet_runtime_tools -- --exact --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --test quick_translate_behavior default_tray_menu_keeps_native_tray_shape_without_platform_adapter -- --exact --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --test default_api_boundary_behavior default_ -- --nocapture`
...
- `EasydictUiState` now owns the `MouseSelectionProducer`, so low-level mouse/keyboard events enter the same Rust state machine as the prior pure tests. The default app starts/stops the stream from the persisted `MouseSelectionTranslate` setting on startup and save/discard, and ignores residual hook events when the setting is off.
- Pending double/triple-click actions now create a real delayed completion task instead of only exposing timer metadata; completion re-enters the producer and maps to the existing native selected-text capture + PopButton chain. PopButton self-click suppression and excluded-app checks are applied at the app boundary.
...
## 2026-06-13: Added Rust Sidecar IPC E2E to the default rust-only CI gate

- No new dependency was needed. The active Sidecar acceptance path is already the Rust `sidecar_ipc_e2e` test against the existing Python JSON Lines mock, replacing the retired `.NET` E2E console app.
- Added that test to `.github/workflows/ci.yml` under the `rust-only-boundary` job, which already forces `EASYDICT_RUNTIME_PROFILE=rust-only` and `RUNTIME_PROFILE=rust-only` and does not set up .NET.
- Tightened the packager release contract so the default CI boundary must keep running the Rust Sidecar IPC E2E gate alongside runtime policy, default API boundary, portable payload, and stale app-dir smoke checks.
...
## 2026-06-13: Revalidated Rust MDD support and aligned desktop runtime-plan tray parity

- No new dependency was needed. The MDD route stays on the in-tree MIT `lib/rs-mdict` fork plus app-owned HTML/resource rewrite glue.
- Re-ran the focused Rust MDD suites after the user-requested MDD check: low-level MDD parsing/resource lookup, app MDX/MDD HTML inlining, and Quick Translate rich HTML retention all pass, including entity-decoded resource references.
- Fixed the desktop integration runtime-plan test to expect the same tray tooltip as `default_tray_menu()` and the .NET tray service (`Easydict - Dictionary & Translation`), so the Rust runtime plan no longer fails while forwarding the app-owned tray/menu/named-event/protocol surfaces.
...
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --test mdx_native_behavior mdd -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --test quick_translate_behavior mdd -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --test quick_translate_behavior desktop -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --test ocr_behavior -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --test screen_capture_behavior -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --test text_selection_behavior -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --test mouse_selection_behavior -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --test browser_registrar_behavior -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --test cli_translate_behavior local_ai -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --test long_document_cli_behavior -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --test long_document_behavior local_ai -- --nocapture`

## 2026-06-13: Hardened release contracts against negative rust-only workflow gates

- No new dependency was needed. This strengthens the existing Rust packager release-contract tests.
- Added a shared assertion that rejects workflow lines combining `RUNTIME_PROFILE`, `rust-only` / `rustonly`, and negative comparisons such as `!=` or `-ne`. Retained worker/runtime workflow steps must stay positively gated by explicit hybrid enablement instead of “anything not rust-only”.
- Applied the helper to the default release workflow and the hybrid MSIX workflow checks, so future rewrites like `${{ env.RUNTIME_PROFILE != 'rust-only' }}` or PowerShell `-ne 'rust-only'` do not bypass the current exact-string guard.
...
## 2026-06-13: Stopped Makefile from forwarding empty runtime profiles to legacy shims

- No new dependency was needed. This continues the legacy packaging runtime-profile hardening inside `dotnet/Makefile` and the existing Rust packager release-contract test.
- `msix-*`, `installer-*`, and MSIX runtime extraction paths now pass the normalized shell `$$runtime_profile` value into `Package-Msix.ps1`, `Build-Installer.ps1`, and `Extract-DotnetRuntime.ps1` instead of forwarding the raw Make variable `$(RUNTIME_PROFILE)`.
- The MSIX and installer targets also re-check explicit `hybrid` immediately before invoking their legacy PowerShell shims, so a split or reordered target cannot pass an empty profile down to a retained-runtime-producing path.

Validation:

- `cargo test --manifest-path rs/Cargo.toml -p easydict_packager --test release_contract_behavior legacy_dotnet_packaging_paths_reject_rust_only_and_require_hybrid_profile -- --exact --nocapture`

## 2026-06-13: Guarded MSIX bundle validation with explicit hybrid profile

- No new dependency was needed. This is a release-orchestration guard in the existing GitHub Actions workflow and packager contract tests.
- Added an explicit `create-bundle` runtime-profile guard before bundle validation. The job now rejects empty, rust-only, and unknown profile values locally, and normalizes the environment to `RUNTIME_PROFILE=hybrid` before passing it to `easydict_msix_validate verify-bundle-minversion`.
- This closes the remaining direct bundle path where `--runtime-profile "${{ env.RUNTIME_PROFILE }}"` could receive an empty workflow input if the DAG or job invocation changes later.

Validation:

- `cargo test --manifest-path rs/Cargo.toml -p easydict_packager --test release_contract_behavior legacy_dotnet_packaging_paths_reject_rust_only_and_require_hybrid_profile -- --exact --nocapture`

## 2026-06-13: Made Rust-only MSIX validation reject the legacy registrar alias

- No new dependency was needed. This continues the helper packaging policy cleanup inside the existing Rust `easydict_msix_validate` payload layout validator.
- Split MSIX helper requirements by runtime profile: Rust-only/default validation now requires only the four Rust helper executables and rejects `BrowserHostRegistrar.exe`, while explicit hybrid/coexistence validation still requires the legacy alias for the current WinUI browser-support path.
- Tightened `prepare-package-inputs` so the Rust-only/default preflight also rejects a root `BrowserHostRegistrar.exe` publish-dir payload before writing the prepared manifest; hybrid/coexistence prepare still leaves that alias for full validation.
- Updated MSIX and nested bundle fixtures so `rust_helper_entries()` represents the alias-free Rust-only payload and `hybrid_helper_entries()` represents the legacy/hybrid payload.
- Updated the migration checklist to keep the MSIX validator aligned with `build-rust-helpers`: the legacy alias is no longer a default package requirement.

Validation:

- `cargo test --manifest-path rs/Cargo.toml -p easydict_msix_validate --all-targets -- --nocapture`
- `cargo check --manifest-path rs/Cargo.toml -p easydict_msix_validate --all-targets`
- `cargo fmt --manifest-path rs/Cargo.toml -p easydict_msix_validate --check`

## 2026-06-13: Made the legacy browser registrar alias opt-in for helper builds

- No new dependency was needed. This is a packaging policy cleanup inside the existing Rust `easydict_packager` helper build path and the PowerShell shim that delegates to it.
- `easydict_packager build-rust-helpers` now defaults to copying only Rust helper executables. The legacy `BrowserHostRegistrar.exe` compatibility alias is created only when callers pass `--include-legacy-registrar-alias`.
- `dotnet/scripts/Build-RustHelpers.ps1` mirrors that rule with `-IncludeLegacyRegistrarAlias`; legacy/hybrid WinUI portable and MSIX callers pass the switch explicitly, while first-release rs portable packaging remains alias-free.
- Updated the migration checklist so `build-rust-helpers` no longer reads as a generic alias-producing path. The alias is now documented as a legacy/hybrid coexistence requirement, not part of the default rs portable payload.

Validation:

- `cargo test --manifest-path rs/Cargo.toml -p easydict_packager copy_built_rust_helpers -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_packager --test release_contract_behavior build_rust_helpers_child_cargo_is_forced_to_rust_only_runtime_profile -- --exact --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_packager --test release_contract_behavior release_orchestration_uses_rust_helpers_not_retired_dotnet_helper_projects -- --exact --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_packager --test release_contract_behavior build_rust_helpers_powershell_shim_delegates_and_forces_runtime_profile -- --exact --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_packager --test release_contract_behavior pack_rs_portable_creates_and_validates_zip_without_retained_dotnet_payload -- --exact --nocapture`

## 2026-06-13: Centralized Foundry Local CLI runtime guard

- No new third-party dependency was needed. The default rs Foundry Local route already uses the native `foundry` CLI boundary, so this slice reuses the in-tree `lib/easydict-runtime-guards` policy instead of adding another parser or process-wrapper crate.
- Moved Foundry Local's `EASYDICT_FOUNDRY_LOCAL_CLI` retained runtime/script command classifier onto `easydict_runtime_guards::command_target_is_retained_runtime_or_script_marker(...)`. The shared guard now covers direct command names, canonical targets, `dotnet/` and `workers/` path components, `host/fxr` and shared framework layouts, loose runtime assemblies, retired Easydict helper/worker names, and `.ps1` script targets.
- Updated the default app source-boundary test so the only allowed external process boundary still belongs to `lib/easydict-foundry-local`, while retained runtime/script classification must be delegated to `lib/easydict-runtime-guards` instead of re-inlined in the Foundry wrapper.
- Added a local dictionary app-dir regression proving `run_local_dictionary_suggestion_request_with_app_dir(...)` ignores stale retained payloads under the supplied app dir and fails locally through the Rust-native MDX/index route.

Validation:

- `cargo test --manifest-path lib/easydict-runtime-guards/Cargo.toml command_target -- --nocapture`
- `cargo test --manifest-path lib/easydict-runtime-guards/Cargo.toml --features retained-dotnet-workers --all-targets -- --nocapture`
...
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --test quick_translate_behavior local_dictionary_suggestions_app_dir_ignores_stale_dotnet_payload_markers -- --exact --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --test quick_translate_behavior local_dictionary -- --test-threads=1 --nocapture`

## 2026-06-13: Decoded HTML-escaped MDD resource references in Rust

- Rechecked the MDD request with a read-only subagent against the `.NET` service, WebView resource interception path, existing Rust fixtures, and migration docs. The active route remains the in-tree MIT `lib/rs-mdict` plus app-owned HTML rewrite/credential glue; no new parser dependency was added.
- Tightened the Rust MDD HTML inliner so resource references are decoded like browser/WebView input before lookup. This now covers `&amp;` query separators, numeric entities in Unicode paths, and `&quot;...&quot;`-wrapped CSS `url(...)` references before percent decoding and MDD key normalization.
- Added focused coverage for entity-escaped image, audio, `srcset`, and CSS resources while preserving external URLs and the existing first-hit / failure-skip MDD behavior.

Validation:

- `cargo test --manifest-path lib/rs-mdict/Cargo.toml --lib mdd -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --test mdx_native_behavior native_mdd_html_inline_decodes_html_entities_in_resource_paths -- --exact --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --test mdx_native_behavior mdd -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --test quick_translate_behavior mdd -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --test settings_storage_behavior mdd -- --nocapture`

## 2026-06-13: Centralized retained runtime marker scanning for packages

- Reused `lib/easydict-runtime-guards` from `easydict_msix_validate` for both retained payload path classification and retained byte marker scanning. The MSIX validator no longer keeps its own marker table, path classifier, or ASCII / UTF-16LE scanning helpers.
- Moved the first-release rs portable retained payload path classifier onto the same library-owned policy, so rs portable ZIP validation and MSIX / nested bundle validation reject the same stale `.NET` runtime layouts, worker roots, loose assemblies, and retired helper payload names.
- Added source-boundary regressions proving both `easydict_msix_validate` and `easydict_packager` delegate retained marker checks to `easydict_runtime_guards` instead of re-inlining package-specific copies.

Validation:

- `cargo test --manifest-path lib/easydict-runtime-guards/Cargo.toml --all-targets -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_packager rs_portable -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_msix_validate retained_runtime_marker_scanners_are_lib_owned -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_msix_validate rust_only_package -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_msix_validate rust_only_package_rejects_utf16le_dotnet_host_markers -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_msix_validate rust_only_package_rejects_allowlisted_helper_exe_that_contains_dotnet_host_markers -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_msix_validate --all-targets -- --nocapture`
- `cargo check --manifest-path rs/Cargo.toml -p easydict_msix_validate --all-targets`

## 2026-06-13: Renamed legacy hybrid ZIP artifacts away from rs portable

- A parallel worker updated the release workflow so explicit hybrid/coexistence ZIPs are named `Easydict-legacy-hybrid-win-<platform>-<version>.zip` and uploaded as `easydict-legacy-hybrid-zip-*` artifacts instead of the old `easydict_win32-*` style name.
- The create-bundle job now downloads the clearly named legacy/hybrid ZIP artifacts, while the first rs release path remains isolated to `easydict-rs-portable-*`. This reduces the chance that retained `.NET` coexistence ZIPs are mistaken for the default rs portable package.
- The release contract test now guards the destination, upload artifact name, download pattern, and absence of the old ambiguous ZIP prefix on the hybrid workflow path.
...
## 2026-06-13: Rejected retired MSIX helper payloads by prefix

- Tightened the Rust MSIX payload validator so retired helper/tool names are rejected by file-name prefix rather than only by exact `.exe` names. This now covers `.dll`, `.deps.json`, `.runtimeconfig.json`, and similar stale `.NET` payload forms for old `Easydict.NativeBridge*`, `Easydict.BrowserRegistrar*`, `MSIXValidate*`, `EncryptSecret*`, `PdfToImages*`, `Easydict.CompatHost*`, and retired OCR worker entries.
- No new third-party library was needed; this is package-layout policy inside the existing Rust validator. The rule applies before the rust-only/hybrid runtime-profile branch, so both first rs portable/MSIX and explicit coexistence packages reject retired helper/tool residue.

Validation:

- `cargo test --manifest-path rs/Cargo.toml -p easydict_msix_validate rejects_retired_comphost_ocr_worker_and_dotnet_tool_payloads -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_msix_validate rust_only_package_rejects_loose_dotnet_assemblies_and_legacy_helpers -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_msix_validate --lib -- --nocapture`
- `cargo check --manifest-path rs/Cargo.toml -p easydict_msix_validate --all-targets`

## 2026-06-13: Scanned browser native bridge binaries before registration

- Added a second guard to the Rust browser Native Messaging registrar. In addition to rejecting legacy names and retained payload roots, `BrowserRegistrarCore::install(...)` now scans the source bridge bytes through `lib/easydict-runtime-guards` before copying and scans the staged bridge again before writing Chrome/Firefox manifests.
- `status` also treats a registration as installed only when the managed `easydict-native-bridge.exe` still passes the same retained payload/script marker scan. This closes the renamed-apphost case where a stale `.NET` bridge could have the expected Rust helper file name.
- No new dependency was added; this reuses the existing lib-owned ASCII / UTF-16LE marker scanner so packaging and browser registration share the same runtime residue policy.

Validation:

- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --test browser_registrar_behavior -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --test native_bridge_behavior -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --test quick_translate_behavior browser_support -- --nocapture`

## 2026-06-13: Hardened bundled shell helper launch names

- Tightened the Rust-owned desktop shell boundary after reusing the existing runtime-guard crate rather than adding a new dependency. Bundled helper names now keep the plain-file-name requirement and additionally reject retained runtime/script/helper markers before `std::process::Command` is constructed.
- `lib/easydict-runtime-guards` now treats `dotnet.exe` / `dotnet.cmd`, PowerShell / Pwsh executable variants, and existing retained worker/CompatHost/NativeBridge names as retained runtime components. `lib/easydict-windows-shell` reuses that classifier for `run_bundled_executable(...)`, while continuing to allow Rust-native helpers such as `easydict_browser_registrar.exe`.
- This protects the browser registrar install/uninstall path from being repointed at stale `.NET` runtime, PowerShell, or retained worker names in the first rs portable package.

Validation:

- `cargo test --manifest-path lib/easydict-windows-shell/Cargo.toml --all-targets -- --nocapture`
- `cargo test --manifest-path lib/easydict-runtime-guards/Cargo.toml --all-targets -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --test quick_translate_behavior browser_support -- --nocapture`

## 2026-06-13: Narrowed OCR screen-capture DTOs to the Rust-owned helper

- Completed the follow-up cleanup on the OCR screen-capture boundary. `easydict_app::screen_capture_native` now accepts and returns `lib/easydict-windows-screen-capture` request/result DTOs directly instead of converting through WinFluent `ScreenCaptureRequest`, `ScreenCaptureResult`, `ScreenRect`, or `ScreenWindowSnapshotRequest`.
- No third-party crate was added. This continues using the existing Microsoft `windows` crate based helper under `lib/`; WinFluent remains only as the neutral `Task::perform(...)` wrapper used to schedule app messages.
- `screen_capture_request_from_selection(...)`, capture overlay background freeze, window snapshot requests, and OCR capture result conversion now all use the lib-owned DTOs. Source-boundary tests guard against reintroducing WinFluent capture DTOs into the native facade or OCR conversion path.

Validation:

- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --test ocr_behavior app_ocr_screen_capture_uses_native_helper_instead_of_winfluent_task_surface -- --exact --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app screen_capture_native --lib -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --test screen_capture_behavior -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --test ocr_behavior capture_window -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --test ocr_behavior capture_overlay_confirm_uses_selected_screen_region -- --exact --nocapture`

## 2026-06-13: Revalidated Rust-native MDD support and tightened encrypted route gating

- Rechecked current Rust MDict/MDD library options before changing code. The in-tree MIT `lib/rs-mdict` route remains the lowest-risk choice for the current app because it already shares MDX/MDD block parsing, supports raw MDD resource lookup, and avoids replacing the app's credential and HTML-rewrite integration with a newer parser surface.
- Confirmed the Rust-native MDD path covers companion `.mdd` discovery, first-hit lookup across multiple MDD files, raw bytes and MIME metadata, zlib/non-ASCII resource blocks, record-encrypted MDD files using the MDX dictionary credentials, and WebView-ready `data:` URL inlining for image/audio/CSS/resource references.
- Fixed the record-block encrypted MDX/MDD route gate so missing credentials are not reported as natively routable. A record-encrypted dictionary now only routes through the Rust reader when credentials are present and the regcode passes local validation.

Validation:

- `cargo test --manifest-path lib/rs-mdict/Cargo.toml --all-targets -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --test mdx_native_behavior -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --test quick_translate_behavior mdd -- --nocapture`

## 2026-06-13: Moved OCR overlay background freeze off WinFluent platform adapter

- Completed the remaining OCR screen-capture backend cleanup in the default app path. The capture overlay's background freeze now calls `screen_capture_native::capture_screen_region(ScreenCaptureRequest::virtual_desktop())`, so selected-region capture, window snapshots, and overlay background screenshots all route through `lib/easydict-windows-screen-capture`.
- No new third-party crate was added in this slice. The earlier screen-capture migration already chose the narrow Microsoft `windows` crate based helper; this pass reuses that helper instead of keeping the duplicate WinFluent platform adapter call.
- Removed the direct `win_fluent_platform_win` dependency from `easydict_app/Cargo.toml`. The app still depends on neutral `win_fluent` runtime types, while the Windows backend crate remains reachable through the preview/iced runtime stack rather than the app crate.
- Added source-boundary coverage proving OCR capture tasks, overlay background freeze, and the app manifest all stay on the Rust-native helper path and do not reintroduce direct `win_fluent_platform_win` / `WindowsPlatformAdapter::capture_screen_region` usage.
...
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app screen_capture_native --lib -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --test screen_capture_behavior -- --nocapture`
- `cargo test --manifest-path lib/easydict-windows-screen-capture/Cargo.toml --all-targets -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --test default_api_boundary_behavior default_app_manifests_do_not_link_hybrid_packaging_or_dotnet_runtime_tools -- --exact --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --test default_api_boundary_behavior default_process_spawn_surface_has_no_retained_dotnet_runtime_entries -- --exact --nocapture`
- `cargo check --manifest-path rs/Cargo.toml -p easydict_app --all-targets`

## 2026-06-13: Narrowed desktop integration declarations to app-owned DTOs

- Continued the desktop-integration cleanup without touching UI-owned files or `lib/winfluent-rs`. The shell/protocol registry writer now consumes app-owned `DesktopShellVerb` and `DesktopProtocolRegistration` DTOs instead of WinFluent platform DTOs.
- No third-party crate is involved; this is an internal boundary reduction. WinFluent `ShellVerb` / `ProtocolRegistration` remain only at the `Application` trait declaration boundary, where private conversion helpers project the app-owned DTOs for runtime-plan parity.
- The Settings shell context-menu toggle and startup `easydict://` protocol ensure keep using Rust-owned futures through `easydict_app::desktop_integration` and `lib/easydict-windows-registry`; tests now guard that the registry facade does not import `win_fluent::`.
- Read-only OCR audit confirmed the remaining overlay background-freeze capture cannot be migrated without touching UI-owned `state.rs`, so it stays as a coordinated follow-up rather than being changed in this slice.

Validation:

- `cargo test --manifest-path rs/Cargo.toml -p easydict_app desktop_integration --lib -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --test quick_translate_behavior shell_context_menu -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --test quick_translate_behavior startup_protocol_registration -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --test quick_translate_behavior shell_and_protocol_entries_cover_ocr_activation_contract -- --exact --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --test quick_translate_behavior runtime_plan_captures_desktop_integration_entries -- --exact --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_preview_iced desktop_runtime -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_preview_iced preview_runtime -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --test default_api_boundary_behavior default_process_spawn_surface_has_no_retained_dotnet_runtime_entries -- --exact --nocapture`
- `cargo check --manifest-path rs/Cargo.toml -p easydict_app --all-targets`
- `cargo check --manifest-path rs/Cargo.toml -p easydict_preview_iced --all-targets`

## 2026-06-13: Forwarded desktop runtime surfaces through the packaged GUI wrapper

- Continued hardening the first rs portable GUI entrypoint. `easydict_preview_iced.exe` is currently the binary copied to `Easydict.Rust.exe`, so its `PreviewApp` wrapper must expose the same runtime surfaces as the wrapped `EasydictApp`.
- No third-party crate is involved; this is an app/runtime wiring fix. `PreviewApp` now forwards `window_options`, `subscription`, `tray_menu`, `named_events`, `shell_verbs`, and `protocol_registrations` to the inner `EasydictApp`.
- This makes the packaged GUI runtime see the app-owned hotkeys, tray menu, named OCR event, mini/fixed/settings/capture/pop-button window options, shell context menu declarations, and `easydict://` protocol declaration instead of acting as a view/update-only preview wrapper.
- Added `easydict_preview_iced` tests proving the wrapper forwards desktop runtime surfaces and that `RuntimePlan::<PreviewApp>` captures the inner desktop integration entries.

Validation:

- `cargo test --manifest-path rs/Cargo.toml -p easydict_preview_iced -- --nocapture`
- `cargo check --manifest-path rs/Cargo.toml -p easydict_preview_iced --all-targets`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --test default_api_boundary_behavior default_process_spawn_surface_has_no_retained_dotnet_runtime_entries -- --nocapture`
- `cargo check --manifest-path rs/Cargo.toml -p easydict_app --all-targets`
...
## 2026-06-13: Ensured protocol registration through the Rust registry facade at app startup

- Continued the desktop-integration migration by making `Application::new(...)` enqueue a Rust-owned best-effort protocol registration task for the default `easydict://` activation scheme.
- No new third-party crate was needed for this slice. Protocol registration uses the existing `easydict_app::desktop_integration` facade and `lib/easydict-windows-registry`, preserving the same HKCU `Software\Classes\easydict` layout, `URL Protocol` marker, and `"exe" "%1"` command-line contract that the WinFluent platform adapter previously modeled.
- The startup path now calls `desktop_integration::register_protocol(...)` from a `Task::perform(...)` future and no longer needs WinFluent `Task::RegisterProtocol` for the app-owned protocol ensure. The `RuntimePlan` protocol declaration is still retained as declarative metadata for parity tests and future runtime integration.
- Kept the operation best-effort and `Message::Noop`-returning, matching the existing shell context-menu registration style and avoiding user-facing startup failures if HKCU protocol writes fail.

Validation:

- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --test quick_translate_behavior startup_protocol_registration -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --test quick_translate_behavior app_startup_registers_builtin_device_when_token_is_missing -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --test quick_translate_behavior runtime_plan_captures_desktop_integration_entries -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --test quick_translate_behavior shell_and_protocol_entries_cover_ocr_activation_contract -- --exact --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --test quick_translate_behavior startup_activation_parses_shell_and_protocol_ocr_triggers -- --exact --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app desktop_integration::tests::protocol_registration_plan_quotes_uri_argument --lib -- --exact --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --test default_api_boundary_behavior default_process_spawn_surface_has_no_retained_dotnet_runtime_entries -- --nocapture`
- `cargo check --manifest-path rs/Cargo.toml -p easydict_app --all-targets`
- `cargo check --manifest-path rs/Cargo.toml -p easydict_app --all-targets --features retained-dotnet-workers`

## 2026-06-13: Moved URL opening and bundled helper launch off WinFluent platform commands

- Continued the default rs desktop-integration cleanup by moving Settings/About external links, the tray Chrome extension link, and browser registrar install/uninstall launches away from WinFluent `PlatformCommand::OpenUrl` / `RunBundledExecutable`.
- Rechecked reusable crates before changing code. `open`, `webbrowser`, and `win-open` are viable general-purpose URL/default-app launch crates, but this slice only needs a narrow Windows shell boundary and should avoid cmd/script-style launch helpers. Added `lib/easydict-windows-shell` around Microsoft's official `windows` crate and native `ShellExecuteW`, plus a guarded bundled-executable launcher.
- Added `easydict_app::desktop_shell` as the app-owned facade. Browser support install/uninstall still launches `easydict_browser_registrar.exe` next to the current executable with the existing `install|uninstall --bridge-root-name EasydictRs` and optional `--chrome` / `--firefox` arguments, then keeps the 600ms browser-support status refresh. External links still resolve through the OS default handler and return `Message::Noop`.
- Hardened the bundled helper boundary so callers can only request a plain file name; absolute paths, subdirectories, `.` and `..` are rejected before process launch.
...
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --test quick_translate_behavior settings_about_links_open_external_urls -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --test default_api_boundary_behavior default_process_spawn_surface_has_no_retained_dotnet_runtime_entries -- --nocapture`
- `cargo check --manifest-path rs/Cargo.toml -p easydict_app --all-targets`
- `cargo check --manifest-path rs/Cargo.toml -p easydict_app --all-targets --features retained-dotnet-workers`

## 2026-06-13: Moved shell context-menu registration to the Rust registry helper

- Continued the desktop-integration migration by moving the Settings shell context-menu toggle away from WinFluent platform commands. The app now emits Rust-owned futures that call `easydict_app::desktop_integration` instead of `Task::RegisterShellVerb` / `Task::UnregisterShellVerb`.
- Rechecked reusable registry crates before changing code. `winreg` and Microsoft's `windows-registry` are both viable, but this slice extends the existing narrow `lib/easydict-windows-registry` helper so Browser registrar and shell/protocol registration share one HKCU registry boundary and keep exact unnamed/named `REG_SZ`, missing-key, and tree-delete semantics.
- Added `easydict_app::desktop_integration` as an app-owned facade for shell verb and protocol registration plans. The shell toggle writes the same `Software\Classes\*\shell\EasydictOCR` and `Software\Classes\Directory\Background\shell\EasydictOCR` command entries through the Rust helper, including the `Icon` named value, and unregister now deletes the verb tree.
- Kept protocol registration helpers in the same facade for the next coordinated desktop-integration slice, but did not change startup auto-registration behavior in this pass.

Validation:

- `cargo test --manifest-path lib/easydict-windows-registry/Cargo.toml --all-targets -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app desktop_integration --lib -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --test quick_translate_behavior shell_context_menu -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --test default_api_boundary_behavior default_process_spawn_surface_has_no_retained_dotnet_runtime_entries -- --nocapture`
- `cargo check --manifest-path rs/Cargo.toml -p easydict_app --all-targets`
...
## 2026-06-13: Moved OCR capture task surface to a lib-owned Windows helper

- Continued the core OCR migration by moving the selected-region capture and window-snapshot task surface out of WinFluent platform tasks.
- Rechecked reusable Rust screen-capture crates before changing code. `windows-capture` is active and Windows Graphics Capture/DXGI oriented, while `xcap` and `screenshots` are cross-platform screenshot abstractions. Easydict's current OCR overlay needs a narrow synchronous BGRA BitBlt capture plus Win32 window hierarchy enumeration, so this slice uses Microsoft's official `windows` crate directly in `lib/easydict-windows-screen-capture` instead of adding a broader recording/cross-platform dependency.
- Added `easydict_app::screen_capture_native` as an unsafe-free app facade that converts between the helper DTOs and the existing app/WinFluent DTOs. OCR confirm/copy capture and capture-overlay window snapshots now emit Rust-owned `Task::perform(...)` futures rather than WinFluent `Task::CaptureScreenRegion` / `Task::CaptureScreenWindows`.
- Left UI-owned files alone. The overlay background-freeze helper in `state.rs` still calls the existing WinFluent platform capture path and remains a coordinated follow-up with the UI workstream.

Validation:

- `cargo test --manifest-path lib/easydict-windows-screen-capture/Cargo.toml --all-targets -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app screen_capture_native --lib -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --test ocr_behavior capture -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --test default_api_boundary_behavior default_process_spawn_surface_has_no_retained_dotnet_runtime_entries -- --nocapture`
- `cargo check --manifest-path rs/Cargo.toml -p easydict_app --all-targets`
- `cargo check --manifest-path rs/Cargo.toml -p easydict_app --all-targets --features retained-dotnet-workers`

## 2026-06-13: Wired clipboard monitoring through an app-owned stream

- Continued the core clipboard migration by making saved `ClipboardMonitoring` settings start a Rust-owned monitor stream instead of relying on WinFluent `SubscriptionKind::Clipboard`, whose iced backend is still a no-op.
- No new clipboard crate was added. `clipboard-win` remains a viable crate for a future event-listener backend, but this slice can reuse the existing `lib/easydict-windows-text-selection` Win32 clipboard sequence/snapshot helper and stay aligned with TextSelection/TextInsertion semantics.
- `easydict_app::clipboard` now owns a small polling monitor state machine: it seeds from the current clipboard, watches sequence changes, trims/ignores empty text, suppresses duplicate text, and ignores the app's own just-written clipboard text once.
- App startup starts a lazy `Task::stream(...)` only when the saved setting is enabled. Settings edits continue to respect the existing unsaved-changes model: toggling alone does not start/stop monitoring, while saving or discarding settings resynchronizes the monitor with the persisted settings state.
- Added source and behavior coverage proving the app uses `clipboard::clipboard_monitor_stream(...)` rather than WinFluent `Subscription::clipboard(...)`.
...
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --test quick_translate_behavior monitor_clipboard -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --test quick_translate_behavior clipboard_monitor -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --test quick_translate_behavior settings_changes_prompt_on_back_and_save_discard_cancel_are_stateful -- --exact --nocapture`
- `cargo check --manifest-path rs/Cargo.toml -p easydict_app --all-targets`

## 2026-06-13: Centralized rs portable retained byte marker scanning

- Continued the first rs portable no-runtime hardening by moving the allowlisted-executable byte marker scanner out of `easydict_packager` and into `lib/easydict-runtime-guards`.
- No third-party crate is appropriate for this slice: the scanner is an Easydict-specific denylist for stale `.NET` host/runtime, retained worker, PowerShell, and old TTS script markers. Keeping it in `lib/` lets browser/runtime/package boundaries reuse one policy instead of re-copying byte tables.
- `validate-rs-portable` still rejects first-release allowlisted executables such as `Easydict.Rust.exe` and Rust helper `.exe` files if their bytes contain ASCII or UTF-16LE retained runtime markers, but the actual marker matching now calls `easydict_runtime_guards::bytes_contain_retained_runtime_marker(...)`.
- Added a packager source-boundary regression so the byte marker table and ASCII/UTF-16LE scanner are not re-inlined into `easydict_packager`.

Validation:

- `cargo test --manifest-path lib/easydict-runtime-guards/Cargo.toml --all-targets -- --nocapture`
- `cargo test --manifest-path lib/easydict-runtime-guards/Cargo.toml --all-targets --features retained-dotnet-workers -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_packager marker -- --nocapture`
- `cargo check --manifest-path rs/Cargo.toml -p easydict_packager --all-targets`

## 2026-06-13: Moved runtime route policy into lib-owned guards

- Continued the default rs runtime cleanup by moving retained-worker runtime-profile parsing and disable-message policy out of `easydict_app::runtime_policy` and into `lib/easydict-runtime-guards`.
- No third-party crate is appropriate for this slice: the behavior is Easydict-specific runtime-profile policy, not a reusable parser concern. The existing lib boundary now owns both stale retained payload path classification and the runtime route policy gate.
- `easydict_app::runtime_policy` is now a thin re-export shim, preserving current Quick Translate, LongDoc, and retained-feature compatibility call sites while moving the implementation and feature-on policy matrix into `lib/`.
- Propagated the app `retained-dotnet-workers` feature to `easydict_runtime_guards/retained-dotnet-workers`, so compiling the retained bridge still requires the same explicit `EASYDICT_RUNTIME_PROFILE=hybrid` opt-in before workers can be enabled.
- Added a default API boundary regression proving the app shim does not re-inline environment parsing or runtime policy implementation.

Validation:

- `cargo test --manifest-path lib/easydict-runtime-guards/Cargo.toml --all-targets -- --nocapture`
- `cargo test --manifest-path lib/easydict-runtime-guards/Cargo.toml --all-targets --features retained-dotnet-workers -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app runtime_policy --lib -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --features retained-dotnet-workers runtime_policy --lib -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --test default_api_boundary_behavior default_runtime_policy_implementation_is_lib_owned -- --nocapture`
- `cargo check --manifest-path rs/Cargo.toml -p easydict_app --all-targets`

## 2026-06-13: Centralized retained payload path guards outside app code

- Continued shrinking default app production-source exposure to retained runtime markers without changing browser registration behavior.
- No third-party crate is appropriate for this slice: the logic is Easydict-specific policy over stale retained payload roots rather than generic path normalization. Added `lib/easydict-runtime-guards` as a small safe Rust library that classifies retained runtime/path components such as worker roots, legacy bridge names, and `.NET` payload roots.
- Updated browser native-messaging registrar source validation to call `easydict_runtime_guards::path_has_retained_runtime_component(...)` instead of inlining the marker list in `easydict_app::browser_registrar`. Existing protections still reject renamed `easydict-native-bridge.exe` files under stale retained payload directories.
- Added a browser registrar source-boundary guard so retained payload marker policy stays lib-owned.

Validation:

- `cargo test --manifest-path lib/easydict-runtime-guards/Cargo.toml --all-targets -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --test browser_registrar_behavior browser_registrar_source_uses_lib_owned_retained_runtime_guard -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --test browser_registrar_behavior -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --test default_api_boundary_behavior default_process_spawn_surface_has_no_retained_dotnet_runtime_entries -- --nocapture`
- `cargo check --manifest-path rs/Cargo.toml -p easydict_app --all-targets`

## 2026-06-13: Moved app file/folder dialogs off PowerShell System.Windows.Forms

- Continued the default rs no-runtime cleanup on a core UI workflow boundary: LongDoc input selection, LongDoc output-folder selection, and MDX dictionary import no longer emit WinFluent `OpenFileDialog` / `OpenFolderDialog` tasks, whose iced backend still uses PowerShell plus `System.Windows.Forms`.
- Rechecked reusable dialog crates before changing code. `rfd` and `native-dialog` are mature cross-platform file-dialog crates, but this app only needs a narrow Windows portable boundary and must avoid script/runtime helpers. Added `lib/easydict-windows-dialogs` around Microsoft's official `windows` crate and the native COM `IFileOpenDialog` API instead.
- Added `easydict_app::file_dialog` as the app-owned facade. Dialog work runs on a dedicated STA thread, maps user cancel to `None`, supports file filters and initial folders, and keeps app crate unsafe-free.
- Added source/behavior coverage proving app update routes these user-driven file selections through Rust-owned futures instead of WinFluent dialog tasks.

Validation:

- `cargo test --manifest-path lib/easydict-windows-dialogs/Cargo.toml --all-targets -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app file_dialog --lib -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --test quick_translate_behavior import_mdx_dictionary_uses_rust_owned_mdx_file_dialog_task -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --test quick_translate_behavior app_update_routes_file_dialogs_to_rust_owned_helpers -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --test long_document_behavior app_update_long_document_browse_starts_file_dialog_only_in_long_document_mode -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --test default_api_boundary_behavior default_process_spawn_surface_has_no_retained_dotnet_runtime_entries -- --nocapture`
- `cargo check --manifest-path rs/Cargo.toml -p easydict_app --all-targets`

## 2026-06-13: Moved explicit clipboard read/write off WinFluent tasks

- Continued the core input/clipboard migration without touching `state.rs`, `ui.rs`, or `lib/winfluent-rs`.
- Rechecked reusable clipboard crates before changing code. `clipboard-win` is the closest Windows-specific crate and includes optional monitoring support, while `arboard` is a mature cross-platform text/image clipboard abstraction. This slice needs exact reuse of the existing Windows clipboard semantics already implemented for TextSelection/TextInsertion, so it reuses `lib/easydict-windows-text-selection` instead of adding a second clipboard dependency.
- Added `easydict_app::clipboard`, a small app-owned facade over the Rust Windows helper. Explicit tray `Translate Clipboard` reads now use a `Task::perform(...)` future that maps clipboard errors to `None` like the previous non-fatal UI behavior; result Copy and Silent OCR copy now write through the same Rust-owned helper and resolve to `Message::Noop` on completion.
- Added a source-boundary regression so the app core no longer emits WinFluent `Task::read_clipboard_text` or `Task::clipboard_text` for explicit clipboard actions. The later clipboard-monitoring slice wires the sustained monitor through an app-owned stream instead of WinFluent's no-op clipboard subscription.
...
- Rechecked the user-requested MDD follow-up on the current dirty tree. The core Rust path is already implemented through the existing in-tree `lib/rs-mdict` fork and app-side `mdx_native` glue; no new parser dependency is needed.
- Confirmed support for MDD raw/base64 resource lookup, compressed and non-ASCII resources, cross-record-block reads, credential-encrypted companion MDD via the MDX key-header transform, same-stem `.mdd` / numbered `.N.mdd` discovery, multi-MDD first-hit lookup, failed-MDD skipping, WebView-ready `data:<mime>;base64,...` HTML inlining, and Quick Translate `rawHtml` retention only when MDD resources were actually inlined.
- The remaining MDD-related work is evidence/UX polish rather than core runtime migration: real large-dictionary/manual WebView screenshot review is still useful, but the Rust parser and Quick Translate behavior are covered by focused automated tests.
...
## 2026-06-13: Moved browser registrar registry access off WinFluent

- Continued shrinking default app dependencies on WinFluent platform internals by moving the Browser Native Messaging registrar registry adapter into `lib/easydict-windows-registry`.
- Rechecked the reusable option first. Microsoft `windows-registry` is a high-quality safe crate, but direct use would broaden deletion semantics to recursive tree removal and does not map as directly onto unnamed/default value handling. This helper therefore uses Microsoft's official `windows` crate directly to preserve the previous exact Win32 behavior: HKCU default `REG_SZ` read/write, non-recursive `RegDeleteKeyW`, missing key/value as a neutral success/miss.
- Updated `easydict_browser_registrar` so `SystemBrowserRegistry` calls the Rust-owned helper rather than `win_fluent_platform_win::WindowsPlatformAdapter`. Added a browser registrar source-boundary test to prevent the binary from reintroducing WinFluent registry access.
- Left UI, state, and WinFluent files untouched. The remaining non-UI direct WinFluent platform dependency in default app code is the screen-capture bridge in `state.rs`, which is owned by the parallel UI work and was not changed in this slice.
...
## 2026-06-13: Moved native bridge named-event signaling off WinFluent

- Continued shrinking default app dependencies on WinFluent platform internals by moving the browser native-messaging OCR signal path into `lib/easydict-windows-ipc`.
- Rechecked reusable options first. Named events are a very narrow Win32 synchronization-object boundary, and the available higher-level Rust crates do not add value over the official `windows` crate for Easydict's exact `OpenEventW` / `SetEvent` semantics. The helper therefore exposes a small safe `signal_named_event(...) -> Result<bool, WindowsIpcError>` API, with a missing event preserving the existing `Ok(false)` behavior.
- Updated `easydict-native-bridge` to signal `Local\EasydictRs-OcrTranslate` through the Rust-owned helper instead of `win_fluent_platform_win::WindowsPlatformAdapter`. The browser stdio protocol, unknown-action no-signal behavior, and Rust helper naming remain unchanged.
- Left UI, state, and WinFluent files untouched. The remaining non-UI direct WinFluent platform dependency in default app code is the browser registrar registry adapter.

Validation:

- `cargo test --manifest-path lib/easydict-windows-ipc/Cargo.toml --all-targets -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --test native_bridge_behavior -- --nocapture`

## 2026-06-13: Moved credential DPAPI boundary off WinFluent

- Continued removing default app dependencies on WinFluent platform internals by moving credential DPAPI and HKLM `MachineGuid` registry access into `lib/easydict-windows-credentials`.
- Rechecked reusable DPAPI crates first. `windows-dpapi` now exists as a small safe wrapper, but this path needs exact compatibility with Easydict's existing `edcred1` optional entropy, current-user/local-machine scope wire format, and machine-id fallback behavior, so the implementation uses Microsoft's official `windows` crate directly in a narrow in-tree helper.
- Updated `easydict_app::credential_protection` to call the new helper instead of `win_fluent_platform_win::WindowsPlatformAdapter`. The public credential format remains `edcred1:user|machine:<base64>`, legacy `edloc1:` AES-GCM migration remains unchanged, and app-side unsafe code stays out of `easydict_app`.
- Left UI, state, and WinFluent files untouched; remaining direct WinFluent platform uses outside UI/state are browser registrar registry helpers and native-bridge named-event signaling, which are good follow-up slices.

Validation:

- `cargo check --manifest-path lib/easydict-windows-credentials/Cargo.toml --all-targets`
- `cargo test --manifest-path lib/easydict-windows-credentials/Cargo.toml --all-targets -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --test credential_protection_behavior -- --nocapture`
- `cargo check --manifest-path rs/Cargo.toml -p easydict_app --all-targets`
- `cargo check --manifest-path rs/Cargo.toml -p easydict_app --all-targets --features retained-dotnet-workers`

## 2026-06-13: Hardened Rust-only package scans for script/TTS runtime markers

- Followed up the Rust-owned TTS backend by hardening the release gates that prove rs portable/MSIX payloads do not carry the old script-based speech route. Rechecked PE parser options first: `goblin`, `object`, and `pelite` all support PE/COFF inspection, but this slice only needs fixed ASCII/UTF-16LE marker detection in package payload bytes, so the existing byte scanner was extended instead of adding a parser dependency.
- Extended `easydict_packager` rs portable validation markers beyond retained .NET host/runtime strings to include explicit script/TTS backend markers: `powershell.exe`, `pwsh.exe`, `System.Speech`, `System.Management.Automation`, and the old `WIN_FLUENT_TTS_TEXT` environment marker. Directory and ZIP portable validation both reject allowlisted helper executables containing those markers.
- Extended `easydict_msix_validate` rust-only payload scanning with the same markers. This covers direct MSIX validation, `prepare-package-inputs` preflight before manifest writing, and rust-only nested appx/msix validation inside MSIX bundles.

Validation:

- `cargo test --manifest-path rs/Cargo.toml -p easydict_packager validate_rs_portable_rejects_allowlisted_exe_that_contains_script_tts_markers -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_packager validate_rs_portable_rejects_allowlisted_exe_that_contains_utf16le_dotnet_markers -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_msix_validate rust_only_package_rejects_script_tts_markers -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_msix_validate prepare_package_inputs_rejects_script_tts_marker_before_manifest_write -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_msix_validate verify_bundle_minversion_rust_only_rejects_nested_helper_script_tts_markers -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_msix_validate rust_only_package_rejects_utf16le_dotnet_host_markers -- --nocapture`

## 2026-06-13: Moved Quick Translate TTS off PowerShell/System.Speech

- Continued the core `.NET runtime` removal work on the TTS path. Rechecked reusable Rust TTS options first: `tts` provides a broad cross-platform abstraction but brings extra backend/event surface that Easydict does not need for first rs portable; `sapi-lite` is a small SAPI wrapper but has limited maintenance/coverage. The chosen route is a small in-tree wrapper under `lib/` using Microsoft's official `windows` crate and SAPI directly.
- Added `lib/easydict-windows-tts`, which initializes COM inside the helper, creates `ISpVoice`, optionally chooses an installed SAPI voice by BCP-47/LCID language match, and speaks plain text through the system default audio output without launching PowerShell or loading `System.Speech`.
- Added `easydict_app::tts`, a fakeable app-owned TTS boundary. Result Speak actions and `AutoPlayTranslation` now emit Rust `Task::perform(...)` futures instead of WinFluent `PlatformCommand::SpeakText`, so the app-side Quick Translate route no longer depends on the WinFluent PowerShell backend.
- Kept error handling fire-and-forget for parity with the previous platform side effect: empty text is skipped before backend calls, and native TTS errors resolve to `Message::Noop` so translation state is not disrupted. Real Notepad/app audio smoke remains pending for a later Windows ignored/manual pass.

Validation:

- `cargo check --manifest-path lib/easydict-windows-tts/Cargo.toml --all-targets`
- `cargo test --manifest-path lib/easydict-windows-tts/Cargo.toml --all-targets -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app tts --lib -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --test quick_translate_behavior app_update_result_actions_emit_platform_side_effect_tasks -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --test quick_translate_behavior auto_play_translation_speaks_first_successful_result_only -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --test quick_translate_behavior auto_play_translation_uses_floating_surface_target_language -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --test quick_translate_behavior auto_play_translation_ignores_disabled_empty_error_and_stale_updates -- --nocapture`
- `cargo check --manifest-path rs/Cargo.toml -p easydict_app --all-targets`

## 2026-06-13: Moved TextInsertion replace backend into Rust-owned helper boundary

- Continued the core interaction migration without touching `ui.rs`, `state.rs`, `ui_contract.rs`, or WinFluent. Rechecked reuse options first: `clipboard-win` only covers clipboard operations, while `enigo`/`rdev` abstract keyboard simulation but do not preserve Easydict's HWND capture, foreground-window validation, thread-input attach, clipboard write, and `dwExtraInfo` requirements. The implementation therefore stays on the existing Microsoft `windows` crate helper in `lib/easydict-windows-text-selection`.
- Added Rust-native text insertion primitives next to TextSelection: foreground insertion-target capture, target validation, `Ctrl+V` SendInput, and `insert_text_into_target(...)` built from clipboard write + foreground restoration. Non-Windows remains an explicit unsupported boundary.
- Added `easydict_app::text_insertion`, a fakeable app-owned backend/cache for the last captured source window. `TranslateSelection`, PopButton click, Mini show hotkey, tray Translate Clipboard, and result Replace now use Rust `Task::perform(...)` helpers instead of WinFluent `PlatformCommand::CaptureTextInsertionTarget` / `InsertText`.
- Kept user-visible error handling conservative for this slice: insertion failures complete as `Message::Noop`, matching the previous fire-and-forget platform side effect shape. A later UI slice can surface failures without changing the helper boundary.

Validation:

- `cargo check --manifest-path lib/easydict-windows-text-selection/Cargo.toml --all-targets`
- `cargo test --manifest-path lib/easydict-windows-text-selection/Cargo.toml --all-targets -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app text_insertion --lib -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --test quick_translate_behavior app_update_result_actions_emit_platform_side_effect_tasks -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --test quick_translate_behavior app_update_translate_selection_captures_text_insertion_target -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --test quick_translate_behavior pop_button_click_translates_pending_text_in_mini_window -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --test quick_translate_behavior translate_selection_hotkey_captures_source_and_starts_native_selection_task -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --test quick_translate_behavior tray_commands_route_to_existing_desktop_actions -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --test quick_translate_behavior floating_window_show_hotkeys_keep_explicit_show_semantics -- --nocapture`
- `cargo check --manifest-path rs/Cargo.toml -p easydict_app --all-targets`
- `cargo check --manifest-path rs/Cargo.toml -p easydict_app --all-targets --features retained-dotnet-workers`

## 2026-06-13: Hardened Rust-only MSIX UTF-16LE .NET marker scan

- Reviewed PE inspection crate options before changing the release gate. `object`, `goblin`, and `pelite` all expose PE/COFF parsing, but this slice only needs known retained-runtime string detection across MSIX and prepared publish inputs, so the existing byte scanner was extended without adding a new dependency.
- `easydict_msix_validate` now scans Rust-only payload content markers in both ASCII and UTF-16LE encodings. This aligns MSIX/prepare-package-inputs with the existing `validate-rs-portable` protection and closes the renamed .NET apphost/resource-string variant before manifest writing or package validation succeeds.
- Added regressions for a helper executable and a `prepare-package-inputs` publish-dir file containing UTF-16LE `hostfxr.dll` / CoreLib markers.

Validation:

- `cargo test --manifest-path rs/Cargo.toml -p easydict_msix_validate rust_only_package_rejects_utf16le_dotnet_host_markers -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_msix_validate prepare_package_inputs_rejects_utf16le_dotnet_apphost_marker_before_manifest_write -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_msix_validate -- --nocapture`

## 2026-06-13: Wired Rust Quick Translate auto-play TTS

- Continued with a small core UX closure that avoids the UI/state files currently being edited in parallel. The reusable-library check does not call for a new dependency here: Rust already has the platform-neutral `Task::speak_text` side-effect contract, and the backend maps it to the current Windows speech path.
- `EasydictApp::update(...)` now turns a successful `QuickTranslateServiceFinished` into a TTS platform task when `AutoPlayTranslation` is enabled. It matches the .NET behavior of playing only the first successful result for an active query, skips disabled/empty/error/stale updates, and uses the active surface target language for Main/Mini/Fixed windows.
- Streaming remains safe because only the final service-finished update can trigger auto-play; intermediate `QuickTranslateStreamChunk` messages do not speak.
- While running the full quick-translate suite, restored the encrypted MDX route predicate: `Encrypted=1` dictionaries with no credentials are still Rust-native candidates, so lookup fails locally with credentials-required instead of falling out of the native route. Invalid supplied credentials remain a local input error.
- Added the next MouseSelection app-boundary adapter without touching UI/state/WinFluent: producer `DismissPopButton` actions map to an app message task, `CaptureSelectionText` actions reuse the native selected-text capture task, and pending multi-click timer metadata was exposed at this step before the default runtime hook/timer wiring was completed later on 2026-06-13.

Validation:

- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --test quick_translate_behavior auto_play_translation -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --test quick_translate_behavior encrypted_mdx -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --test quick_translate_behavior mouse_selection_producer -- --nocapture`

## 2026-06-13: Aligned unknown-process TextSelection fallback with .NET

- Closed a small but user-visible TextSelection parity gap found while reviewing the .NET strategy. When the foreground process name cannot be resolved, `.NET TextSelectionService` still classifies it as neither Electron nor terminal and therefore tries UIA first, then clipboard fallback with the standard timeout.
- Removed the Rust-only conservative UIA-only branch for unknown process names. Rust now treats that case like a regular desktop app: UIA first, then clipboard fallback; empty normalized process names still do not enter adaptive non-text suppression.
- Kept the change inside the existing Rust TextSelection strategy layer. No new dependency was needed; the native primitives remain in `lib/easydict-windows-text-selection`.

Validation:

- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --test text_selection_behavior -- --nocapture`

## 2026-06-13: Added Rust TextSelection UIA timeout guard

- Continued the core selected-text migration without touching UI/WinFluent. The reusable-library decision remains the same as the TextSelection backend foundation: `get-selected-text` is too narrow/clipboard-oriented for Easydict's terminal-safe flow, and `uiautomation` only covers UIA while the app also needs Win32 foreground, clipboard sequence, SendInput, and low-level hook primitives. The safe wrapper stays in `lib/easydict-windows-text-selection` on Microsoft's `windows` crate.
- Added a Rust UIA execution timeout matching the .NET guard shape: one UIA capture at a time, a 200ms "busy" wait before skipping, and an 800ms execution timeout before returning a local `WindowsTextSelectionError` instead of blocking the hotkey/PopButton capture path indefinitely.
- Aligned the UIA pattern fallback with the .NET path: try `TextPattern` before `TextPattern2`, and treat an individual pattern failure as a miss so the next pattern or clipboard fallback can still run.
- Switched `NativeTextSelectionBackend` to the timeout-aware helper. Existing app strategy stays unchanged: regular desktop apps fall back to clipboard after UIA timeout/failure, Electron still tries clipboard first and avoids double Ctrl+C, and terminal apps remain UIA-only so a timeout never sends SIGINT.
...
## 2026-06-13: Added WindowsNative OCR RecognizeAsync timeout/cancel guard

- Continued the Rust-native OCR hardening after the image-dimension guard. The reusable-library check remains unchanged: Windows Native OCR is already best served by Microsoft `windows` / `windows-future`; third-party OCR engines such as Tesseract/leptess would replace the engine and add native packaging, while generic wrappers would not improve Easydict's DTO/language/fallback contract.
- `recognize_bgra_file(...)` no longer blocks indefinitely on `RecognizeAsync(...).join()`. It registers a `windows-future` completion callback with `when(...)`, waits on a standard channel with a 30s default timeout, and calls `Cancel()` on timeout before returning a local Rust `WindowsOcrError`.
- Kept the public API stable and did not introduce an async runtime or new dependency. The timeout is intentionally generous for screenshot OCR and leaves normal completion behavior unchanged.

Validation:

- `cargo test --manifest-path rs/Cargo.toml -p easydict_windows_ocr -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --test ocr_behavior -- --nocapture`
- `cargo check --manifest-path rs/Cargo.toml -p easydict_windows_ocr --all-targets`
- `cargo check --manifest-path rs/Cargo.toml -p easydict_app --all-targets`
- `cargo check --manifest-path rs/Cargo.toml -p easydict_app --all-targets --features retained-dotnet-workers`

## 2026-06-13: Added Rust low-level mouse selection hook foundation

- Continued the PopButton/mouse-selection migration after TextSelection capture was wired. Two read-only subagents confirmed the remaining gap is the real `WH_MOUSE_LL` / `WH_KEYBOARD_LL` producer and its app message adapter; the PopButton consumer path (`SelectionTextReady`, auto-dismiss, click-to-Mini translation) already exists.
- Rechecked reusable hook crates before changing code. `willhook` wraps Windows low-level hooks and `rdev` exposes cross-platform global input events, but both abstract away details Easydict needs to preserve for parity: raw Win32 mouse/keyboard message ids, `dwExtraInfo` for `EASYDICT_SYNTHETIC_KEY`, exact event timestamps, and point-to-PopButton-root hit-testing. The implementation therefore stays in `lib/easydict-windows-text-selection` on the official Microsoft `windows` crate instead of adding a third-party hook dependency.
- Added `LowLevelInputHookEvent` plus a Windows `start_low_level_input_hook()` helper that installs `WH_MOUSE_LL` and `WH_KEYBOARD_LL` on a background message-loop thread, forwards raw hook events through a channel, and unhooks/joins on drop. Non-Windows remains an explicit unsupported boundary.
- Added small platform helpers for `GetDoubleClickTime`, `GetTickCount64`, and `WindowFromPoint`/`GetAncestor(GA_ROOT)` root hit-testing so the later PopButton producer can keep the same double-click timing and "clicking the PopButton itself does not dismiss" behavior as the .NET service.
- Added an app-side raw-event reducer in `mouse_selection.rs` that feeds the new helper events into the existing pure `MouseSelectionHookState` without touching UI/state files. Mouse events now reuse drag/multi-click planning, while keyboard events reuse the synthetic Ctrl+C dismiss guard.
- Added a pure `MouseSelectionProducer` action layer on top of the reducer. It owns monotonic PopButton generations, converts drag selections into `CaptureSelectionText` requests, converts double/triple-click detections into cancellable pending timer actions, ignores stale pending generations, and exposes a `MouseSelectionTextReady` shape matching the existing `SelectionTextReady` app message fields. This lets the later runtime subscription map actions to `DismissPopButton`, timer tasks, selection capture, and `SelectionTextReady` without adding unsafe code or UI dependencies to `easydict_app`.
- Added `mouse_selection_capture_result_message(...)` and `mouse_selection_capture_task(...)` at the app boundary. A producer `CaptureSelectionText` request can now reuse the existing native selected-text capture future and maps nonblank output directly into the existing `SelectionTextReady` PopButton consumer; blank/failed capture maps to `Noop`.
- Remaining work: wire this helper into an actual app subscription/background producer, schedule pending multi-click timers in the runtime task layer, and dispatch producer actions to `DismissPopButton`, `mouse_selection_capture_task(...)`, and timer completion. Real Notepad/VS Code/Windows Terminal smoke coverage is still pending.

Validation:

- `cargo test --manifest-path lib/easydict-windows-text-selection/Cargo.toml --all-targets -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --test mouse_selection_behavior -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --test quick_translate_behavior mouse_selection_capture -- --nocapture`
- `cargo check --manifest-path rs/Cargo.toml -p easydict_app --all-targets`
- `cargo check --manifest-path rs/Cargo.toml -p easydict_app --all-targets --features retained-dotnet-workers`

## 2026-06-13: Added WindowsNative OCR MaxImageDimension guard

- Continued the Rust-native OCR hardening slice after the availability/status boundary. A read-only subagent checked the `windows 0.62.2` bindings and the .NET reference path; Microsoft exposes `OcrEngine::MaxImageDimension()` as the documented per-side image limit for `Windows.Media.Ocr`, while `RecognizeAsync` cancellation/timeout parity should be a later API-shape decision rather than part of this small guard.
- Kept the implementation inside the existing `easydict_windows_ocr` helper crate. No new OCR dependency was added: Tesseract/leptess remain heavier engine replacements, while the current Rust wrapper already targets Easydict's Windows Native OCR contract through Microsoft's `windows` WinRT bindings.
- `recognize_bgra_file(...)` now creates the OCR engine first, preserving the existing "no engine available -> empty result" behavior, then checks `OcrEngine::MaxImageDimension()` before allocating `SoftwareBitmap` or calling `RecognizeAsync`. Images whose width or height exceeds the engine limit now fail locally with a clear Rust-native OCR error instead of depending on WinRT allocation/recognition failure.
- Added pure crate tests for `width == max` / `height == max` acceptance, width/height over-limit rejection, and invalid zero engine-limit handling. The broader app OCR behavior tests still prove WindowsNative/Ollama/Custom API routes do not probe legacy OCR workers or CompatHost.
...
## 2026-06-13: Hardened WindowsNative OCR availability for portable rs

- Rechecked OCR replacement options before changing code. `leptess`/Tesseract are mature general OCR bindings but bring Tesseract/Leptonica/native language-data packaging; `oneocr-rs` targets the Windows 11 Snipping Tool engine rather than the documented Easydict behavior; thin Windows OCR wrappers such as `win_ocr` do not cover Easydict's DTO/language/fallback contracts better than the current in-tree wrapper. The existing Rust-owned `easydict_windows_ocr` crate on Microsoft `windows` WinRT bindings remains the best fit for Windows Native OCR parity.
- Microsoft documents `Windows.Media.Ocr` as a package-identity API for desktop apps, which matters for the first rs portable build. The Rust app already runs WindowsNative OCR locally and does not probe legacy `workers/ocr`/CompatHost; this slice tightens the availability/status path so portable/unavailable environments report `isAvailable=false` with an empty language list instead of attempting `AvailableRecognizerLanguages()` after the engine is unavailable.
- Added `windows_native_ocr_availability_skips_language_query_when_unavailable` to lock that behavior through the fake recognizer. This keeps settings/diagnostics callers on a clear Rust-native unavailable boundary and avoids resurrecting any `.NET` OCR worker fallback.

Validation:

- `cargo test --manifest-path rs/Cargo.toml -p easydict_windows_ocr -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --test ocr_behavior windows_native_ocr_availability -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --test ocr_behavior app_dir_runner_uses_native_windows_ocr_without_legacy_runtime -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --test ocr_behavior current_app_dir_runner_uses_default_windows_native_despite_hybrid_profile_and_stale_worker_markers -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --test ocr_behavior -- --nocapture`
- `cargo check --manifest-path rs/Cargo.toml -p easydict_windows_ocr --all-targets`
- `cargo check --manifest-path rs/Cargo.toml -p easydict_app --all-targets`
- `cargo check --manifest-path rs/Cargo.toml -p easydict_app --all-targets --features retained-dotnet-workers`

## 2026-06-13: Wired Rust TextSelection native backend into Translate Selection

- Continued the core input/selection migration after the MDD route was verified. A read-only subagent audit confirmed the critical gap was not the planner but the missing app event wiring and foreground process-name context needed to avoid sending Ctrl+C to terminals.
- Kept the backend on the official Microsoft `windows` crate inside the new `lib/easydict-windows-text-selection` helper crate. `uiautomation` remains useful for UIA-only experiments, but this slice also needs foreground HWND/PID, process-image lookup, clipboard sequence polling, clipboard restore, thread-input focus, and `SendInput`; generic `get-selected-text` crates do not cover Easydict's Electron/terminal/suppression contract.
- Added `process_name_for_id(...)` to the helper crate so Rust can classify the foreground target using `.NET Process.ProcessName`-style stems. If process-name resolution fails, the Rust planner now takes a conservative UIA-only route rather than risking a clipboard fallback into a terminal.
- Added a native capture wrapper with a shared suppression tracker in `text_selection.rs`: foreground target discovery, process classification, UIA attempt, clipboard fallback with ClipWait/restore, and success/non-text suppression recording now run behind the same fakeable planner tested earlier.
- Wired `Message::TranslateSelection` and the settings "Translate Selection" hotkey to capture the insertion target and then run the Rust-native selected-text future. Non-empty captured text reuses the existing quick-translate entry point; the tray's explicit "Translate Clipboard" command remains on the old read-clipboard task.
- Remaining TextSelection work: real mouse-hook producer to feed `SelectionTextReady` for PopButton, Windows ignored/E2E coverage against Notepad/terminal apps, and a timeout guard around synchronous UIA calls to match the .NET 200ms/800ms safeguards.

Validation:

- `cargo test --manifest-path lib/easydict-windows-text-selection/Cargo.toml --all-targets`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --test text_selection_behavior -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --test quick_translate_behavior translate_selection -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --test quick_translate_behavior tray_commands_route_to_existing_desktop_actions -- --nocapture`
- `cargo check --manifest-path rs/Cargo.toml -p easydict_app --all-targets`

## 2026-06-13: Hardened raw retained WorkerCommand startup guard

- Continued the first rs portable/runtime boundary hardening. No new library was needed; this is an internal retained-worker feature guard rather than a replacement of parser/OCR/Win32 functionality.
- `compat_client::WorkerCommand::spawn(...)` now classifies retained worker/runtime marker paths even when callers construct the command directly with `WorkerCommand::new(...)` instead of the packaged helper. Paths under `workers/<kind>`, `Easydict.Workers.*` executables, `dotnet.exe`, hostfxr/hostpolicy/CoreCLR DLLs, and `dotnet` path components all require the same explicit `EASYDICT_RUNTIME_PROFILE=hybrid` opt-in before any executable probing or deferred `.NET` runtime env injection.
- Added feature-gated `compat_client` regressions proving raw LongDoc, LocalAI, unknown worker, and dotnet runtime commands fail locally with Rust-native-route-required when the profile is unset, while explicit `hybrid` still reaches the normal missing-executable I/O boundary for coexistence tests.

Validation:

- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --features retained-dotnet-workers --test compat_client raw_worker_command -- --nocapture`

## 2026-06-13: Added Rust TextSelection pure strategy core

- Rechecked reusable Windows selection/clipboard crates before changing code. `uiautomation` is a useful Windows UI Automation wrapper, but this slice only migrates Easydict's app-specific strategy planner; `get-selected-text` style crates are too generic for the terminal/Electron/suppression/clipboard-restore contract. No new dependency was added; the later Win32/UIA backend can stay on the official `windows` crate boundary already used elsewhere.
- Added Rust `text_selection` pure logic for process-name normalization, Electron and terminal classification, target selection planning, ClipWait payload classification, per-process non-text suppression, clipboard restore decisions, synthetic Ctrl+C key input planning, and a small attempt-result reducer.
- Added `text_selection_behavior`, mirroring the .NET `TextSelectionServiceTests` strategy boundaries: Electron clipboard-first route, terminal UIA-only route, ordinary UIA-then-clipboard route, own-process skip, two-tick non-text ClipWait, suppression expiry/recovery/exemptions, unknown-process non-suppression, modifier flushing before Ctrl+C, `EASYDICT_SYNTHETIC_KEY` marker propagation, and final outcome recording for success/failed clipboard attempts.

Validation:

- `cd rs; cargo test -p easydict_app --test text_selection_behavior -- --nocapture`
- `rustfmt --edition 2021 --check rs/crates/easydict_app/src/text_selection.rs rs/crates/easydict_app/tests/text_selection_behavior.rs`

## 2026-06-12: Added Rust encrypted-MDD key-header transform support

- Split the user-requested MDD follow-up with read-only subagents: app-side companion discovery/resource inlining is already wired, while the library audit found one real core gap in `lib/rs-mdict`: MDD did not expose the same key-header transform entry point that MDX already had.
- Rechecked reusable parser options before changing code. `mdict-rs` advertises defensive MDX/MDD parsing but is AGPL-3.0-only; `readmdict` is MIT but direct-port style; `libmdx` advertises broader features but is AGPL-3.0. The existing MIT in-tree `lib/rs-mdict` remains the lowest-risk route.
- Added `Mdd::new_with_key_header_transform(...)`, reusing `MdictBase::new_with_key_header_transform(...)`, so encrypted companion MDD files can be opened by callers that already own the credential-derived decryptor. Without the transform, encrypted MDD still fails locally with `EncryptedFileRequiresPasscode` instead of being treated as a silent miss.
- Updated the app-side `RsMdictMddReaderFactory` to receive the MDX dictionary snapshot and reuse the existing MDX credential-derived key-header decryptor for companion MDD files. Plain MDDs still open through the unchanged `Mdd::new(...)` route.
- Added generated encrypted-MDD fixtures covering the public constructor, caller transform, encrypted record-block decrypt path, resolved resource key, raw bytes, MIME inference, and HTML data-URL inlining through the production app MDD factory.

Validation:

- `cargo test --manifest-path lib/rs-mdict/Cargo.toml new_with_key_header_transform_reads_record_encrypted_mdd_resource -- --nocapture`
- `cd rs; cargo test -p easydict_app --test mdx_native_behavior native_mdd_resource_lookup_reads_record_encrypted_mdd_with_dictionary_credentials -- --exact --nocapture`
...
- `cd rs; cargo test -p easydict_app --test settings_storage_behavior mdd -- --nocapture`
...
## 2026-06-12: Required real files for first rs portable payload entries

- Continued the rs portable validator hardening from the packaging audit. No new dependency was needed; the existing ZIP and filesystem metadata checks are enough for this payload-shape guard.
- `validate-rs-portable` now treats required entries as missing unless they are ordinary non-empty files. A ZIP directory named like `easydict_cli.exe` or an empty helper exe no longer satisfies the first-release allowlist.
- Added directory and ZIP regression coverage for required helper exe entries that are directories or zero-byte files.

Validation:

- `cd rs; cargo test -p easydict_packager validate_rs_portable_rejects_required_exe_directory_or_empty_entries -- --nocapture`
- `cd rs; cargo test -p easydict_packager validate_rs_portable -- --nocapture`

## 2026-06-12: Required explicit hybrid opt-in for ARM64 MSIX smoke

- A parallel packaging audit found that the ARM64 MSIX smoke workflow still defaulted its manual `runtime_profile` input to `hybrid`, which could build the retained .NET/MSIX path without an explicit opt-in.
- Changed the workflow input to a blank string default and removed the `|| 'hybrid'` environment fallback. The existing validation step now fails omitted/blank values and only continues after the caller enters `hybrid`.
- Added `arm64_msix_smoke_requires_explicit_hybrid_runtime_profile` to keep the smoke workflow aligned with the first rs portable-only default.

Validation:

- `cd rs; cargo test -p easydict_packager --test release_contract_behavior arm64_msix_smoke_requires_explicit_hybrid_runtime_profile -- --exact --nocapture`

## 2026-06-12: Hardened rs portable validation for UTF-16 .NET host markers

- Rechecked reusable PE inspection crates before changing the validator. `goblin`, `object`, and `pelite` all provide PE/COFF parsing surfaces, but this slice only needs to catch known retained-runtime marker strings in allowlisted portable executables, so the existing byte scanner was extended instead of adding a parser dependency.
- `validate-rs-portable` now scans allowlisted executable bytes for both ASCII and UTF-16LE encodings of the retained .NET host/runtime markers, closing the rename-and-wide-string variant for apphost-style payloads inside `Easydict.Rust.exe` or Rust helper names.
- Added directory and ZIP regression coverage for an allowlisted `easydict_cli.exe` that contains a UTF-16LE `.NET` host marker.

Validation:

- `cd rs; cargo test -p easydict_packager validate_rs_portable_rejects_allowlisted_exe_that_contains_utf16le_dotnet_markers -- --nocapture`
- `cd rs; cargo test -p easydict_packager validate_rs_portable -- --nocapture`
- `cd rs; cargo check -p easydict_packager --all-targets`

## 2026-06-12: Locked Windows Native OCR language fallback order

- Rechecked OCR replacement options before changing code. No new dependency was added: Windows Native OCR stays on the official `windows` crate / WinRT `Windows.Media.Ocr` path, while Tesseract/leptess would add native engine, language-data, and packaging surface that does not fit the first rs portable target.
- `easydict_windows_ocr` now has a pure engine-attempt planner that trims a preferred language tag, ignores empty/`auto`, tries the preferred recognizer first, and always falls back to `TryCreateFromUserProfileLanguages()` afterward.
- Added crate-level coverage for the fallback order so preferred-language PDF/OCR hints cannot regress into a hard unsupported-language failure before user-profile fallback.

Validation:

- `cd rs; cargo test -p easydict_windows_ocr -- --nocapture`
- `cargo fmt --manifest-path rs/Cargo.toml --package easydict_windows_ocr --check`
- `cd rs; cargo check -p easydict_windows_ocr --all-targets`

## 2026-06-12: Revalidated Rust MDD support and restored MDX credential preflight parity

- Rechecked available MDict/MDD libraries before changing code. `mdict-rs` is the only newer Rust library-first candidate that advertises MDX/MDD support, but switching parser crates would add license/API and behavior risk for no immediate core gap; `readmdict` is MIT but is a Python-origin route, and `mdict-parser` still does not implement MDD. The existing MIT `lib/rs-mdict` fork remains the selected reusable library.
- Confirmed the current Rust route already covers the core MDD path: `Mdd` raw/base64 resource lookup, MIME inference, zlib/non-ASCII generated fixtures, cross-record-block reads, same-stem companion `.mdd` discovery, first-hit multi-MDD lookup, failed-MDD skipping, HTML data-URL inlining, and Quick Translate rich HTML separation.
- Fixed one adjacent MDX preflight regression found while validating the MDD suite: record-block encrypted dictionaries with present but invalid regcode/email now report the local Base64/key-length credential error before opening the native reader, while unsupported `Encrypted=3` / unknown modes still fail locally as unsupported before any CompatHost path.

Validation:

- `cargo test --manifest-path lib/rs-mdict/Cargo.toml -- --nocapture`
- `cd rs; cargo test -p easydict_app --test mdx_native_behavior -- --nocapture`
- `cd rs; cargo test -p easydict_app --test quick_translate_behavior mdd -- --nocapture`
- `cd rs; cargo test -p easydict_app --test settings_storage_behavior mdd -- --nocapture`

## 2026-06-12: Added Rust-native mouse selection gesture core and tightened LongDoc helper resolution

- Rechecked reusable hook libraries before changing code. `willhook` and `rdev` can wrap global input hooks, but this slice only migrates the pure MouseHook state machine, so no new dependency was added; later Win32 hook integration can stay on the existing `windows` crate boundary.
- Added `mouse_selection` with Rust-owned drag detection, multi-click detection, hook-message outcome planning, PopButton self-click dismissal guard, excluded-app suppression, and the `EASYDICT_SYNTHETIC_KEY` keyboard marker used to ignore synthetic Ctrl+C.
- Added `mouse_selection_behavior`, mirroring the .NET `MouseHookServiceTests` thresholds: 10px drag distance, 4px multi-click distance, double/triple-click timing, drag cancellation of pending multi-click, scroll/right/key dismiss semantics, and synthetic-key suppression.
- Hardened `scripts/translate-long-doc.ps1` helper resolution so every resolved helper path, including explicit `-RustHelperPath`, `-AppDir\easydict_long_doc.exe`, source target candidates, and PATH discovery, is rejected if it lives under retained `dotnet/`, `workers/`, `host/fxr/`, shared runtime, `Easydict.CompatHost*`, `Easydict.NativeBridge*`, or `Easydict.Workers.*` payload roots. A file renamed to `easydict_long_doc.exe` can no longer bypass the leaf-name guard.

Validation:

- `cd rs; cargo test -p easydict_app --test mouse_selection_behavior -- --nocapture`
- `cd rs; cargo test -p easydict_packager --test release_contract_behavior translate_long_doc_script_rejects_renamed_helper_under_retained_payload_root_before_spawn -- --exact --nocapture`
- `cd rs; cargo test -p easydict_packager --test release_contract_behavior translate_long_doc_script_rejects_retained_runtime_rust_helper_paths_before_spawn -- --exact --nocapture`
- `cd rs; cargo test -p easydict_packager --test release_contract_behavior translate_long_doc_script_resolves_app_dir_helper_without_cargo_or_dotnet_tools -- --exact --nocapture`
- `rustfmt --edition 2021 rs/crates/easydict_app/src/mouse_selection.rs rs/crates/easydict_app/tests/mouse_selection_behavior.rs rs/crates/easydict_app/src/lib.rs`

## 2026-06-12: Reconfirmed MDD rs support and tightened LongDoc/browser no-runtime boundaries

- No new dependency was needed. MDD remains on the existing MIT `lib/rs-mdict` fork, which is already the best-fit in-tree route for the required MDX/MDD semantics.
- Re-ran the focused MDD suites after the user-requested check. The current Rust path covers MDD parsing, raw/base64 resource lookup, cross-record-block reads, companion `.mdd` discovery, MIME inference, WebView-ready data-URL inlining, first-hit ordering across multiple MDD files, failed-MDD skipping, and Quick Translate rich HTML separation.
- Moved native LongDoc output/result JSON preflight ahead of PDF source reader/OCR fallback. If a PDF sidecar/output target is a directory, the run now fails locally before PDFium/source extraction, content-stream fallback, OCR, provider translation, or partial output writes.
- Hardened the browser registrar source guard so renamed `easydict-native-bridge.exe` candidates are rejected not only from `workers/` and `dotnet/`, but also from legacy `Easydict.CompatHost`, `Easydict.NativeBridge`, and `Easydict.Workers.*` payload directories, including canonical target checks.
- Tightened the browser bridge default wording: registrar usage/help now names `easydict_browser_registrar`, native bridge fatal stderr uses the `easydict-native-bridge` prefix, and the default boundary scan rejects legacy `BrowserHostRegistrar` / `Easydict NativeBridge` markers.
- Added LongDoc PowerShell shim evidence for the remaining native options: `-EnvFile`, `-Layout`, `-MaxConcurrency`, and `-Vision*` settings are forwarded to the Rust helper under a forced rust-only child environment without invoking cargo/dotnet tools.

Validation:

- `cargo test --manifest-path lib/rs-mdict/Cargo.toml mdd -- --nocapture`
- `cd rs; cargo test -p easydict_app --test mdx_native_behavior mdd -- --nocapture`
- `cd rs; cargo test -p easydict_app native_pdf_result_json_path_prechecked_before_source_reader_or_ocr --lib -- --nocapture`
- `cd rs; cargo test -p easydict_app --test long_document_behavior native_pdf_result_json_path_prechecked_before_provider_translation -- --exact --nocapture`
- `cd rs; cargo test -p easydict_app --test long_document_behavior native_pdf_both_output_prechecks_bilingual_text_path_before_writing_pdf_file -- --exact --nocapture`
- `cd rs; cargo test -p easydict_app --test browser_registrar_behavior install_rejects_renamed_bridge_sources_from_legacy_payload_roots -- --exact --nocapture`
- `cd rs; cargo test -p easydict_app --test browser_registrar_behavior browser_registrar_usage_names_rust_binary_not_legacy_alias -- --exact --nocapture`
- `cd rs; cargo test -p easydict_app --test native_bridge_behavior native_bridge_binary_signals_rs_ocr_event_not_legacy_dotnet_event -- --exact --nocapture`
- `cd rs; cargo test -p easydict_app --test native_bridge_behavior native_bridge_binary_handles_unknown_action_without_dotnet_host_or_event_signal -- --exact --nocapture`
- `cd rs; cargo test -p easydict_app --test default_api_boundary_behavior -- --nocapture`
- `cd rs; cargo test -p easydict_packager --test release_contract_behavior translate_long_doc_script_forwards_layout_vision_env_file_and_max_concurrency -- --exact --nocapture`
- `cd rs; cargo test -p easydict_app source_bridge_path_rejects_canonical_legacy_worker_or_dotnet_bridge_source --lib -- --nocapture`

## 2026-06-12: Tightened MDX/MDD credentials, discovery, and CLI no-runtime contracts

- No new dependency was needed. The MDX/MDD work stays on the existing local `lib/rs-mdict` fork and the app-owned credential/settings glue; the CLI guards reuse existing Rust-native route tests.
- Fixed record-block encrypted MDX routing so native lookup is allowed only when both credentials are present and the registration code passes local validation. Without credentials, the route now stays credential-gated instead of being treated as plain native lookup.
- Settings load now preserves saved MDD paths while appending newly discovered same-stem companion MDD files. This keeps user-added external MDDs and picks up later `Demo.1.mdd` / `Demo.4.mdd` resources without requiring manual settings edits.
- Added a CLI unsupported-language guard proving `easydict_cli translate --service google --to auto` fails during Rust language preflight before provider HTTP and without retained worker, CompatHost, or `.NET` wording.
- Strengthened LongDoc CLI contracts: `--list-services` includes canonical `windows-local-ai` without stale LocalAI aliases, and legacy `--app-dir` remains a no-op even when both runtime profile env vars are `hybrid`.

Validation:

- `cd rs; cargo test -p easydict_app --test mdx_native_behavior native_mdx_record_encrypted_dictionary_still_requires_credentials -- --exact --nocapture`
- `cd rs; cargo test -p easydict_app --test settings_storage_behavior settings_storage_load_merges_discovered_companion_mdds_with_saved_paths -- --exact --nocapture`
- `cd rs; cargo test -p easydict_app --test cli_translate_behavior native_google_cli_rejects_target_auto_before_provider_http_or_worker_lookup -- --exact --nocapture`
- `cd rs; cargo test -p easydict_app --test long_document_cli_behavior list_services_succeeds_without_document_arguments -- --exact --nocapture`
- `cd rs; cargo test -p easydict_app --test long_document_cli_behavior app_dir_ignores_stale_dotnet_payload_markers_and_does_not_enable_worker_lookup -- --exact --nocapture`
...
## 2026-06-12: Made Linguee a default Rust-native Quick Translate service

- No new dependency was needed. The existing Rust `traditional_http` request planner, Reqwest executor, Linguee parser, and local endpoint override already cover the service.
- Moved Linguee out of the core `enable-linguee-service` gate: the default service catalog now registers `linguee`, and `traditional_http_config_for_service("linguee", ...)` returns the native keyless Linguee config in default builds.
- Converted the Linguee catalog/native-route/CLI contracts from feature-only to default-build coverage, proving `easydict_cli translate --service linguee --json` can execute against a local HTTP server and stays free of retained worker, CompatHost, or `.NET` wording.
- The old feature remains available for UI/compatibility slices; this change only moves the core catalog and provider route onto the default Rust path.

Validation:

- `cd rs; cargo test -p easydict_app --test translation_services_behavior service_catalog -- --nocapture`
- `cd rs; cargo test -p easydict_app --test traditional_http_behavior traditional_http_config_routes_linguee_by_default -- --exact --nocapture`
- `cd rs; cargo test -p easydict_app --test quick_translate_behavior default_linguee_catalog_entry_routes_to_native_traditional_http -- --exact --nocapture`
- `cd rs; cargo test -p easydict_app --test cli_translate_behavior native_linguee_cli_succeeds_against_local_api_without_worker_or_compat_host_wording -- --exact --nocapture`
- `cargo fmt --manifest-path rs/Cargo.toml --package easydict_app --check`
- `cd rs; cargo check -p easydict_app --all-targets`
- `cd rs; cargo check -p easydict_app --all-targets --features enable-linguee-service`

## 2026-06-12: Added default DeepL Web CLI no-runtime contract

- No new dependency was needed. This reuses the existing DeepL Web JSON-RPC planner/parser and the debug-only traditional HTTP endpoint override pattern.
- Added `EASYDICT_TEST_TRADITIONAL_HTTP_ENDPOINT_DEEPL_WEB` for debug/test builds so the real CLI binary can exercise the default DeepL Web route against a local HTTP server without touching the live provider. Release builds keep the normal `https://www2.deepl.com/jsonrpc` endpoint.
- Added a CLI regression for `easydict_cli translate --service deepl --json` with an empty settings directory, proving the default mode is native DeepL Web JSON-RPC rather than the official API or any retained worker path.
- The test asserts the JSON-RPC method, text, source/target language fields, successful parsed output, and absence of retained worker, CompatHost, or `.NET` wording.

Validation:

- `cd rs; cargo test -p easydict_app --test cli_translate_behavior native_deepl_web_cli_uses_default_web_mode_without_worker_or_compat_host_wording -- --exact --nocapture`

## 2026-06-12: Promoted no-runtime smoke coverage into CI and release gates

- No new dependency was needed. This reuses the existing Rust behavior tests and release-contract checks.
- The rs portable release gate now runs retained-feature Quick Translate and LongDoc behavior tests, proving that even when `retained-dotnet-workers` is compiled and a worker policy is injected, retained workers still require explicit `hybrid` runtime opt-in.
- The default CI `rust-only-boundary` job now runs a real fake-cargo rs portable ZIP payload validation plus CLI/LongDoc stale `--app-dir` payload smokes, so PRs catch regressions before tag release.
- Added a CLI black-box regression for `easydict_cli translate` without `--service`; the default Google route hits the Rust-native HTTP path and does not emit retained runtime, worker, CompatHost, dotnet, PowerShell, pwsh, or hostfxr wording.
- The legacy `.NET` publish script still supports `-CreateZip` for explicit hybrid coexistence, but now names that archive `Easydict-legacy-hybrid-win-...zip` so it cannot be confused with the first rs portable ZIP.
...
- `cd rs; cargo test -p easydict_packager --test release_contract_behavior ci_workflow_runs_default_rs_rust_only_boundary_tests -- --exact --nocapture`
- `cd rs; cargo test -p easydict_packager --test release_contract_behavior pack_rs_portable_creates_and_validates_zip_without_retained_dotnet_payload -- --exact --nocapture`
- `cd rs; cargo test -p easydict_app --features retained-dotnet-workers --test quick_translate_behavior explicit_worker_policy_without_hybrid_runtime_profile_stays_rust_only -- --exact --nocapture`
- `cd rs; cargo test -p easydict_app --features retained-dotnet-workers --test long_document_behavior explicit_longdoc_worker_policy_without_hybrid_runtime_profile_stays_rust_only -- --exact --nocapture`
- `cd rs; cargo test -p easydict_app --test cli_translate_behavior default_translate_uses_native_google_without_retained_runtime_or_shell_wording -- --exact --nocapture`
- `cd rs; cargo test -p easydict_app --test cli_translate_behavior local_ai_cli_app_dir_ignores_stale_dotnet_payload_markers -- --exact --nocapture`
- `cd rs; cargo test -p easydict_app --test long_document_cli_behavior app_dir_ignores_stale_dotnet_payload_markers_and_does_not_enable_worker_lookup -- --exact --nocapture`
- `cd rs; cargo test -p easydict_packager --test release_contract_behavior legacy_publish_create_zip_is_hybrid_named_and_excluded_from_rs_release_contract -- --exact --nocapture`

## 2026-06-12: Hardened Rust-only helper and extension package ingress points

- No new dependency was needed. This closes two read-only subagent/runtime-boundary findings with local denylist and filesystem-entry checks.
- `package-browser-extension` now rejects symlink or Windows reparse-point source entries for manifest/common allowlisted files before creating the ZIP/XPI archive, so a linked `background.js`, icon, or locale file cannot smuggle a stale `.NET` payload into the browser extension package.
- `scripts/translate-long-doc.ps1 -RustHelperPath` now rejects explicit retained `.NET` runtime/worker/CompatHost command names (`dotnet.*`, `Easydict.Workers.*`, `Easydict.CompatHost*`) before spawn, while preserving fake/custom Rust helper names. `-UseCargo` remains cargo-first and ignores a retained `-RustHelperPath` value instead of resolving or spawning it.

Validation:

- `cd rs; cargo test -p easydict_packager package_browser_extension_rejects_linked_common_file_before_creating_package -- --nocapture`
- `cd rs; cargo test -p easydict_packager --test release_contract_behavior translate_long_doc_script_rejects_retained_runtime_rust_helper_paths_before_spawn -- --exact --nocapture`
- `cd rs; cargo test -p easydict_packager --test release_contract_behavior translate_long_doc_script_use_cargo_forwards_retry_sidecar_without_dotnet_tools -- --exact --nocapture`
- `cd rs; cargo test -p easydict_packager --test release_contract_behavior translate_long_doc_script_is_rust_only_and_rejects_dotnet_legacy_mode -- --exact --nocapture`

## 2026-06-12: Provisioned WindowsAI WinMD metadata for hybrid helper builds

- No new dependency was needed. Hybrid release and ARM64 MSIX smoke now use the same official `Microsoft.WindowsAppSDK.AI` NuGet metadata preparation step that the rs portable release already uses.
- `publish-msix` and `arm64-msix-smoke` export `EASYDICT_WINDOWS_APP_SDK_AI_METADATA_DIR` and `EASYDICT_WINDOWS_AI_REQUIRE_WINRT_BINDINGS=1` before `Build-RustHelpers.ps1`, so strict `lib/easydict-windows-ai` binding generation does not depend on runner cache state.
- Added release-contract coverage that checks both hybrid paths provision `Microsoft.Windows.AI.winmd`, `Microsoft.Windows.AI.Foundation.winmd`, and `Microsoft.Windows.AI.Text.winmd` before invoking Rust helper builds.

Validation:

- `cd rs; cargo test -p easydict_packager --test release_contract_behavior hybrid_msix_paths_provision_windows_ai_winmd_metadata_before_rust_helpers -- --exact --nocapture`

## 2026-06-12: Blocked Windows command shims in the Foundry Local CLI boundary

- No new dependency was needed. This tightens the existing `lib/easydict-foundry-local` denylist for the one default rs process boundary that may spawn an external native CLI.
- `EASYDICT_FOUNDRY_LOCAL_CLI` now rejects Windows shim forms such as `dotnet.cmd`, `dotnet.bat`, `powershell.cmd`, `powershell.bat`, `pwsh.cmd`, and `pwsh.bat`, alongside the already blocked `.exe` names and retained worker/runtime paths.
- Added CLI and LongDoc CLI regressions proving `dotnet.cmd` overrides are rejected before spawn and do not fall through to retained worker, CompatHost, or bundled `.NET` runtime lookup.

Validation:

- `cargo test --manifest-path lib/easydict-foundry-local/Cargo.toml cli_executable_override_rejects_retained_dotnet_runtime_commands -- --exact --nocapture`
- `cd rs; cargo test -p easydict_app --test cli_translate_behavior auto_local_ai_cli_rejects_foundry_cli_override_targeting_dotnet_cmd_before_spawn -- --exact --nocapture`
- `cd rs; cargo test -p easydict_app --test long_document_cli_behavior foundry_local_cli_override_targeting_dotnet_cmd_is_not_spawned -- --exact --nocapture`

## 2026-06-12: Moved hidden .NET payload checks earlier in Rust-only packaging

- No new dependency was needed. The MSIX prepare path now reuses the existing ASCII-insensitive `.NET` host/runtime content marker scan that final MSIX and bundle validation already use.
- `prepare-package-inputs` now scans regular publish-dir files in Rust-only mode before writing the prepared manifest. A renamed apphost or support file with `hostfxr.dll`, CoreLib, runtimeconfig/deps, CompatHost, or worker markers is rejected even when its path does not look like `dotnet/` or `workers/`.
- `publish-rs-portable` now runs the existing CLI and LongDoc `--app-dir` stale-payload regressions before building the ZIP, and `release_contract_behavior` locks those workflow commands so the release gate cannot silently drop them.

Validation:

- `cd rs; cargo test -p easydict_msix_validate prepare_package_inputs_ -- --nocapture`
- `cd rs; cargo test -p easydict_packager --test release_contract_behavior rs_portable_release_jobs_stay_isolated_from_dotnet_artifacts -- --exact --nocapture`
- `cd rs; cargo test -p easydict_app --test cli_translate_behavior local_ai_cli_app_dir_ignores_stale_dotnet_payload_markers -- --exact --nocapture`
- `cd rs; cargo test -p easydict_app --test long_document_cli_behavior app_dir_ignores_stale_dotnet_payload_markers_and_does_not_enable_worker_lookup -- --exact --nocapture`

## 2026-06-12: Added default CI and Cargo-bin guards for Rust-only entrypoints

- Two read-only subagent audits looked for the next non-UI runtime boundary gaps. No new dependency was needed; both fixes reuse existing Cargo manifest and release-contract checks.
- `.github/workflows/ci.yml` now has a `rust-only-boundary` job that forces `EASYDICT_RUNTIME_PROFILE=rust-only` and `RUNTIME_PROFILE=rust-only`, then runs the packager CI self-check, `easydict_app runtime_policy --lib`, and `default_api_boundary_behavior` without enabling retained worker features or setting up .NET.
- `easydict_app` now sets `autobins = false` and explicitly lists the app, CLI, LongDoc, NativeBridge, BrowserRegistrar, and LexIndex bins. The default API boundary test enumerates `src/bin/*.rs` so a future helper cannot become a default rs executable just by being added to the directory.

Validation:

- `cd rs; cargo test -p easydict_packager --test release_contract_behavior ci_workflow_runs_default_rs_rust_only_boundary_tests -- --exact --nocapture`
- `cd rs; cargo test -p easydict_app --test default_api_boundary_behavior default_app_manifest_disables_auto_discovered_binary_entrypoints -- --exact --nocapture`
- `cd rs; cargo test -p easydict_app --test default_api_boundary_behavior -- --nocapture`
- `cd rs; cargo test -p easydict_app runtime_policy --lib -- --nocapture`
- `cd rs; cargo check -p easydict_app --all-targets`

## 2026-06-12: Added gated real-provider smoke entries for native PDF layout

- No new dependency was needed. The smoke entries reuse the existing Rust `ort` DocLayout-YOLO/TATR wrappers and the current `ReqwestVisionLayoutHttpClient` Vision LLM executor.
- Added default-skipped, env-gated tests for the real provider boundaries: `EASYDICT_DOC_LAYOUT_YOLO_ONNX_SMOKE_RUNTIME_DIR` + `EASYDICT_DOC_LAYOUT_YOLO_ONNX_SMOKE_MODEL`, `EASYDICT_TATR_ONNX_SMOKE_RUNTIME_DIR` + `EASYDICT_TATR_ONNX_SMOKE_MODEL`, and `EASYDICT_VISION_LAYOUT_SMOKE_ENDPOINT` + `EASYDICT_VISION_LAYOUT_SMOKE_MODEL` + optional `EASYDICT_VISION_LAYOUT_SMOKE_API_KEY`.
- When the env vars are absent, the tests print a skip message and do not load models or call a network endpoint. When configured, they load/run the provider on synthetic BGRA document/table images and assert finite in-bounds detections or table structures.
- This closes the missing executable smoke entry for native PDF layout enrichment while leaving real-corpus parity gates and Settings UI button wiring as follow-up work.

Validation:

- `cd rs; cargo test -p easydict_app --test doc_layout_yolo_onnx_behavior doc_layout_yolo_onnx_real_provider_smoke_when_env_configured -- --nocapture`
- `cd rs; cargo test -p easydict_app --test table_structure_onnx_behavior tatr_onnx_real_provider_smoke_when_env_configured -- --nocapture`
- `cd rs; cargo test -p easydict_app --test vision_layout_behavior vision_layout_real_provider_smoke_when_env_configured -- --nocapture`
...
## 2026-06-12: Hardened explicit LongDoc PDF overlay font selection

- No new dependency was needed. The explicit `SettingsSnapshot.cjk_font_path` path now reuses the existing `ttf-parser` based managed CJK font validation from `font_download`.
- LongDoc PDF overlay still requires explicit CJK fonts to live under the configured/default Rust-managed `Fonts` cache and use a known NotoSans CJK asset filename, but now also rejects corrupt or non-CJK files even when the name and directory look correct.
- This aligns explicit font selection with automatic cache fallback, so a first rs portable run cannot be tricked into embedding an arbitrary or broken font file during native PDF overlay export.

Validation:

- `cd rs; cargo test -p easydict_app long_document::tests::native_pdf_overlay_font_rejects_invalid_explicit_managed_cjk_font_path --lib -- --nocapture`
- `cd rs; cargo test -p easydict_app native_pdf_overlay_font --lib -- --nocapture`

## 2026-06-12: Moved default Foundry runtime construction behind the lib boundary

- Rechecked the reusable provider first. The official Microsoft `foundry-local-sdk` adapter already lives under `lib/easydict-foundry-local` as an opt-in feature, so this slice did not add another SDK wrapper or dependency.
- `lib/easydict-foundry-local` now exposes `DefaultFoundryLocalRuntimeController` plus `default_foundry_local_runtime_controller()`. The default no-SDK build still uses the existing CLI-backed controller, while SDK-enabled builds keep SDK selection inside the lib.
- Quick Translate, streaming Quick Translate, LongDoc native LocalAI probing, CLI native probing, and the app Foundry prepare task now construct the default Foundry controller through the lib-owned factory instead of directly naming the CLI resolver.
- The app crate gained explicit opt-in `foundry-sdk` / `foundry-sdk-winml` feature plumbing to the lib, while `default = []` remains unchanged for the first rs portable package.

Validation:

- `cargo test --manifest-path lib/easydict-foundry-local/Cargo.toml default_runtime_controller_uses_cli_boundary_without_sdk_feature -- --nocapture`
- `cargo test --manifest-path lib/easydict-foundry-local/Cargo.toml --features sdk sdk_provider_stays_inside_lib_owned_runtime_and_model_traits -- --nocapture`
- `cd rs; cargo test -p easydict_app --test default_api_boundary_behavior default_app_uses_lib_owned_foundry_runtime_controller_factory -- --nocapture`
- `cd rs; cargo test -p easydict_app --test default_api_boundary_behavior -- --nocapture`
...
- `cd rs; cargo check -p easydict_app --all-targets --features foundry-sdk-winml`

## 2026-06-12: Provisioned official WindowsAI WinMD metadata for rs portable releases

- Rechecked the source before changing CI. The official `Microsoft.WindowsAppSDK.AI` NuGet package is the right upstream for Windows App SDK AI WinMD metadata; no Rust dependency was added.
- `publish-rs-portable` now downloads the latest stable package from NuGet flat container, extracts only the package contents on the runner, verifies `Microsoft.Windows.AI.winmd`, `Microsoft.Windows.AI.Foundation.winmd`, and `Microsoft.Windows.AI.Text.winmd`, then exports `EASYDICT_WINDOWS_APP_SDK_AI_METADATA_DIR` and `EASYDICT_WINDOWS_AI_REQUIRE_WINRT_BINDINGS=1` through `GITHUB_ENV`.
- The metadata step runs before release contract tests and before `Package-Portable.ps1`, so strict WindowsAI bindings fail in CI instead of silently shipping the unsupported fallback.

Validation:

- `cd rs; cargo test -p easydict_packager --test release_contract_behavior rs_portable_release_provisions_windows_ai_winmd_metadata_before_cargo -- --exact --nocapture`

## 2026-06-12: Hardened Rust-only archives against hidden .NET runtime payloads

- No new dependency was needed. This reuses `zip` crate archive metadata, existing ASCII-insensitive runtime marker scans, and the current packager/MSIX validators.
- Rust-only MSIX validation now scans every non-directory payload file for .NET host/runtime markers, not just `.exe` entries, and rejects archive symlink entries before payload allowlist checks. The same policy is applied to nested packages inside MSIX bundles.
- `prepare-package-inputs` now fails before writing the prepared manifest when a Rust-only publish directory contains any symlink or Windows reparse-point payload entry, including safe-looking `Assets/...` names.
- `validate-rs-portable` now rejects symlink entries inside the portable ZIP before required/allowed payload checks, so the first rs portable archive cannot hide retained `.NET` runtime files through links.
...
- `cd rs; cargo test -p easydict_packager validate_rs_portable --lib -- --nocapture`
- `cd rs; cargo check -p easydict_msix_validate --all-targets`
- `cd rs; cargo check -p easydict_packager --all-targets`

## 2026-06-12: Expanded Rust-only MSIX apphost marker scanning to all payload executables

- No new dependency was needed. Reused the existing marker-based renamed-apphost scan and made it ASCII case-insensitive while indexing package payloads.
- Rust-only MSIX and nested bundle validation now scan every payload `.exe`, not only required Rust helper names, so an extra `tools/*.exe` or `assets/*.exe` cannot smuggle a renamed .NET apphost into the first rs package path.
- Added ordinary MSIX and bundle regressions for non-helper payload executables with mixed-case .NET host/runtime markers.
...
## 2026-06-12: Added native LongDoc PDF backfill metrics to quality reports

- No new dependency was needed. The native PDF exporter already returns content-stream replacement and overlay/font-embedding summaries; the LongDoc result builder now maps those into the retained-worker-compatible `BackfillQualityMetrics` field names.
- PDF content-stream replacement now reports `candidateBlocks`, `renderedBlocks`, and `objectReplaceBlocks` in `qualityReport.backfillMetrics`; overlay/font embedding and content-stream no-match overlay fallback report `overlayModeBlocks`.
- The same `qualityReport` JSON string is returned in `TranslateDocumentResult` and persisted in the Rust-native result sidecar.

Validation:

- `cd rs; cargo test -p easydict_app --test long_document_behavior native_pdf_text_long_document_runner_extracts_pdf_and_writes_text_outputs -- --exact --nocapture`
- `cd rs; cargo test -p easydict_app --test long_document_behavior native_pdf_result_json -- --nocapture`
- `cd rs; cargo test -p easydict_app native_pdf_export_uses_overlay_font_embedding_for_cjk_translation --lib -- --nocapture`
- `cd rs; cargo test -p easydict_app native_pdf_export_overlay_mode_uses_overlay_without_content_stream_match --lib -- --nocapture`
- `cd rs; cargo test -p easydict_app native_pdf_content_stream_no_match_uses_overlay_before_text_fallback --lib -- --nocapture`
- `cd rs; cargo test -p easydict_app native_pdf_runner_source_extractor_error_falls_back_to_content_stream_and_writes_result_json --lib -- --nocapture`

## 2026-06-12: Added native LongDoc quality reports to result JSON sidecars

- No new dependency was needed. This reuses the existing Rust-native LongDoc checkpoint/source-chunk metadata and serializes the same high-level `LongDocumentQualityReport` shape used by retained worker results.
- Native Text/Markdown/simple-PDF runs now populate `TranslateDocumentResult.qualityReport` with a JSON string containing `stageTimingsMs`, `backfillMetrics`, `totalBlocks`, `translatedBlocks`, `skippedBlocks`, and `failedBlocks`.
- Retry Failed now rebuilds the report from the updated checkpoint before rewriting the sidecar, so a completed retry clears the failed-block diagnostics instead of preserving the initial partial report.

Validation:

- `cd rs; $env:CARGO_PROFILE_TEST_DEBUG='0'; $env:CARGO_INCREMENTAL='0'; $env:CARGO_BUILD_JOBS='2'; cargo test -p easydict_app --test long_document_behavior native_text_long_document_writes_result_json_sidecar -- --exact --nocapture`
- `cd rs; $env:CARGO_PROFILE_TEST_DEBUG='0'; $env:CARGO_INCREMENTAL='0'; $env:CARGO_BUILD_JOBS='2'; cargo test -p easydict_app --test long_document_behavior native_text_result_json_sidecar_persists_partial_checkpoint -- --exact --nocapture`
- `cd rs; $env:CARGO_PROFILE_TEST_DEBUG='0'; $env:CARGO_INCREMENTAL='0'; $env:CARGO_BUILD_JOBS='2'; cargo test -p easydict_app --test long_document_behavior native_text_result_json_retry_failed_retranslates_only_failed_chunks -- --exact --nocapture`
- `cd rs; $env:CARGO_PROFILE_TEST_DEBUG='0'; $env:CARGO_INCREMENTAL='0'; $env:CARGO_BUILD_JOBS='2'; cargo test -p easydict_app --test long_document_behavior retry_failed -- --nocapture`
- `cd rs; $env:CARGO_PROFILE_TEST_DEBUG='0'; $env:CARGO_INCREMENTAL='0'; $env:CARGO_BUILD_JOBS='2'; cargo test -p easydict_app --test long_document_behavior native_pdf_result_json -- --nocapture`
- `cargo fmt --manifest-path rs/Cargo.toml --package easydict_app --check`
- `cd rs; cargo check -p easydict_app --all-targets`
- `cd rs; cargo check -p easydict_app --all-targets --features retained-dotnet-workers`

## 2026-06-12: Ran runtime-policy unit tests in the rs portable release gate

- No new dependency was needed; this tightens the existing release workflow and Rust release-contract test.
- `publish-rs-portable` now runs default `runtime_policy --lib` and `--features retained-dotnet-workers runtime_policy --lib` before packaging. This proves the first rs portable path keeps retained `.NET` workers disabled by default, and that merely compiling the retained-worker feature still requires explicit hybrid runtime opt-in.
- `release_contract_behavior::rs_portable_release_jobs_stay_isolated_from_dotnet_artifacts` now asserts both runtime-policy commands remain in the Rust-only release verification step.

Validation:

- `cd rs; $env:CARGO_PROFILE_TEST_DEBUG='0'; $env:CARGO_INCREMENTAL='0'; $env:CARGO_BUILD_JOBS='2'; cargo test -p easydict_packager --test release_contract_behavior rs_portable_release_jobs_stay_isolated_from_dotnet_artifacts -- --exact --nocapture`
- `cd rs; $env:CARGO_PROFILE_TEST_DEBUG='0'; $env:CARGO_INCREMENTAL='0'; $env:CARGO_BUILD_JOBS='2'; cargo test -p easydict_app runtime_policy --lib -- --nocapture`
- `cd rs; $env:CARGO_PROFILE_TEST_DEBUG='0'; $env:CARGO_INCREMENTAL='0'; $env:CARGO_BUILD_JOBS='2'; cargo test -p easydict_app --features retained-dotnet-workers runtime_policy --lib -- --nocapture`
- `cargo fmt --manifest-path rs/Cargo.toml --package easydict_packager --check`
- `cd rs; cargo check -p easydict_packager --all-targets`

## 2026-06-12: Kept encrypted MDX Quick Translate on the Rust-native route

- Rechecked reusable-library options for the MDD/MDX slice before changing code. Existing `lib/rs-mdict` remains the best fit for this repo because it already owns the migrated MDD resource reader, MDX lookup, encryption primitives, and Quick Translate integration; no new dependency was added.
- `native_mdx_dictionary_can_route_natively` now treats `Encrypted=1` record-block dictionaries without credentials as Rust-native candidates when the header mode itself is supported. The actual lookup still returns a local credentials-required error before opening the reader.
- Invalid regcodes, missing encrypted files, and unsupported combined encryption modes still fail locally with the existing native input errors. The route split prevents default rs Quick Translate from falling back to CompatHost/.NET for the supported encrypted MDX mode while preserving the stricter bad-input matrix.

Validation:

- `cd rs; $env:CARGO_PROFILE_TEST_DEBUG='0'; $env:CARGO_INCREMENTAL='0'; $env:CARGO_BUILD_JOBS='2'; cargo test -p easydict_app --test quick_translate_behavior encrypted_mdx -- --nocapture`
- `cd rs; $env:CARGO_PROFILE_TEST_DEBUG='0'; $env:CARGO_INCREMENTAL='0'; $env:CARGO_BUILD_JOBS='2'; cargo test -p easydict_app --test quick_translate_behavior encrypted_local_dictionary_suggestions -- --nocapture`
- `cd rs; $env:CARGO_PROFILE_TEST_DEBUG='0'; $env:CARGO_INCREMENTAL='0'; $env:CARGO_BUILD_JOBS='2'; cargo test -p easydict_app --test mdx_native_behavior mdd -- --nocapture`
- `$env:CARGO_PROFILE_TEST_DEBUG='0'; $env:CARGO_INCREMENTAL='0'; $env:CARGO_BUILD_JOBS='2'; cargo test --manifest-path lib/rs-mdict/Cargo.toml mdd -- --nocapture`
- `cd rs; $env:CARGO_PROFILE_TEST_DEBUG='0'; $env:CARGO_INCREMENTAL='0'; $env:CARGO_BUILD_JOBS='2'; cargo test -p easydict_app --test quick_translate_behavior mdd -- --nocapture`
- `cargo fmt --manifest-path rs/Cargo.toml --package easydict_app --check`
- `cd rs; cargo check -p easydict_app --all-targets`
- `cd rs; cargo check -p easydict_app --all-targets --features retained-dotnet-workers`

## 2026-06-12: Made root README source builds and distribution copy rs-portable first

- No new dependency was needed; this is release/documentation contract hardening around the existing Rust packager and workflow.
- Root README/README_ZH now show `rs/scripts/Package-Portable.ps1` and `cargo run -p easydict_app --bin easydict_preview` as the default source-build path. The .NET/WinUI build commands are kept under a Legacy/Hybrid subsection.
- Tech Stack and Distribution sections now list Rust portable first and label .NET/WinUI, Store, and WinGet as legacy/hybrid paths.
- Updated the Rust release-contract tests and the stale `.NET` `WorkerPackagingTests` workflow assertion so no test still expects `runtime_profile || 'hybrid'`.

Validation:

- `cd rs; $env:CARGO_INCREMENTAL='0'; $env:CARGO_BUILD_JOBS='2'; cargo test -p easydict_packager --test release_contract_behavior root_readmes_build_from_source_default_to_rs_portable_before_legacy_dotnet -- --exact --nocapture`
- `cd rs; $env:CARGO_INCREMENTAL='0'; $env:CARGO_BUILD_JOBS='2'; cargo test -p easydict_packager --test release_contract_behavior root_readmes_tech_stack_and_distribution_keep_rs_portable_as_default -- --exact --nocapture`
- `cd rs; $env:CARGO_INCREMENTAL='0'; $env:CARGO_BUILD_JOBS='2'; cargo test -p easydict_packager --test release_contract_behavior root_readmes_mark_winget_as_legacy_hybrid_not_default_rs_install -- --exact --nocapture`
- `cd rs; cargo check -p easydict_packager --all-targets`
- `cd rs; cargo check -p easydict_packager --all-targets --features hybrid-dotnet-runtime-packaging`
- `dotnet test tests/Easydict.WinUI.Tests/Easydict.WinUI.Tests.csproj --filter "FullyQualifiedName~WorkerPackagingTests"` was retried after a 30s wait, but the project still fails to compile in unrelated long-document/layout tests (`DocLayoutYoloService`, `VisionLayoutDetectionService`, `DocumentOutputMode` ambiguity, and removed `LongDocumentTranslationService` test hooks) before `WorkerPackagingTests` can run.

## 2026-06-12: Enriched Quick Translate cache hits with native Youdao phonetics

- No new dependency was needed; this reuses the existing Rust-native Youdao phonetic enrichment path and in-memory phonetic cache.
- Quick Translate cache hits that translate into English and are missing target phonetics now route through a phonetic-only task before emitting `QuickTranslateServiceFinished`. The task does not start the translation provider, retained worker, or stream path.
- Non-phonetic cache hits still return a direct message, preserving the fast cache-hit behavior for normal repeated queries.
- Added `quick_translate_cache_hit_enriches_missing_english_phonetics_without_provider_task`, which seeds the native phonetic cache, hits the translation cache, executes the returned future, and verifies phonetics are merged while no provider cache miss is registered.

Validation:

- `cd rs; $env:CARGO_PROFILE_TEST_DEBUG='0'; $env:CARGO_INCREMENTAL='0'; $env:CARGO_BUILD_JOBS='2'; cargo test -p easydict_app --test quick_translate_behavior quick_translate_cache_hit_enriches_missing_english_phonetics_without_provider_task -- --exact --nocapture`
- `cd rs; $env:CARGO_PROFILE_TEST_DEBUG='0'; $env:CARGO_INCREMENTAL='0'; $env:CARGO_BUILD_JOBS='2'; cargo test -p easydict_app --test quick_translate_behavior quick_translate_cache -- --nocapture`
- `cd rs; $env:CARGO_PROFILE_TEST_DEBUG='0'; $env:CARGO_INCREMENTAL='0'; $env:CARGO_BUILD_JOBS='2'; cargo test -p easydict_app --test quick_translate_behavior phonetic_enrichment -- --nocapture`
- `cd rs; $env:CARGO_PROFILE_TEST_DEBUG='0'; $env:CARGO_INCREMENTAL='0'; $env:CARGO_BUILD_JOBS='2'; cargo test -p easydict_app --test translation_cache_behavior phonetic -- --nocapture`

## 2026-06-12: Required explicit hybrid runtime profile for release publishing

- No new dependency was needed; this tightens the existing release workflow and packager contract tests.
- Stable tag and default manual releases now document the rs portable ZIP as the default path. WinGet is described as hybrid-only, and `publish-msix` / `create-bundle` no longer silently default `runtime_profile` to `hybrid`.
- Removed the top-level README/README_ZH WinGet and Microsoft Store CTAs so the first install path users see is `easydict-rs-portable-*`, with WinGet retained only under the legacy/hybrid section.
- `release_contract_behavior` now checks the blank `runtime_profile` dispatch default, the absence of hidden hybrid fallback in hybrid jobs, and the README ordering/CTA boundary.

Validation:

- `cd rs; $env:CARGO_INCREMENTAL='0'; $env:CARGO_BUILD_JOBS='2'; cargo test -p easydict_packager --test release_contract_behavior release_workflow -- --nocapture`
- `cd rs; $env:CARGO_INCREMENTAL='0'; $env:CARGO_BUILD_JOBS='2'; cargo test -p easydict_packager --test release_contract_behavior root_readmes_mark_winget_as_legacy_hybrid_not_default_rs_install -- --exact --nocapture`
- `cd rs; $env:CARGO_INCREMENTAL='0'; $env:CARGO_BUILD_JOBS='2'; cargo test -p easydict_packager --test release_contract_behavior legacy_dotnet_packaging_paths_reject_rust_only_and_require_hybrid_profile -- --exact --nocapture`

## 2026-06-12: Added real MDX/MDD Quick Translate smoke coverage

- A read-only MDD subagent confirmed the core MDD rs path is implemented: `lib/rs-mdict` parses MDD resources, the app discovers companion MDD files, MDX lookup inlines resources into `rawHtml`, and the default rs/portable build does not rely on .NET runtime or CompatHost.
- Rechecked replacement options before touching the path. The local `lib/rs-mdict` fork is already the Rust implementation under migration, and previous scouting found no better licensed drop-in replacement for the current MDX/MDD behavior; no new dependency was added.
- Added `native_quick_translate_reads_real_mdx_and_inlines_real_mdd_resources`, which creates real minimal MDX and MDD fixtures, runs the default Quick Translate native route, and proves `translatedText` stays plain text while `rawHtml` carries an MDD `data:image/png;base64,...` URL without CompatHost/.NET wording.
- Added `native_mdd_html_inline_decodes_utf8_percent_encoded_resource_paths`, covering UTF-8 percent-encoded CJK MDD paths such as `%E6%A0%87%E5%BF%97.png`.

Validation:

- `cd rs; $env:CARGO_PROFILE_TEST_DEBUG='0'; $env:CARGO_INCREMENTAL='0'; $env:CARGO_BUILD_JOBS='2'; cargo test -p easydict_app --test quick_translate_behavior native_quick_translate_reads_real_mdx_and_inlines_real_mdd_resources -- --exact --nocapture`
- `cd rs; $env:CARGO_PROFILE_TEST_DEBUG='0'; $env:CARGO_INCREMENTAL='0'; $env:CARGO_BUILD_JOBS='2'; cargo test -p easydict_app --test mdx_native_behavior native_mdd_html_inline_decodes_utf8_percent_encoded_resource_paths -- --exact --nocapture`
- `cd rs; $env:CARGO_PROFILE_TEST_DEBUG='0'; $env:CARGO_INCREMENTAL='0'; $env:CARGO_BUILD_JOBS='2'; cargo test -p easydict_app --test mdx_native_behavior mdd -- --nocapture`
- `cd rs; $env:CARGO_PROFILE_TEST_DEBUG='0'; $env:CARGO_INCREMENTAL='0'; $env:CARGO_BUILD_JOBS='2'; cargo test -p easydict_app --test quick_translate_behavior mdd -- --nocapture`
...
- Added stale-payload regressions for both modes using an app dir that contains `workers/localai`, `Easydict.CompatHost.exe`, `dotnet/dotnet.exe`, `hostfxr.dll`, and CoreLib markers.
- Both tests prove `--app-dir` remains a legacy no-op for rs CLI routing: the commands fail before emitting stdout, require a Rust-native LocalAI route, and do not mention retained worker, CompatHost, dotnet, or hostfxr payloads.

Validation:

- `cd rs; cargo test -p easydict_app --test cli_translate_behavior local_ai_stream_cli_app_dir_no_longer_enables_retained_worker_fallback -- --exact --nocapture`
- `cd rs; cargo test -p easydict_app --test cli_translate_behavior local_ai_grammar_cli_app_dir_no_longer_enables_retained_worker_fallback -- --exact --nocapture`

## 2026-06-12: Covered LongDoc LocalAI native route matrix before worker fallback

- Parallel subagent review pointed at LongDoc LocalAI routing as the next core guard after MDD and runtime-profile work. A separate library audit did not find a higher-quality replacement dependency for this slice; the existing Rust-native WindowsAI, Foundry Local, OpenVINO/NLLB, and `rs-mdict` libraries remain the right ownership boundaries.
- Added `local_ai_long_document_route_matrix_stays_native_before_worker_fallback`, which runs LongDoc LocalAI with stale `workers/longdoc`, `workers/localai`, `Easydict.CompatHost.exe`, `dotnet/dotnet.exe`, `hostfxr.dll`, and CoreLib markers in `app_dir`.
- The matrix covers explicit WindowsAI, Auto with ready WindowsAI, Auto falling through to native Foundry probing, explicit FoundryLocal configured endpoint, and OpenVINO cache preflight. Each path either succeeds through an injected Rust-native client or fails on a native/preflight boundary without retained worker, CompatHost, or dotnet wording.

Validation:

- `cd rs; cargo test -p easydict_app --test long_document_behavior local_ai_long_document_route_matrix_stays_native_before_worker_fallback -- --exact --nocapture`

## 2026-06-12: Forced LongDoc PowerShell shim children to Rust-only profile

- A read-only subagent audit found no remaining executable dotnet/CompatHost/worker launch path in `scripts/translate-long-doc.ps1`; the remaining risk was inherited parent environment such as `EASYDICT_RUNTIME_PROFILE=hybrid`.
- `scripts/translate-long-doc.ps1` now wraps both packaged helper and `-UseCargo` invocations with temporary `EASYDICT_RUNTIME_PROFILE=rust-only` and `RUNTIME_PROFILE=rust-only`, then restores the caller environment in `finally`.
- The retired `-UseDotnetLegacy` compatibility switch is hidden from normal PowerShell help/completion while still failing locally before any helper starts. The rs README now documents only the Rust helper/Cargo development modes and clarifies that `-AppDir` only locates `easydict_long_doc.exe`.
- `release_contract_behavior` now runs fake helper/cargo paths with parent profiles set to `hybrid` and proves the child process receives `rust-only`.

Validation:

- `cd rs; cargo fmt -p easydict_packager -- --check`
- `cd rs; cargo test -p easydict_packager --test release_contract_behavior translate_long_doc -- --nocapture`
- PowerShell parser check for `scripts/translate-long-doc.ps1`
- `git diff --check -- scripts/translate-long-doc.ps1 rs/README.md migration-list.md refactor-progress.md rs/crates/easydict_packager/tests/release_contract_behavior.rs`

## 2026-06-12: Aligned Makefile MSIX validators with Rust-only default profile

- A read-only subagent audit found `validate-msix` already omitted `--runtime-profile` when `RUNTIME_PROFILE` was empty, while `msix-x64`, `msix-x86`, and `msix-arm64` still passed `--runtime-profile "$(RUNTIME_PROFILE)"` to the Rust validator.
- The three MSIX package targets now normalize `RUNTIME_PROFILE` and pass `--runtime-profile "$runtime_profile"` only when the caller provided a value. When it is empty, they call `easydict_msix_validate -- <msix> --allow-unsigned`, preserving the validator's Rust-only default.
- Extended `release_contract_behavior` so the Makefile contract covers the post-package validators as well as `validate-msix`.

Validation:

- `cd rs; cargo fmt -p easydict_packager -- --check`
- `cd rs; cargo test -p easydict_packager --test release_contract_behavior legacy_dotnet_packaging_paths_reject_rust_only_and_require_hybrid_profile -- --exact --nocapture`

## 2026-06-12: Hardened LongDoc CLI app-dir no-op against stale payloads

- Rechecked the LongDoc native output preflight first; the multi-output and result JSON sidecar failure boundary was already implemented and covered, so this slice focused on the still-useful CLI app-dir runtime guard.
- `app_dir_ignores_stale_dotnet_payload_markers_and_does_not_enable_worker_lookup` now builds an app dir containing stale `workers/longdoc`, `workers/localai`, `Easydict.CompatHost.exe`, `dotnet/dotnet.exe`, `hostfxr.dll`, and CoreLib markers. The default `easydict_long_doc --service windows-local-ai --app-dir` path still fails on the Rust-native LocalAI boundary without worker/CompatHost/dotnet wording.
...
- `cd rs; cargo fmt -p easydict_packager -- --check`
- `cd rs; cargo test -p easydict_packager --test release_contract_behavior sign_and_install -- --nocapture`
- `git diff --check -- dotnet/scripts/sign-and-install.ps1 rs/crates/easydict_packager/tests/release_contract_behavior.rs`

## 2026-06-12: Hardened Quick CLI app-dir no-op against stale LocalAI payloads

- Rechecked the Quick CLI LocalAI app-dir boundary before changing tests. No new dependency was needed; this is a default rs no-runtime guard.
- `local_ai_cli_app_dir_ignores_stale_dotnet_payload_markers` now builds an app dir containing stale `workers/localai`, `Easydict.CompatHost.exe`, `dotnet/dotnet.exe`, `hostfxr.dll`, and CoreLib markers, then proves `windows-local-ai --app-dir` still fails on the Rust-native-required boundary without worker/CompatHost/dotnet wording.
- Strengthened the shared LocalAI CLI no-worker assertion and the batch `--app-dir` path so batch mode is covered by the same stale payload guard.
...
- `cargo fmt --manifest-path rs/Cargo.toml --package easydict_app --check`
- `cd rs; cargo check -p easydict_app --test cli_translate_behavior`

## 2026-06-12: Restricted retained-worker hybrid opt-in to Easydict env

- Rechecked the retained runtime-profile boundary after a read-only subagent audit. No new dependency was needed; this is a small Rust policy change.
- `RuntimeRoutePolicy` now treats `EASYDICT_RUNTIME_PROFILE=hybrid` as the only environment opt-in that can enable retained workers when the compatibility feature is compiled. Generic `RUNTIME_PROFILE=hybrid` no longer enables retained workers by itself; `RUNTIME_PROFILE=rust-only` and unknown generic values still participate as kill switches.
- Updated the retained-feature runtime-policy test to cover `generic_hybrid_profile_does_not_enable_retained_workers_when_feature_is_compiled`.

Validation:

- `cd rs; cargo test -p easydict_app --lib runtime_policy -- --nocapture`
- `cd rs; cargo test -p easydict_app --lib --features retained-dotnet-workers runtime_policy -- --nocapture`

Note:

- An earlier broad `cargo test -p easydict_app runtime_policy` attempt compiled unrelated integration tests and failed with disk/PDB/memory limits (`no space on device`, `LNK1318`, `LNK1140`), so the focused `--lib` commands above were used for this policy slice.

## 2026-06-12: Aligned persistent MDX suggestions with MDict header normalization

- Rechecked the local dictionary index/header-normalization gap before changing code. No new dependency was needed; the app now reuses the existing `lib/rs-mdict` normalization helper instead of duplicating MDict strip/case rules.
- `LexIndex` keeps the compatible LXDX binary format while allowing callers to provide pre-normalized trie keys. `LocalDictionaryIndexService` records a header-aware manifest normalization id and normalizes queries with the matching `StripKey` / `KeyCaseSensitive` rules when the MDX header is readable.
- Added coverage for pre-normalized LexIndex entries and for persistent MDX indexes whose headers set `StripKey=Yes` or `KeyCaseSensitive=Yes`.
...
## 2026-06-12: Hardened OCR current-app-dir against stale worker markers

- Rechecked the OCR default route with a parallel worker. No new dependency was needed; this only strengthens the existing Rust-native OCR behavior tests.
- Updated `current_app_dir_runner_uses_default_windows_native_despite_hybrid_profile_and_stale_worker_markers` to create stale `workers/ocr`, `Easydict.CompatHost.exe`, and `dotnet` markers next to the current test executable while parent runtime-profile env vars say `hybrid`.
- The current-app-dir WindowsNative path still returns the local pixel-data error boundary and does not mention worker fallback, CompatHost, `.NET`, or dotnet.

Validation:

- `cd rs; cargo test -p easydict_app --test ocr_behavior current_app_dir_runner -- --nocapture`
- `cd rs; cargo test -p easydict_app --test ocr_behavior app_dir_runner_uses -- --nocapture`

## 2026-06-12: Covered default encrypted MDX suggestions through native index

- Rechecked encrypted MDX local-dictionary suggestion routing before changing tests. No new dependency was needed; this uses the existing Rust `rs-mdict` header parsing and native `LexIndex` suggestion path.
- Added `encrypted_local_dictionary_suggestions_with_valid_credentials_route_native_index_by_default`, proving valid `Encrypted=1` plus regcode/email routes through the default native persistent index path without the retained bridge feature.
- The test verifies the encrypted dictionary opens through the native reader factory, returns suggestions, writes the native index, and produces no bridge/worker error.

Validation:

- `cd rs; cargo test -p easydict_app --test quick_translate_behavior encrypted_local_dictionary_suggestions_with_valid_credentials_route_native_index_by_default -- --exact --nocapture`
- `cd rs; cargo test -p easydict_app --test quick_translate_behavior local_dictionary_suggestions -- --nocapture`
- `cargo fmt --manifest-path rs/Cargo.toml --package easydict_app --check`
- `cd rs; cargo check -p easydict_app --all-targets`

## 2026-06-12: Covered plain LocalAI env aliases for CLI native routes

- Rechecked Quick CLI and LongDoc CLI LocalAI environment parsing before changing tests. No new dependency was needed; both CLIs already normalize provider aliases through the shared Rust protocol helper.
- Added Quick CLI coverage for plain `LOCAL_AI_PROVIDER`, `FOUNDRY_LOCAL_ENDPOINT`, `FOUNDRY_LOCAL_MODEL`, `OPENVINO_DEVICE`, and `EASYDICT_CACHE_DIR` so portable/CI callers can enter native Foundry/OpenVINO routes without writing settings JSON or using retained worker hints.
- Added matching LongDoc CLI coverage for the same plain env alias shape. Both tests assert native provider preflight/error behavior and reject retained worker, CompatHost, and `.NET` wording.

Validation:

- `cd rs; cargo test -p easydict_app --test cli_translate_behavior local_ai_cli_ -- --nocapture`
- `cd rs; cargo test -p easydict_app --test long_document_cli_behavior env_ -- --nocapture`
- `cargo fmt --manifest-path rs/Cargo.toml --package easydict_app --check`
- `cd rs; cargo check -p easydict_app --all-targets`
...
- Added Quick Translate coverage for legacy LocalAI provider mode spellings (`windows_ai`, `windows-ai`, `phi_silica`, `foundry_local`, `foundry-local`, `local-ai`, `open_vino`, `open-vino`) so they normalize to Rust-native WindowsAI/Foundry/OpenVINO boundaries and never silently fall back to retained LocalAI workers.
- Added the same alias-matrix contract at the `easydict_long_doc` CLI env boundary. `EASYDICT_LOCAL_AI_PROVIDER` values now have coverage for WindowsAI/Phi aliases, Foundry aliases, and OpenVINO aliases routing into native preflight/native endpoint failures without retained LongDoc worker, CompatHost, or `.NET workers` wording.
- Added LongDoc native-runner coverage for explicit Foundry Local with an empty endpoint. A test-owned OpenAI-compatible translator now proves LongDoc starts the Foundry runtime, loads the configured model, resolves the endpoint, sends the native OpenAI-compatible request, writes the translated output, and does not surface retained worker wording.
- Added Markdown/PDF/result sidecar LongDoc preflight coverage. The native Markdown runner now has regression evidence that a conflicting bilingual output target is rejected before provider translation starts and before a monolingual `.md` file is written; PDF `resultJsonPath` conflicts and Retry Failed restored output conflicts are also rejected before provider translation.
- Added settings storage coverage for legacy `foundry-local` LocalAI service ids. The real `load_settings_file` path now proves Foundry-only legacy settings persist as `windows-local-ai` + `LocalAIProvider=FoundryLocal`, while conflicting OpenVINO+Foundry legacy ids merge to one `windows-local-ai` entry without guessing a provider.
- `easydict_msix_validate prepare-package-inputs` now defaults to Rust-only runtime policy and rejects stale retained `dotnet/` / `workers/` payloads before rewriting manifests or copying resources. Legacy `dotnet/scripts/Package-Msix.ps1` still works only after requiring explicit Hybrid and now passes `--runtime-profile hybrid` into the Rust helper.
- Release-contract coverage now checks that the legacy MSIX script cannot bypass the Rust-only default of `prepare-package-inputs`.
- Added a fake-`PATH` execution contract for the legacy PowerShell publish/MSIX/installer/runtime-extraction scripts. Empty or `rust-only` runtime profiles now have test coverage proving they fail before invoking `dotnet`, `cargo`, `winapp`, or `ISCC`.
- Added browser extension packaging coverage proving `easydict_packager package-browser-extension` stays on its positive file allowlist: stale `Easydict.NativeBridge.exe`, `Easydict.CompatHost.exe`, `BrowserHostRegistrar.exe`, retained workers, and bundled `.NET` runtime residue in the source directory are ignored instead of entering Chrome/Firefox packages.
- Hardened `validate-rs-portable` ZIP reads to reject unsafe raw archive entry names before payload allowlist checks. The new table covers `../...`, leading `/workers/...`, and Windows drive-prefix `C:/workers/...`, matching the MSIX-side unsafe archive guard.

Validation:

- `cd rs; cargo test -p easydict_app --test quick_translate_behavior local_ai_provider_aliases_route_to_native_boundaries_without_worker_bridge -- --nocapture`
- `cd rs; cargo test -p easydict_app --test quick_translate_behavior local_ai_route_decision -- --nocapture`
- `cd rs; cargo test -p easydict_app --test long_document_cli_behavior local_ai_provider_aliases_route_to_native_preflight_without_worker_lookup -- --nocapture`
...
- `cd rs; cargo test -p easydict_app --test long_document_behavior explicit_foundry_local_long_document_with_empty_endpoint_discovers_endpoint_natively_without_worker -- --nocapture`
- `cd rs; cargo test -p easydict_app --test long_document_behavior foundry -- --nocapture`
- `cd rs; cargo test -p easydict_app --test long_document_behavior native_markdown_both_output_prechecks_bilingual_path_before_provider_translation -- --nocapture`
- `cd rs; cargo test -p easydict_app --test long_document_behavior native_pdf_result_json_path_prechecked_before_provider_translation -- --nocapture`
- `cd rs; cargo test -p easydict_app --test long_document_behavior native_text_result_json_retry_failed_prechecks_restored_output_before_provider_translation -- --nocapture`
- `cd rs; cargo test -p easydict_app --test long_document_behavior output_precheck -- --nocapture`
- `cd rs; cargo test -p easydict_app --test settings_storage_behavior foundry_local -- --nocapture`
- `cd rs; cargo test -p easydict_app --test settings_storage_behavior legacy_local_ai -- --nocapture`
- `cd rs; cargo test -p easydict_app --test settings_migration_behavior foundry_local -- --nocapture`
- `cd rs; cargo test -p easydict_msix_validate prepare_package_inputs -- --nocapture`
- `cd rs; cargo test -p easydict_msix_validate rust_only -- --nocapture`
- `cd rs; cargo test -p easydict_packager --test release_contract_behavior legacy_dotnet_packaging_paths_reject_rust_only_and_require_hybrid_profile -- --nocapture`
- `cd rs; cargo test -p easydict_packager --test release_contract_behavior legacy_packaging_scripts_reject_non_hybrid_profiles_before_invoking_external_tools -- --nocapture`
- `cd rs; cargo test -p easydict_packager package_browser_extension_ignores_retained_native_host_and_runtime_residue -- --nocapture`
- `cd rs; cargo test -p easydict_packager package_browser_extension -- --nocapture`
- `cd rs; cargo test -p easydict_packager validate_rs_portable_rejects_unsafe_zip_entry_paths_before_payload_allowlist -- --nocapture`
- `cd rs; cargo test -p easydict_packager validate_rs_portable -- --nocapture`

## 2026-06-12: Hardened LongDoc retry CLI and Foundry SDK default boundaries

- Re-audited LongDoc CLI/resultJson retry and LocalAI/Foundry boundaries. No new third-party library was needed: the remaining gaps were defense-in-depth contracts over existing Rust sidecar, PowerShell shim, and Cargo feature behavior.
- Added a CLI regression proving `easydict_long_doc --retry-failed` can re-export a PDF OCR sidecar as TXT + bilingual TXT without writing a fake PDF and without mentioning retained LongDoc workers, CompatHost, or `.NET` workers.
- Added an execution-level PowerShell shim contract for `scripts/translate-long-doc.ps1 -UseCargo`: it now proves `-ResultJsonPath`, `-RetryFailed`, `-AppDir`, output mode, PDF export mode, and page range are forwarded to `cargo run -p easydict_app --bin easydict_long_doc -- ...`, while fake `dotnet.cmd` / stale app-dir runtime markers are not touched.
- Strengthened the default API boundary test so `foundry-local-sdk` remains optional and owned by `lib/easydict-foundry-local`; app/workspace Cargo manifests that depend on `easydict_foundry_local` must not enable `sdk` or `sdk-winml` features on the default rs path.

Validation:

- `cd rs; cargo test -p easydict_app --test long_document_cli_behavior retry_failed_pdf_ocr_sidecar_reexports_text_without_pdf_or_worker_lookup -- --nocapture`
- `cd rs; cargo test -p easydict_packager --test release_contract_behavior translate_long_doc_script_use_cargo_forwards_retry_sidecar_without_dotnet_tools -- --nocapture`
- `cd rs; cargo test -p easydict_app --test default_api_boundary_behavior default_foundry_local_sdk_features_remain_lib_only_and_not_enabled_by_app_manifests -- --nocapture`
- `cd rs; cargo test -p easydict_app --test default_api_boundary_behavior -- --nocapture`
- `cd rs; cargo test -p easydict_packager --test release_contract_behavior translate_long_doc_script -- --nocapture`

## 2026-06-12: Covered native PDF OCR fallback runner sidecars and default install docs

- Rechecked the LongDoc/PDF fallback chain before changing code. No new crate was needed: the native runner already has PDF source extraction, selectable-text fallback, OCR fallback, TXT/PDF export, and result JSON sidecar support.
- Added full runner coverage for the case where PDF source extraction returns empty, selectable-text fallback also returns empty, and OCR fallback succeeds. The native runner now proves it completes without retained workers, exports TXT + bilingual TXT instead of a fake PDF, writes `resultJsonPath`, and stores PDF/OCR metadata in the sidecar so retry restores the chunk as `PdfOcr`.
- Native PDF fallback-to-text exports now keep a `pdf` checkpoint in the sidecar, matching successful native PDF export sidecars and preserving PDF source metadata even when the final artifact is TXT.
- Updated the root English and Chinese README install flow so the default path points at `easydict-rs-portable-*` / `Easydict.Rust.exe`; WinGet is labeled as a legacy/hybrid retained .NET package path and protected by a release contract test.

Validation:

- `cd rs; cargo test -p easydict_app --lib native_pdf_runner_empty_source_and_text_fallback_uses_ocr_and_writes_result_json -- --nocapture`
- `cd rs; cargo test -p easydict_app --lib native_pdf_runner_source_extractor_error_falls_back_to_content_stream_and_writes_result_json -- --nocapture`
- `cd rs; cargo test -p easydict_app --lib native_pdf_ocr_chunks_export_as_text_output -- --nocapture`
- `cd rs; cargo test -p easydict_packager --test release_contract_behavior root_readmes_mark_winget_as_legacy_hybrid_not_default_rs_install -- --nocapture`

## 2026-06-11: Added runner-level evidence for native PDF source fallback sidecars

- Rechecked the LongDoc/PDF source fallback route before adding code. No new crate was needed: the runner already has Rust PDFium/source extraction, `pdf-extract`/`lopdf` selectable-text fallback, OCR fallback, native PDF export, and result JSON sidecar infrastructure.
- `run_native_text_long_document_request_inner(...)` now delegates source reading through a private injectable reader helper. Production still uses the normal source reader, while tests can force the PDF source extractor to fail without depending on machine-specific PDFium behavior.
- Added a full runner regression proving that when the PDF source extractor fails, native LongDoc falls back to content-stream/selectable text, translates, writes PDF + bilingual TXT output, writes a PDF checkpoint in `resultJsonPath`, and does not mention retained LongDoc workers.

Validation:

- `cd rs; cargo test -p easydict_app --lib native_pdf_runner_source_extractor_error_falls_back_to_content_stream_and_writes_result_json -- --nocapture`
- `cd rs; cargo test -p easydict_app --lib native_pdf_ -- --nocapture`
- `cd rs; cargo check -p easydict_app --lib`
- `cd rs; cargo check -p easydict_app --lib --features retained-dotnet-workers`

## 2026-06-11: Feature-gated the hybrid .NET runtime downloader out of the default packager

- Rechecked the packaging dependency surface before changing libraries. No new dependency was needed; this slice removes `reqwest` from the default `easydict_packager` build and keeps it only behind the explicit `hybrid-dotnet-runtime-packaging` feature.
- Default `easydict_packager` usage no longer lists `extract-dotnet-runtime`, and invoking that command in a default build fails locally with a feature-gate error. The `download_and_extract_dotnet_runtime(...)` API and HTTP downloader compile only when the hybrid feature is enabled.
- `dotnet/scripts/Extract-DotnetRuntime.ps1` now opts into `--features hybrid-dotnet-runtime-packaging` after requiring `-RuntimeProfile Hybrid`, so retained-worker coexistence packaging still has an explicit path while the first rs portable packager stays Rust-only by default.

Validation:

- `cd rs; cargo test -p easydict_packager -- --nocapture`
- `cd rs; cargo test -p easydict_packager --features hybrid-dotnet-runtime-packaging -- --nocapture`
- `cd rs; cargo tree -p easydict_packager --no-default-features` (verified no `reqwest`)
- `cd rs; cargo tree -p easydict_packager --features hybrid-dotnet-runtime-packaging` (verified `reqwest` is present only for the hybrid feature)

## 2026-06-11: Routed unmatched native PDF content-stream export through Rust overlay

- Rechecked the LongDoc/PDF replacement path before changing dependencies. No new library was needed: the native PDF route already combines `lopdf` content-stream replacement with the Rust `easydict_pdf_overlay` wrapper for font/overlay output.
- `try_export_native_pdf_document(...)` now treats `NoReplacements` from content-stream replacement like the existing font-embedding case: if translated PDF export blocks include usable geometry, Rust overlay PDF export is attempted before falling back to TXT output.
- This closes another retained-worker expectation from complex/simple-selectable PDFs: a selectable PDF whose source text cannot be patched in-place can still produce a native PDF when overlay metadata and a font are available, without probing the LongDoc worker or `.NET` runtime.

Validation:

- `cd rs; cargo test -p easydict_app --lib native_pdf_content_stream_no_match_uses_overlay_before_text_fallback -- --nocapture`
- `cd rs; cargo test -p easydict_app --lib native_pdf_export_ -- --nocapture`
- `cd rs; cargo test -p easydict_app --test pdf_native_export_behavior native_pdf_export_fails_when_no_text_operator_can_be_patched -- --nocapture`
...
- 2026-06-03: Wired Rust-only runtime profile into packaging and CI validation entry points.
  ...
  - Historical note: this slice originally left the Makefile at `RUNTIME_PROFILE ?= hybrid` while adding rust-only skip branches. The later 2026-06-11 runtime-profile cleanup supersedes that default: Makefile `RUNTIME_PROFILE` is now empty by default, and omitted WinUI `RuntimeProfile` resolves to `RustOnly`.
...
- 2026-06-08: Removed the legacy worker target from the default CLI parser API.
  ...
  - `CliOptions` no longer exposes parsed legacy host details, and the old worker-target enum was removed from the default public module surface. `--host`, `--host-arg`, and `--app-dir` are still accepted for old command lines, but parsed values are intentionally ignored.
...
- 2026-06-08: Tightened rs portable validation from blocklist-only to first-release allowlist.
  ...
  - `validate-rs-portable` still reports known retained `.NET` payload entries first for clear diagnostics, then requires the package to contain only the first-release portable files. The original allowlist also accepted the legacy `BrowserHostRegistrar.exe` alias; the later 2026-06-09 portable-alias cleanup removed that alias from first-release rs portable packages.
...
- 2026-06-08: Moved LongDoc native output validation ahead of provider translation.
  - No new library was needed. This closes a native-runner ordering gap in the existing Rust LongDoc export path.
  - `run_native_text_long_document_request_inner(...)` now preflights the Text/Markdown/PDF output targets immediately after source chunk extraction and before document-context prompts or chunk translation. Invalid monolingual or bilingual output paths now fail locally before any native provider/translator call can start.
  - Strengthened the text `Both` and PDF `Both` output-path regressions so they assert the injected translator is not called when the bilingual target is already a directory.
...
- 2026-06-08: Extended the default no-runtime API boundary to rs portable binary entrypoints.
  - No new library was needed. This is release-surface hardening after the Rust-native CLI/LongDoc/native-bridge/browser-registrar/lex-index entrypoints already existed.
  - `default_api_boundary_behavior` now scans all default app bin sources (`easydict_cli`, `easydict_long_doc`, `easydict_native_bridge`, `easydict_browser_registrar`, and `easydict_lex_index`) for retained `.NET` runtime/worker markers alongside the library modules. This closes a coverage gap where default APIs were guarded but packaged CLI entrypoints were not explicitly part of the source scan.
- 2026-06-08: Guarded the Store listings workflow against PowerShell YAML parser regressions.
  - No new dependency was needed. `easydict_store_listings` already owns listing YAML parsing with its Rust `serde-saphyr` path, plus validate/preview/summary rendering and msstore payload generation.
  - Filled a submit-payload parity gap by preserving `sortTitle`, `voiceTitle`, and `additionalLicenseTerms` alongside the existing title, short title, copyright, developed-by, and release-notes fields.
  - Added a Rust contract test that includes `.github/workflows/store-listings.yml` and `.winstore/scripts/Sync-StoreListings.ps1`, rejecting `powershell-yaml`, `ConvertFrom-Yaml`, and similar parser re-entry while proving both entrypoints still delegate to `easydict_store_listings`.
  - The same test keeps `MSStore.CLI` installation behind the submit-only workflow branch, so validate/preview/summary stay free of `.NET` tool installation.
- 2026-06-09: Persisted Rust settings cleanup for retired worker isolation keys and tightened Browser NativeBridge event evidence.
  - No new library was needed. This is a settings/storage and NativeBridge contract hardening slice over existing Rust modules.
  - `settings_migration` now keeps the retired runtime-only worker isolation keys in one list and removes `UseLongDocWorker`, `UseLocalAiWorker`, and `UseOcrWorker` regardless of stored value type.
  - `load_settings_file(...)` now persists the migrated JSON back to the same settings file when local migration changes it, so stale retained-worker keys and legacy LocalAI service ids do not remain on disk after normal rs settings load.
  - Added storage/migration regressions proving settings save never writes those keys and file load persists cleanup while preserving the `openvino-local-ai` -> `windows-local-ai` / `LocalAIProvider=OpenVINO` migration.
  - Strengthened `native_bridge_behavior` so the Rust bridge binary is locked to `OCR_TRANSLATE_EVENT_NAME == Local\EasydictRs-OcrTranslate` rather than the dotnet legacy event, and invalid length / incomplete Native Messaging frames produce no response and no OCR signal. The invalid/missing JSON compatibility fallback remains intentionally unchanged.
- 2026-06-09: Extended the default app entrypoint boundary beyond direct `.NET` runtime markers.
  - No new library was needed. This is a default-surface guard over existing Rust app entrypoints and source scans.
  - `default_api_boundary_behavior` now includes `activation.rs` and rejects PowerShell / `.ps1` / `pwsh` / `Compress-Archive`, `dotnet run` / `dotnet publish`, and old `Easydict.NativeBridge` / `Easydict.BrowserRegistrar` project markers on the default rs surface. This prevents rs portable activation/browser/CLI code from quietly reintroducing legacy script or .NET helper entrypoints even when it avoids direct `dotnet.exe` strings.
- 2026-06-09: Guarded release orchestration against retired `.NET` helper project regressions.
  - No new library was needed. This is release-contract hardening around the existing Rust `easydict_packager` helper build path.
  - Added `release_orchestration_uses_rust_helpers_not_retired_dotnet_helper_projects`, which proves `Build-RustHelpers.ps1` delegates to `cargo -p easydict_packager build-rust-helpers`.
  - The same packager contract scans the release workflow, arm64 MSIX smoke workflow, dotnet Makefile, legacy publish/MSIX scripts, and rs portable shim so they cannot reintroduce retired `Easydict.NativeBridge`, `Easydict.BrowserRegistrar`, or `Easydict.CompatHost` `.NET` project references.
  - Mirrored the same source-contract guard in `.NET` `WorkerPackagingTests`, so the existing WinUI configuration test lane also rejects retired helper project references without making the rs package depend on those projects.
- 2026-06-09: Kept hybrid packaging/tooling crates out of the default rs app dependency graph.
  - No new library was needed. This is a manifest-level runtime boundary guard rather than a replacement dependency.
  - Extended `default_api_boundary_behavior` so `easydict_app` and `easydict_preview_iced` cannot depend on hybrid/tooling crates such as `easydict_packager`, `easydict_msix_validate`, Store listing tooling, icon generation, or UI parity analysis.
  - The same test rejects manifest mentions of hybrid runtime/packaging markers such as `extract-dotnet-runtime`, `Package-Msix`, `Build-RustHelpers`, and `Compress-Archive`, keeping `.NET` runtime extraction and release-only tooling out of the first rs portable app binary.
- 2026-06-09: Retired the root LongDoc script's explicit `.NET` debug path.
  - No new library was needed. The script already has a Rust helper and Cargo development mode; this slice removes the remaining opt-in WinUI `dotnet run` fallback.
  - `scripts/translate-long-doc.ps1 -UseDotnetLegacy` now fails locally with a retirement message and points callers at `-UseCargo`, `-RustHelperPath`, or `-AppDir` for the Rust `easydict_long_doc.exe` route.
  - Removed the script's legacy argument builder and `dotnet` launcher, and added a packager release-contract regression that rejects `Invoke-DotnetLegacy`, `Easydict.WinUI.csproj`, `dotnet\src\Easydict.WinUI`, and `--translate-long-doc` re-entry.
- 2026-06-09: Hardened MSIX and bundle payload validation against unsafe archive paths.
  - No new dependency was needed. The Rust validator reuses the existing `zip` crate entry metadata (`enclosed_name`) plus explicit component checks for leading roots, `..`, and Windows drive prefixes.
  - `archive_payload_index(...)` now rejects unsafe MSIX entry paths before retained worker / `.NET` runtime payload allowlist checks, so `../workers/...`, `/workers/...`, and `C:/workers/...` cannot bypass Rust-only validation by normalizing differently.
  - `verify_bundle_min_version(...)` now rejects unsafe bundle payload paths and unsafe entries inside nested appx/msix packages. It also accepts an optional runtime profile, and release workflow bundle validation passes `${{ env.RUNTIME_PROFILE }}` so nested packages reuse the same Rust-only/hybrid payload layout policy.
  - Tightened the explicit `hybrid` profile from "required files exist" to a scoped allowlist for the current retained-worker layout. x64/arm64 coexistence packages may keep `workers/longdoc`, `workers/localai`, `dotnet/host/fxr`, and `dotnet/shared/Microsoft.NETCore.App`; `workers/shared` is file-allowlisted to the deduped WinUI/WinRT projection DLLs. Stale roots such as `workers/legacy`, `dotnet/sdk`, `Microsoft.WindowsDesktop.App`, and `Microsoft.AspNetCore.App` now fail validation. Runtime engines or nested runtime layouts inside worker directories, including `coreclr.dll`, `hostfxr.dll`, `System.Private.CoreLib.dll`, `workers/*/host/fxr`, and `workers/*/shared/Microsoft.NETCore.App`, now fail while framework-dependent worker `.runtimeconfig.json` / `.deps.json` metadata remains allowed.
  - `BundleMinVersionOptions::default()` now uses `Some(PackageRuntimeProfile::RustOnly)`, so direct library calls and CLI invocations that omit `--runtime-profile` validate nested appx/msix payload layout with the first rs portable no-runtime policy instead of only checking MinVersion.
  - `create-bundle` now defines its own `RUNTIME_PROFILE` before calling `verify-bundle-minversion --runtime-profile`, so nested package layout validation does not depend on another job's environment.
  - Added regressions for rust-only MSIX unsafe entries, unsafe bundle package paths, unsafe nested package entries, rust-only nested retained runtime payloads, hybrid nested package runtime payload requirements, hybrid app/worker metadata allowance, hybrid worker runtime-engine/stale-root rejection, Rust-only bundle defaults, and the bundle job runtime-profile handoff.
- 2026-06-09: Removed the legacy browser registrar alias from first rs portable packages.
  - No third-party library was needed. This is a package-layout cleanup that separates the Rust portable payload from the retained dotnet/hybrid compatibility helper name.
  - `pack-rs-portable` now stages only `Easydict.Rust.exe`, the Rust-native helper executables, and `README-portable.txt`. It no longer copies `easydict_browser_registrar.exe` to `BrowserHostRegistrar.exe` for the first rs portable ZIP.
  - `validate-rs-portable` now treats `BrowserHostRegistrar.exe` as an unexpected first-release portable entry. The later 2026-06-13 helper-build cleanup made that legacy alias an explicit dotnet/hybrid opt-in rather than a generic helper-build side effect.
  - Updated the rs README and migration checklist to keep the alias out of the first portable allowlist while preserving it as an explicit hybrid compatibility artifact.
- 2026-06-09: Neutralized the default rs runtime policy module name.
  - No third-party library was needed. The default app still needs a small local policy object to short-circuit routes that require Rust-native LocalAI/LongDoc support.
  - Moved the default policy implementation from `retained_workers.rs` to `runtime_policy.rs` and renamed the default type to `RuntimeRoutePolicy`. Quick Translate and LongDoc now call `RuntimeRoutePolicy::all_disabled()` in default routes, so the first rs portable compile surface no longer depends on a default `retained_workers` module.
  - `RetainedWorkerPolicy`, `LOCAL_AI_WORKER_DISABLED_MESSAGE`, and `LONGDOC_WORKER_DISABLED_MESSAGE` remain as `retained-dotnet-workers` feature-only compatibility aliases for `compat_client` and retained-worker audit tests.
  - `default_api_boundary_behavior` now scans `runtime_policy.rs` and asserts the crate root does not reintroduce `mod retained_workers;`, while preserving the existing feature-gated crate-root export checks for retained compatibility APIs.
- 2026-06-09: Locked down the default rs process-spawn allowlist.
  - No third-party library was needed. The only default external process boundary should remain the Rust-native Foundry Local CLI wrapper in `lib/easydict-foundry-local`.
  - Added a dynamic `default_api_boundary_behavior` scan over `easydict_app/src/**/*.rs` that rejects direct `std::process::Command`, `Command::new(...)`, and `WorkerCommand::new(...)` usage on the default app surface. `compat_client` and `compat_protocol` are allowed only because the crate root keeps them behind `retained-dotnet-workers`.
  - The same regression asserts the Foundry Local wrapper only spawns `Command::new(&self.executable_name)`, keeps the `EASYDICT_FOUNDRY_LOCAL_CLI` override explicit, and still rejects retained `.NET` runtime/worker markers in that library.
- 2026-06-09: Guarded the Foundry Local CLI override against retained runtime commands.
  - No third-party library was needed; `lib/easydict-foundry-local` already owns the Foundry Local boundary and keeps the optional official `foundry-local-sdk` feature separate from the default CLI route.
  - `CommandFoundryLocalEndpointResolver` now validates the configured executable name before any spawn. Blank values fall back to `foundry`, while `dotnet.exe`, PowerShell/`.ps1`, `hostfxr.dll`, `Easydict.CompatHost*`, `Easydict.Workers.*`, `.runtimeconfig.json`, and bundled `host/fxr` style paths fail locally with a native-CLI-required error.
  - Added Foundry lib regressions for accepted native CLI names and rejected retained runtime/worker/script overrides, tightened the default API boundary test so these forbidden strings are allowed only inside the Foundry CLI denylist, and added Quick/LongDoc CLI regressions proving a fake `Easydict.Workers.LocalAi.exe.cmd` override is not spawned.
- 2026-06-09: Added release-job isolation coverage for first rs portable assets.
  - No new library was needed. This is GitHub Actions release-policy hardening on the existing Rust `easydict_packager` path.
  - Added `rs_portable_release_jobs_stay_isolated_from_dotnet_artifacts`, which section-scans `publish-rs-portable` and `create-rs-portable-release` instead of only checking whole-workflow markers.
  - The test proves the package job runs from the Rust workspace, calls `Package-Portable.ps1`, validates with `validate-rs-portable`, and uses the separate `easydict-rs-portable-*` artifact name.
  - The release job must depend only on `prepare` and `publish-rs-portable`, download only rs portable artifacts, and upload only `rs-portable/*.zip`; both jobs reject dotnet setup, WinApp/MSIX, installer, `.NET` runtime extraction, retained worker, CompatHost, and WinGet markers.
- 2026-06-09: Made the default rs runtime-marker boundary dynamic.
  - No new library was needed. This is a source-contract hardening step over the existing default app boundary test.
  - A read-only subagent found no current default rs path that triggers `.NET` runtime, retained workers, CompatHost, `dotnet.exe`, or PowerShell helpers.
  - `default_process_spawn_surface_has_no_retained_dotnet_runtime_entries` now walks `easydict_app/src/**/*.rs` instead of a fixed file list, skips only feature-gated retained worker modules, and rejects retained runtime/helper markers on every default production source line.
  - This closes the regression gap where a newly added default module could avoid the older hand-maintained marker list while still reintroducing `CompatHost`, `Easydict.Workers`, `dotnet.exe`, `DOTNET_ROOT`, `host/fxr`, PowerShell, or retired helper project markers.
- 2026-06-09: Closed the feature-on Linguee Quick Translate native-route evidence gap.
  - No new library was needed. The existing Rust `traditional_http` module already owns the Linguee request plan and response parser.
  - A read-only subagent ranked Linguee as the smallest core Quick Translate closure item because the HTTP plan/parser existed but the service catalog remains behind `enable-linguee-service`.
  - Added `feature_enabled_linguee_catalog_entry_routes_to_native_traditional_http`, which starts from `find_translation_service_descriptor("linguee")`, proves the request is native-routable, and executes the native traditional HTTP backend with a recording client.
  - The test checks the Linguee service id/name, parsed translation result, and GET request plan against the legacy proxy endpoint and `src`/`dst` language codes, without enabling retained workers or CompatHost.
- 2026-06-09: Added cooperative cancellation coverage to the native LongDoc runner.
  - No new library was needed. This is parity hardening inside the existing Rust-native Text/Markdown/simple-PDF LongDoc batching path.
  - Added a hidden injected-cancellation native runner used by behavior tests while keeping the normal production runner as an always-not-cancelled wrapper.
  - The native runner now checks cancellation before context extraction, before and during chunk scheduling, before each batch translation, and before export.
  - Added a regression proving cancellation after the first chunk stops later chunk scheduling, does not emit the `Exporting` stage, and leaves no partial output file behind.
- 2026-06-09: Closed the Foundry Local SDK adapter boundary inside `lib/easydict-foundry-local`.
  - Rechecked the locked local `foundry-local-sdk 1.2.1` API and the existing lib feature split. No app-crate SDK wrapper was added.
  - `FoundryLocalSdkProvider` now implements the lib-owned `FoundryLocalRuntimeController` / `FoundryLocalEndpointResolver` traits as well as `FoundryLocalSdkModelProvider`, so the existing Foundry prepare/status flow can be reused by an SDK-backed runtime without exposing SDK types in `easydict_app`.
  - Added feature-gated compile coverage proving the real SDK provider satisfies both trait boundaries, and default app boundary coverage proving the app manifest does not enable `sdk` / `sdk-winml` and app source does not reference `foundry_local_sdk` / `FoundryLocalSdkProvider`.
- 2026-06-09: Closed the default Bing Quick Translate catalog-to-native evidence gap.
  - No new library was needed. The Rust-native two-phase Bing backend and request parser already exist.
  - Added a regression that starts from `find_translation_service_descriptor("bing")`, proves the default catalog entry is native-routable, and runs the native Bing backend with a recording two-phase client.
  - The test checks the default `www.bing.com` host, `ttranslatev3` request plan, translated text, service id/name, and detected source language without retained worker or CompatHost involvement.
  - Added a Bing-specific unsupported-language regression proving invalid language pairs fail locally before fetching the translator session page or building a translate request.
- 2026-06-09: Added native LongDoc OpenVINO/NLLB success-path evidence.
  - Reused the existing `lib/easydict-nllb` tokenizer/engine traits and `NativeOpenVinoQuickTranslateBackend`; no new inference dependency was added.
  - Added an injected LongDoc translator test that runs a `windows-local-ai` / OpenVINO text document request through a fake NLLB tokenizer/engine, verifies quick-translate request mapping, forced BOS token usage, and translated output file contents.
  - This closes the evidence gap between the already-tested Quick Translate OpenVINO backend and LongDoc's native chunk runner.
- 2026-06-09: Added runtime env execution evidence for `build-rust-helpers`.
  - Evaluated `assert_cmd`/`assert_fs` as possible CLI-test helpers, but kept this slice on `std` because a tiny fake `cargo`/`rustup` executable gives a narrower contract test without adding dependencies.
  - Added `build_rust_helpers_child_cargo_is_forced_to_rust_only_runtime_profile`, which runs `build_rust_helpers` with fake child tooling and proves inherited `hybrid` profile envs are overridden to `EASYDICT_RUNTIME_PROFILE=rust-only` / `RUNTIME_PROFILE=rust-only` before helper builds.
  - The same test verifies the helper build command line and copied helper outputs, tightening the first rs portable no-runtime boundary.
- 2026-06-09: Hardened the native Bing session parser against translator-page drift.
  - Rechecked reusable HTML parser options first: `scraper`/`html5ever` are useful for DOM/selector parsing, and `lol_html` is strong for streaming rewrite, but Bing's fragile fields are mostly JavaScript values inside script text. No new dependency was added; the existing local scanner was a better fit for JS string literal handling and default rs no-worker behavior.
  - `parse_bing_credentials_from_html` now accepts bare or quoted `IG` properties with single/double quoted values and JS escapes, whitespace/single-quote/unquoted `data-iid` attributes, and single-quote/escaped `params_AbusePreventionHelper` tokens.
  - The abuse-prevention parser now scans all marker occurrences, skips identifier-boundary noise and malformed arrays, and accepts boot-data property arrays such as `"params_AbusePreventionHelper":[key,"token",expiry]`.
  - Added `traditional_http_behavior` regressions for minified/single-quote/escaped script values, noise before valid script fields, invalid marker candidates before a valid array, and boot-data property credentials.
- 2026-06-09: Added real multi-MDD first-hit evidence for native HTML inlining.
  - Reused the existing MIT `lib/rs-mdict` fork and test fixture writer; no new parser dependency was needed.
  - Added a native MDX lookup test with a mock MDX definition and two real minimal `.mdd` files containing the same image key. The result proves app-level HTML inlining goes through `RsMdictMddReaderFactory` and uses the first MDD hit instead of a later duplicate.
- 2026-06-09: Reconfirmed MDD core support and replaced remaining small hand-written base64 helpers.
  - Re-ran the focused MDD suites after the user-requested MDD check: `lib/rs-mdict` MDD tests, app `mdx_native_behavior mdd`, and Quick Translate `mdd` all pass. The current Rust route covers MDD parsing, raw resource lookup, cross-record-block reads, companion discovery, WebView-ready data-URL inlining, and first-hit behavior across multiple MDD files.
  - Reused the existing MIT `lib/rs-mdict` fork for MDD and the already-present `base64` crate for credential/OCR payload encoding; no new dependency was added. For credential decode, the `base64` crate engine is configured to require standard padding while allowing historical trailing bits that the old hand-written decoder accepted.
  - Added credential tests for malformed payloads, missing padding rejection, and legacy trailing-bit compatibility; tightened OCR tests to decode BMP/JPEG data URL payloads with the standard padded engine and assert concrete image markers.
- 2026-06-09: Filled the remaining core MDD gaps found by the focused audit.
  - Settings load now auto-discovers companion `.mdd` / numbered `.MDD` files for imported MDX dictionaries whose legacy settings did not persist `MddFilePaths`, matching the .NET-era behavior without touching UI code.
  - Native MDX lookup now returns an `mddResourcesInlined` signal only when MDD resource rewriting actually changed entry HTML. Quick Translate uses that signal instead of merely checking whether the settings contain MDD paths, so missing/unopenable MDD files no longer cause misleading `rawHtml` retention.
  - Added regression coverage for legacy settings MDD discovery, all-MDD-open-failure no-inline signaling, and Quick Translate dropping rich HTML when no MDD resource was actually inlined.
- 2026-06-09: Strengthened default MDD fixture coverage for compressed/non-ASCII resources.
  - No new parser dependency was added. The test reuses `flate2`, which `lib/rs-mdict` already uses for MDict zlib block handling, and keeps the existing MIT `lib/rs-mdict` route.
  - Added a generated MDD fixture with a zlib-compressed record block, non-ASCII image/audio/CSS resource keys, and larger binary payloads. The test verifies raw lookup, prefix lookup, MIME metadata, and standard-base64 `locate(...)` output.
  - This does not replace real-corpus testing, but it closes a default-test blind spot that previously required local `RS_MDICT_TEST_MDD` fixtures to exercise compressed resource blocks and Unicode resource names.
- 2026-06-09: Replaced the remaining hand-written `lib/rs-mdict` RIPEMD-128 implementation with RustCrypto.
  - Rechecked the reusable hash implementation first: RustCrypto `ripemd` is a pure-Rust RIPEMD crate, exposes `Ripemd128`, documents the crate as MIT OR Apache-2.0 licensed, and already fits the RustCrypto stack used elsewhere in the app (`https://docs.rs/ripemd/latest/ripemd/`).
  - `lib/rs-mdict` now depends on `ripemd = "0.2"` and keeps the old public `ripemd128(&[u8]) -> [u8; 16]` compatibility wrapper, so MDX/MDD decryption callers do not change while the local hash code is no longer maintained by hand.
  - Retained the existing RIPEMD-128 vector tests and re-ran encrypted MDX behavior tests that exercise credential-derived dictionary lookup.
- 2026-06-11: Made legacy Quick CLI host/app-dir hints unconditional no-op compatibility inputs.
  - No library research was needed. This is default rs runtime-surface cleanup after the retained LocalAI worker route had already been removed from the default CLI.
  - `easydict_cli` no longer rejects command lines that pass both legacy `--host` and `--app-dir`; both values, plus `--host-arg`, are parsed only to keep old scripts from failing and are ignored before route selection.
  - Added CLI behavior coverage proving a `windows-local-ai` request with both legacy hints still fails locally with Rust-native-route wording in a hybrid environment and does not probe retained LocalAI worker paths or mention CompatHost.
- 2026-06-11: Added fake-cargo execution evidence for the first rs portable build path.
  - No new dependency was needed; this extends the existing std-based fake `cargo`/`rustup` release-contract test harness.
  - Added `pack_rs_portable_child_cargo_is_forced_to_rust_only_runtime_profile`, which runs `pack_rs_portable(...)` with parent env set to `hybrid`, records the preview-app and helper child cargo invocations, and proves both receive `EASYDICT_RUNTIME_PROFILE=rust-only` / `RUNTIME_PROFILE=rust-only`.
  - The same regression asserts the child cargo args do not enable `retained-dotnet-workers`, `--features`, or `--all-features`, and that the staged first-release payload contains only the six allowed Rust portable entries.
- 2026-06-11: Added Rust-only boundary tests to the rs portable release job.
  - No new dependency was needed. This strengthens release orchestration by running existing Rust contract tests before the portable ZIP is built.
  - `publish-rs-portable` now runs `easydict_packager --test release_contract_behavior`, `easydict_app --test default_api_boundary_behavior`, and default `protocol_behavior` before `Package-Portable.ps1`.
  - Extended `release_contract_behavior` so the release workflow cannot drop those pre-package Rust-only boundary tests while still keeping the job isolated from setup-dotnet, MSIX, installer, WinGet, retained worker, and CompatHost markers.
- 2026-06-11: Removed the silent Hybrid default from dotnet runtime-profile entry points.
  - No library research was needed; this is release/runtime policy cleanup around the existing Rust packager and MSBuild profile gates.
  - `dotnet/Makefile` no longer defaults `RUNTIME_PROFILE` to `hybrid`. Legacy dotnet portable/MSIX paths must opt in with `RUNTIME_PROFILE=hybrid`, while ordinary build/test commands no longer silently select a retained-worker/runtime-producing profile.
  - `Easydict.WinUI.csproj` now treats an omitted `RuntimeProfile` as `RustOnly`, which disables retained worker ProjectReferences/copy and the in-proc LongDoc fallback by default. Explicit `RuntimeProfile=Hybrid` still preserves the coexistence path.
  - Rust `release_contract_behavior` and `.NET` `WorkerPackagingTests` text guards were updated so future changes cannot reintroduce `RUNTIME_PROFILE ?= hybrid` or a csproj empty-profile Hybrid default.
  - MSBuild property checks now show the empty profile as `RuntimeProfile=RustOnly`, `BuildWorkerOutputs=false`, and `EnableInProcLongDocFallback=false`; explicit `RuntimeProfile=Hybrid` still reports `RuntimeProfile=Hybrid` and `EnableInProcLongDocFallback=true`.
  - Attempted the targeted `.NET` `WorkerPackagingTests` run twice after waiting, but the test project currently fails before executing those tests because unrelated LongDoc/layout test sources reference stale symbols such as `DocLayoutYoloService`, `LayoutDetectionStrategy`, and ambiguous `DocumentOutputMode`. Those files were left untouched.
- 2026-06-11: Reconfirmed the first rs portable payload and native PDF fallback boundary.
  - No new dependency was added. The LongDoc PDF path keeps the existing Rust-native stack: `pdfium-render` / `lib/easydict-pdf-render` as an optional source-extraction first pass, then `pdf-extract`, `lopdf` content-stream fallback, and OCR fallback.
  - A read-only subagent audit found no default rs path that starts `.NET` runtime, retained workers, CompatHost, `dotnet.exe`, or PowerShell helpers. An actual Debug no-zip `Package-Portable.ps1` smoke produced the six allowed first-release files and passed `validate-rs-portable`, with no `dotnet/`, `workers/`, runtimeconfig, or `.NET` DLL payload.
  - Added a private `read_native_pdf_source_chunks_with_fallbacks(...)` helper and regression proving a PDF source-extractor failure recovers through native text fallback before OCR and without any worker route.
  - Re-ran focused LongDoc PDF and native text runner behavior tests after the helper split; the existing output precheck regressions still prove `Both`/`Bilingual` modes fail before writing partial TXT/MD/PDF sidecar outputs.
- 2026-06-11: Locked the legacy Inno installer behind the explicit Hybrid profile.
  - No new dependency was needed. A read-only packaging subagent found the core rs portable path already forces Rust-only and validates payloads, but the old `Build-Installer.ps1` entry point still lacked its own runtime-profile gate.
  - `dotnet/scripts/Build-Installer.ps1` now requires `-RuntimeProfile Hybrid` before probing Inno Setup or packaging the legacy dotnet publish output. Missing profiles, `rust-only` aliases, and unknown values fail locally and redirect first-release callers to `rs/scripts/Package-Portable.ps1`.
  - `dotnet/Makefile` now passes `$(RUNTIME_PROFILE)` into all `installer-*` targets, so the publish and installer stages cannot silently diverge on runtime policy.
  - Strengthened the OCR no-worker regression by running the app-dir OCR path against a temp directory containing stale `workers/ocr` and `Easydict.CompatHost.exe` markers; Windows Native OCR still stays on the Rust-native local path and reports no worker/CompatHost wording.
- 2026-06-11: Let Makefile MSIX validation use the Rust validator's Rust-only default.
  - No new dependency was needed. A read-only packaging subagent confirmed the old `validate-msix` recipe passed `--runtime-profile "$(RUNTIME_PROFILE)"` even when the Makefile default was empty, turning the safe Rust-only default into a CLI parse error.
  - `dotnet/Makefile validate-msix` now normalizes `RUNTIME_PROFILE` and passes `--runtime-profile` only when the caller provided a value; with an empty profile it calls `easydict_msix_validate -- "$(MSIX)"`, letting the validator's default `runtime-profile=rust-only` policy reject retained workers/runtime.
  - Extended `release_contract_behavior` so the Makefile target must keep the explicit-profile and omitted-profile branches and cannot reintroduce `--runtime-profile "$(RUNTIME_PROFILE)"`.
- 2026-06-11: Added native LongDoc `resultJsonPath` sidecar parity.
  - No new dependency was needed. The native runner reuses existing `serde_json` and the same output preflight helpers used by TXT/MD/PDF outputs.
  - Native Text/Markdown/simple-PDF LongDoc now prechecks `TranslateDocumentParams.result_json_path` before provider translation starts. A sidecar path that is a directory fails locally before any provider call or partial output write.
  - Successful native runs now write the full `TranslateDocumentResult` JSON to the requested sidecar path and return that path in `resultJsonPath`, closing another retained LongDoc worker parity gap for large/result-file workflows.
- 2026-06-11: Added a LocalAI route-decision matrix for Rust-native-first routing.
  - No new dependency was needed. This is a route-contract regression over the existing Quick Translate decision function.
  - `quick_translate_behavior` now covers Auto/WindowsAI/Foundry/OpenVINO translation, streaming, grammar, cache-ready/cache-missing, and target-auto cases in one table-driven test. The same matrix is also checked under `retained-dotnet-workers` with an explicit hybrid worker policy, proving these native/preflight routes still do not fall through to `RetainedWorkerCompat`.
- 2026-06-11: Wired LongDoc `resultJsonPath` through the Rust CLI and PowerShell shim.
...
- 2026-06-13: Decoupled startup activation parsing from WinFluent task types.
  - No new dependency was needed. Startup activation is only CLI/protocol parsing plus internal message selection, so the route now stays in `activation.rs` as pure app-core logic and `lib.rs` wraps the optional message into a WinFluent `Task` at the application boundary.
  - `startup_activation_message_for_args(...)` now covers the OCR activation message mapping without importing `win_fluent`; the existing `startup_activation_task_for_args(...)` API remains available from the crate root for app startup and tests.
  - Added a default boundary guard so `activation.rs` cannot reintroduce `win_fluent`, `Task<`, or `Task::` while the Quick Translate behavior test continues proving startup OCR activation reuses the hotkey capture-overlay path.
- 2026-06-13: Made the packaged Rust GUI entrypoint default to production state instead of preview demo state.
  - No new dependency was needed. `easydict_preview_iced.exe` is still the first-release binary copied to `Easydict.Rust.exe`, but the wrapper now only enters preview harness behavior when an `EASYDICT_PREVIEW_*` environment variable is present.
  - Without preview env, the packaged GUI loads real settings from `default_settings_storage_path()`, uses `main_window_options_for_settings(...)`, clears demo source text/results/LongDoc history/floating-window sample content, and snapshots loaded settings as saved settings.
  - Preview-only affordances remain available for UI automation and screenshots: preview title/placement/sizing, schema dump, auto-toggle/scroll, and pop-button preview state still run when the preview env prefix is present.
- 2026-06-13: Hid `.NET` runtime extraction from the default packager CLI/API surface.
...
- `cargo check --manifest-path rs/Cargo.toml -p easydict_preview_iced --all-targets`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_packager --test release_contract_behavior pack_rs_portable_zip_extracts_to_gui_entrypoint_smoke_without_dotnet_or_powershell -- --exact --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_packager rust_portable -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_packager dotnet_runtime -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_packager --features hybrid-dotnet-runtime-packaging dotnet_runtime -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_packager --test release_contract_behavior dotnet_runtime_extraction_shim_requires_explicit_hybrid_profile -- --exact --nocapture`
- `cargo check --manifest-path rs/Cargo.toml -p easydict_packager --all-targets`
- `cargo check --manifest-path rs/Cargo.toml -p easydict_packager --all-targets --features hybrid-dotnet-runtime-packaging`
- `git diff --check -- rs/crates/easydict_preview_iced/src/main.rs rs/crates/easydict_packager/src/main.rs rs/crates/easydict_packager/src/lib.rs rs/crates/easydict_packager/tests/release_contract_behavior.rs migration-list.md refactor-progress.md`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --test quick_translate_behavior startup_activation -- --nocapture`
- `cargo test --manifest-path rs/Cargo.toml -p easydict_app --test default_api_boundary_behavior startup_activation_core_stays_decoupled_from_winfluent_task -- --exact --nocapture`
- `rustfmt --edition 2021 rs/crates/easydict_app/src/activation.rs rs/crates/easydict_app/src/lib.rs rs/crates/easydict_app/tests/quick_translate_behavior.rs rs/crates/easydict_app/tests/default_api_boundary_behavior.rs`
...
- `cd rs; cargo test -p easydict_app --test long_document_cli_behavior -- --nocapture`
- `cd rs; cargo test -p easydict_packager --test release_contract_behavior translate_long_doc_script_is_rust_only_and_rejects_dotnet_legacy_mode -- --nocapture`
- `rustfmt --edition 2021 --check rs/crates/easydict_app/src/long_document_cli.rs rs/crates/easydict_app/tests/long_document_cli_behavior.rs rs/crates/easydict_app/tests/quick_translate_behavior.rs rs/crates/easydict_packager/tests/release_contract_behavior.rs`
- `cd rs; cargo test -p easydict_app --test long_document_behavior native_text_long_document_writes_result_json_sidecar -- --nocapture`
- `cd rs; cargo test -p easydict_app --test long_document_behavior native_text_result_json_path_prechecked_before_provider_translation -- --nocapture`
- `cd rs; cargo test -p easydict_app --test long_document_behavior native_text_long_document_ --jobs 2 -- --nocapture`
- `cd rs; cargo test -p easydict_app --test long_document_behavior output_precheck --jobs 2 -- --nocapture`
- `cd rs; cargo check -p easydict_app --all-targets`
- `cd rs; cargo check -p easydict_app --all-targets --features retained-dotnet-workers`
- `cd rs; cargo test -p easydict_packager --test release_contract_behavior legacy_dotnet_packaging_paths_reject_rust_only_and_require_hybrid_profile -- --nocapture`
- `cd rs; cargo test -p easydict_msix_validate runtime_profile -- --nocapture`
- `cd rs; cargo run -p easydict_msix_validate -- --help` (expected exit code 2; confirms `runtime-profile=rust-only` default in usage)
- `cd rs; cargo test -p easydict_packager --test release_contract_behavior -- --nocapture`
- `cd rs; cargo check -p easydict_packager --all-targets`
...
- `cd rs; cargo check -p easydict_app --all-targets`
- PowerShell parser check for `dotnet\scripts\Build-Installer.ps1`
- `pwsh -NoProfile -File dotnet\scripts\Build-Installer.ps1 -Platform x64` (expected failure: explicit Hybrid required)
- `pwsh -NoProfile -Command "try { & .\dotnet\scripts\Build-Installer.ps1 -Platform x64 -RuntimeProfile rust-only; throw 'unexpected success' } catch { $_.Exception.Message }"` (expected failure: first rs release is portable-only)
- `$out = Join-Path $env:TEMP ("easydict-rs-portable-smoke-" + [guid]::NewGuid().ToString("N")); pwsh -NoProfile -File rs\scripts\Package-Portable.ps1 -Platform x64 -Configuration Debug -OutputRoot $out -PackageVersion smoke -NoZip`
- `cd rs; cargo test -p easydict_app native_pdf_source_extractor_error_uses_text_fallback_before_ocr --lib -- --nocapture`
- `cd rs; cargo test -p easydict_app --test long_document_behavior native_pdf --jobs 2 -- --nocapture`
- `cd rs; cargo test -p easydict_app --test long_document_behavior native_text_long_document_runner_ --jobs 2 -- --nocapture`
- `rustfmt --edition 2021 --check rs\crates\easydict_packager\tests\release_contract_behavior.rs`
- `rustfmt --edition 2021 --check lib\rs-mdict\src\mdd.rs lib\rs-mdict\src\ripemd128.rs rs\crates\easydict_app\src\mdx_native.rs rs\crates\easydict_app\tests\mdx_native_behavior.rs rs\crates\easydict_packager\tests\release_contract_behavior.rs`
- `cargo fmt --manifest-path lib\rs-mdict\Cargo.toml --check`
- `cd rs; cargo test -p easydict_packager --test release_contract_behavior -- --nocapture`
- `cd rs; cargo check -p easydict_packager --all-targets`
- `cargo test --manifest-path lib\rs-mdict\Cargo.toml --lib mdd -- --nocapture`
...
- `cd rs; cargo check -p easydict_app --all-targets`
- `cd rs; cargo check -p easydict_app --all-targets --features retained-dotnet-workers`
- `cd dotnet; dotnet msbuild src\Easydict.WinUI\Easydict.WinUI.csproj -nologo -getProperty:RuntimeProfile -getProperty:BuildWorkerOutputs -getProperty:EnableInProcLongDocFallback -p:Configuration=Debug` (`RuntimeProfile=RustOnly`, `BuildWorkerOutputs=false`, `EnableInProcLongDocFallback=false`)
- `cd dotnet; dotnet msbuild src\Easydict.WinUI\Easydict.WinUI.csproj -nologo -getProperty:RuntimeProfile -getProperty:BuildWorkerOutputs -getProperty:EnableInProcLongDocFallback -p:Configuration=Debug -p:RuntimeProfile=Hybrid` (`RuntimeProfile=Hybrid`, `EnableInProcLongDocFallback=true`)
- `git diff --check -- .github/workflows/release-publish.yml dotnet/Makefile dotnet/src/Easydict.WinUI/Easydict.WinUI.csproj dotnet/tests/Easydict.WinUI.Tests/Services/WorkerPackagingTests.cs lib/rs-mdict/Cargo.toml lib/rs-mdict/src/mdd.rs lib/rs-mdict/src/ripemd128.rs rs/crates/easydict_app/src/mdx_native.rs rs/crates/easydict_app/tests/mdx_native_behavior.rs rs/crates/easydict_packager/tests/release_contract_behavior.rs migration-list.md refactor-progress.md`
- `cd rs; cargo test -p easydict_packager --test release_contract_behavior rs_portable_release_jobs_stay_isolated_from_dotnet_artifacts -- --nocapture`
- `cd rs; cargo test -p easydict_packager --test release_contract_behavior build_rust_helpers_child_cargo_is_forced_to_rust_only_runtime_profile -- --nocapture`
- `cd rs; cargo test -p easydict_packager --test release_contract_behavior pack_rs_portable_child_cargo_is_forced_to_rust_only_runtime_profile -- --nocapture`
- `cd rs; cargo test -p easydict_app --test default_api_boundary_behavior default_process_spawn_surface_has_no_retained_dotnet_runtime_entries -- --nocapture`
- `cd rs; cargo test -p easydict_app --test default_api_boundary_behavior default_app_keeps_foundry_sdk_adapter_inside_foundry_local_lib -- --nocapture`
- `cd rs; cargo test -p easydict_app --features enable-linguee-service --test quick_translate_behavior feature_enabled_linguee_catalog_entry_routes_to_native_traditional_http -- --nocapture`
- `cd rs; cargo test -p easydict_app --test quick_translate_behavior default_bing_catalog_entry_routes_to_native_two_phase_backend -- --nocapture`
- `cd rs; cargo test -p easydict_app --test quick_translate_behavior native_bing_rejects_unsupported_language_before_fetching_session_page -- --nocapture`
- `cd rs; cargo test -p easydict_app --test traditional_http_behavior bing -- --nocapture`
- `cd rs; cargo test -p easydict_app --test quick_translate_behavior native_traditional_http_quick_translate_supports_google_and_google_web -- --nocapture`
- `cd rs; cargo test -p easydict_app --test long_document_behavior openvino_local_ai_long_document_translates_with_injected_native_nllb_translator -- --nocapture`
- `cd rs; cargo test -p easydict_app --test long_document_behavior native_text_long_document_runner_stops_before_export_when_cancelled_after_first_chunk -- --nocapture`
...
- `cd rs; cargo test -p easydict_app --test native_bridge_behavior -- --nocapture`
- `cd rs; cargo test -p easydict_app --test default_api_boundary_behavior -- --nocapture`
- `cd rs; cargo test -p easydict_app --test cli_translate_behavior auto_local_ai_cli_rejects_foundry_cli_override_targeting_retained_worker_before_spawn -- --nocapture`
- `cd rs; cargo test -p easydict_app --test long_document_cli_behavior foundry_local_cli_override_targeting_retained_worker_is_not_spawned -- --nocapture`
- `cd rs; cargo test -p easydict_app runtime_policy --lib -- --nocapture`
- `cd rs; cargo test -p easydict_app --features retained-dotnet-workers runtime_policy --lib -- --nocapture`
- `cd rs; cargo check -p easydict_app --all-targets`
- `cd rs; cargo check -p easydict_app --all-targets --features retained-dotnet-workers`
- `cargo test --manifest-path lib/easydict-foundry-local/Cargo.toml -- --nocapture`
- `cargo test --manifest-path lib/easydict-foundry-local/Cargo.toml --features sdk -- --nocapture`
- `cd rs; cargo test -p easydict_packager --test release_contract_behavior -- --nocapture`
- `cd rs; cargo check -p easydict_packager --all-targets`
- `cd rs; cargo test -p easydict_msix_validate -- --nocapture`
- `cd rs; cargo check -p easydict_msix_validate --all-targets`
- `cd dotnet; dotnet test tests/Easydict.WinUI.Tests --filter "FullyQualifiedName~ReleaseWorkflow_UsesRustBundleMinVersionValidator" --logger "console;verbosity=minimal" -m:1 -p:UseSharedCompilation=false`
- `cd dotnet; dotnet test tests/Easydict.WinUI.Tests --filter "FullyQualifiedName~ReleaseOrchestration_UsesRustHelpersInsteadOfRetiredDotnetHelperProjects" --logger "console;verbosity=minimal" -m:1 -p:UseSharedCompilation=false`
- Expected-failure smoke for `powershell -NoProfile -ExecutionPolicy Bypass -File scripts/translate-long-doc.ps1 -ListServices -UseDotnetLegacy`, verifying it fails before launching `dotnet`.
...
- `cd rs; cargo check -p easydict_store_listings --all-targets`
...
- `cargo test --manifest-path lib\rs-mdict\Cargo.toml mdd -- --nocapture`
- `cd rs; cargo test -p easydict_app --test mdx_native_behavior mdd -- --nocapture`
- `cd rs; cargo test -p easydict_app --test mdx_native_behavior native_mdx_lookup_inlines_first_hit_from_real_multi_mdd_fixtures -- --nocapture`
- `cd rs; cargo test -p easydict_app --test quick_translate_behavior mdd -- --nocapture`
- `cd rs; cargo test -p easydict_app --test settings_storage_behavior settings_storage_load_discovers_mdd_for_legacy_mdx_entries_without_saved_paths -- --nocapture`
- `cd rs; cargo test -p easydict_app --test protocol_behavior mdx_lookup_dtos_are_serializable_for_native_mdx_contract -- --nocapture`
- `cd rs; cargo test -p easydict_app --test credential_protection_behavior credential_protection -- --nocapture`
- `cd rs; cargo test -p easydict_app --test ocr_behavior bgra_to_base64 -- --nocapture`
- `cargo test --manifest-path lib\rs-mdict\Cargo.toml ripemd128 -- --nocapture`
- `cd rs; cargo test -p easydict_app --test mdx_native_behavior mdx_encryption_ripemd128_matches_mdict_csharp_vectors -- --nocapture`
- `cd rs; cargo test -p easydict_app --test mdx_native_behavior native_mdx_record_encrypted_dictionary_with_credentials_reads_natively -- --nocapture`
- `cd rs; cargo test -p easydict_app --test mdx_native_behavior native_mdx_record_encrypted_dictionary_with_device_id_credentials_reads_natively -- --nocapture`
- `cd rs; cargo test -p easydict_app --test long_document_behavior native_text_both_output_prechecks_bilingual_path_before_writing_monolingual_file -- --nocapture`
- `cd rs; cargo test -p easydict_app --test long_document_behavior native_pdf_both_output_prechecks_bilingual_text_path_before_writing_pdf_file -- --nocapture`
...
- `cd rs; cargo test -p easydict_app --features retained-dotnet-workers runtime_policy --lib -- --nocapture`
- `cd rs; cargo test -p easydict_app cli_translate::tests --lib -- --nocapture`
- `cd rs; cargo test -p easydict_app --test cli_translate_behavior local_ai_cli_host_and_app_dir_hints_are_legacy_noops_together -- --nocapture`
- `cd rs; cargo test -p easydict_app --test cli_translate_behavior local_ai_cli_ -- --nocapture`
- `cd rs; cargo test -p easydict_app --test default_api_boundary_behavior -- --nocapture`
...
- `cd rs; cargo test -p easydict_app runtime_policy --lib -- --nocapture`
- `cd rs; cargo test -p easydict_app --features retained-dotnet-workers runtime_policy::tests::dotnet_named_runtime_profiles_do_not_enable_retained_workers --lib -- --nocapture`
...
- `cd rs; cargo test -p easydict_app runtime_policy --lib -- --nocapture`
- `cd rs; cargo test -p easydict_app --features retained-dotnet-workers runtime_policy --lib -- --nocapture`
- `cd rs; rustfmt --edition 2021 --check crates/easydict_app/src/runtime_policy.rs crates/easydict_app/src/lib.rs`
- `cd rs; cargo test -p easydict_packager validate_rs_portable -- --nocapture`
- `cd rs; cargo test -p easydict_packager extract_dotnet_runtime -- --nocapture`
- `cd rs; cargo fmt -p easydict_packager --check`
- `cd dotnet; dotnet test tests\Easydict.WinUI.Tests\Easydict.WinUI.Tests.csproj --filter FullyQualifiedName~WorkerPackagingTests --logger "console;verbosity=minimal" -m:1 -p:UseSharedCompilation=false`
...
- `cd rs; cargo check -p easydict_app --all-targets`
- `cd rs; cargo check -p easydict_app --all-targets --features retained-dotnet-workers`
- `cd rs; rustfmt --edition 2021 --check crates/easydict_app/src/quick_translate.rs crates/easydict_app/src/long_document.rs crates/easydict_app/src/runtime_policy.rs crates/easydict_app/src/mdx_native.rs crates/easydict_app/src/lib.rs crates/easydict_app/src/bin/easydict_cli.rs crates/easydict_app/src/cli_translate.rs crates/easydict_app/src/long_document_cli.rs crates/easydict_app/tests/protocol_behavior.rs crates/easydict_app/tests/quick_translate_behavior.rs crates/easydict_app/tests/long_document_behavior.rs crates/easydict_app/tests/mdx_native_behavior.rs crates/easydict_app/tests/cli_translate_behavior.rs`
...
- Temp-output smoke for `rs/scripts/Package-Portable.ps1 -Platform x64 -Configuration Debug -NoZip`, verifying `pack-rs-portable` stages 6 Rust-only entries.
- Temp-output smoke for `rs/scripts/Package-Portable.ps1 -Platform x64 -Configuration Debug -PackageVersion v0.0.0-pack-rs-portable`, verifying `pack-rs-portable` validates both the staged directory and final ZIP with 6 entries.
- `cd rs; cargo check -p easydict_app --all-targets`
- `cd rs; cargo check -p easydict_app --all-targets --features retained-dotnet-workers`
- `cd rs; cargo test -p easydict_app --test protocol_behavior -- --nocapture`
- `cd rs; cargo test -p easydict_app --features retained-dotnet-workers --test protocol_behavior -- --nocapture`
...
- `cd rs; cargo test -p easydict_app runtime_policy --lib -- --nocapture`
- PowerShell parser check for `rs/scripts/Package-Portable.ps1`
- `cd rs; rustfmt --edition 2021 --check crates/easydict_app/src/runtime_policy.rs`
- `cd dotnet; dotnet test tests\Easydict.WinUI.Tests\Easydict.WinUI.Tests.csproj --filter FullyQualifiedName~WorkerPackagingTests --logger "console;verbosity=minimal" -m:1 -p:UseSharedCompilation=false`
- Temp-output smoke for `rs/scripts/Package-Portable.ps1 -Platform x64 -Configuration Debug -NoZip`, verifying the staged rs portable package contains only `Easydict.Rust.exe`, Rust helper executables, and `README-portable.txt` while forbidden `.NET` payload entries and the legacy `BrowserHostRegistrar.exe` alias are absent.
- `git diff --check -- rs/scripts/Package-Portable.ps1 rs/crates/easydict_app/src/runtime_policy.rs dotnet/tests/Easydict.WinUI.Tests/Services/WorkerPackagingTests.cs migration-list.md refactor-progress.md rs/README.md`
