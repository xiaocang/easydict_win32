...
pub fn try_unprotect_credential(protected_value: &str) -> Option<String> {
    ...
    try_unprotect_nested(protected_value).map(|result| result.0)
}
...
pub fn unprotect_or_return_plaintext(stored_value: Option<&str>) -> CredentialPlaintext {
    ...
    if is_protected_credential(Some(stored_value)) {
        if let Some((value, needs_migration)) = try_unprotect_nested(stored_value) {
            return CredentialPlaintext {
...
fn try_unprotect_nested(protected_value: &str) -> Option<(String, bool)> {
    let mut current_value = protected_value.to_string();
    let mut needs_migration = false;

    for depth in 0..MAX_NESTED_PROTECTED_VALUE_DEPTH {
        let (unprotected_value, used_legacy_protection) = try_unprotect_single(&current_value)?;
...
fn try_unprotect_single(protected_value: &str) -> Option<(String, bool)> {
    if let Some(plaintext) = try_unprotect_dpapi(protected_value) {
        return Some((plaintext, false));
    }

    if protected_value.starts_with(LEGACY_PROTECTED_VALUE_PREFIX) {
        return None;
    }
